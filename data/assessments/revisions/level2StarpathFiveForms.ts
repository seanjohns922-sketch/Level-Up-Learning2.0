...
function shapeTasks(f:number):(GroundTask|EdgeTask)[]{
 ...
 return [
  ...
  {mode:'choice',prompt:'Which shape has exactly one pair of parallel sides?',options:shapeOptions([shape('trapezoid'),shape('rectangle'),parallelogram(f)],f+2),correctIds:['o0']},
  words('What is true about both shapes?',['Both have four straight sides.','Both have all four sides equal.','Both have two pairs of parallel sides.'],[shape('rectangle'),irregular(f)],2),
  {mode:'choice',prompt:'Which shape does not belong with the others?',options:shapeOptions([shape(family),shape(family,40),shape(odd,75),shape(family,110)],f),correctIds:['o2'],reasons:options([groupReason,f%2?'It has four sides. The others have three.':'It has five sides. The others have four.',f%2?'It has fewer straight sides than the others.':'It has more straight sides than the others.'],f+1),correctReason:'o0'},
  {mode:'multi',prompt:'Choose every shape with two pairs of parallel sides.',instruction:'Choose all that belong, then choose a reason.',options:shapeOptions([shape('square'),shape('triangle'),parallelogram(f),shape('trapezoid'),shape('rectangle'),shape('hexagon')],f+2),correctIds:['o0','o2','o4'],reasons:options(['Both pairs of opposite sides stay the same distance apart.','Only one pair of opposite sides stays the same distance apart.','The sides next to each other stay the same distance apart.'],f),correctReason:'o0'},
 ];
}
const scenes=[
...
function mapTasks(f:number):Level2MapTask[]{
 ...
 return [
  ...
  {...common,mode:'choice',prompt:`Which directions take you from ${scene.labels[0]} to ${scene.labels[3]}?`,start,options:options([moves(['up','up','right']),moves(['right','right','up']),moves(['up','right','right'])].map(directionsText),f+1),correctId:'o0',goal:checkpoint},
