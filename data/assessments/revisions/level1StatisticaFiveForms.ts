...
export type StatsItem = {
 id:string;slot:number;skillLabel:string;code:'AC9M1ST01'|'AC9M1ST02'|'AC9M2ST01'|'AC9M2ST02'|'AC9M3ST01'|'AC9M3ST02'|'AC9M3ST03'|'AC9M4ST01'|'AC9M4ST02'|'AC9M4ST03'|'AC9M5ST01'|'AC9M5ST02'|'AC9M5ST03'|'AC9M6ST01'|'AC9M6ST02'|'AC9M6ST03'|'AC9M7ST01'|'AC9M7ST02'|'AC9M7ST03'|'AC9M8ST01'|'AC9M8ST02'|'AC9M8ST03'|'AC9M8ST04';week:number;difficulty:'easy'|'moderate'|'challenging';
 prompt:string;instruction:string;context:string;categories:Category[];counts:number[];observations:number[];
 mode:'choice'|'counts'|'list'|'sort'|'leaves';source:'categories'|'objects'|'responses'|'graph'|'missing'|'duplicate'|'table'|'dual'|'brief'|'pictograph'|'comparison'|'line'|'records'|'dataset'|'stem';
 display:'objects'|'pictures'|'symbols'|'tally'|'columns'|'frequency'|'pictograph'|'dots';options:StatsOption[];answer:string|number[];
 numerical?:boolean;valueLabel?:string;axisLabel?:string;graphMin?:number;graphMax?:number;graphStep?:number;sourceText?:string;
 sourceLabel?:string;graphKind?:'pictures'|'columns';secondaryKind?:'pictures'|'columns';secondaryOrder?:number[];
 showValues?:boolean;sampleSetup?:{unit:string;population:number;selected:number[]};
 plantSetup?:{plant:string;weeks:number;perGroup?:number};
 dataset?:number[];stems?:number[];leafAnswer?:string[];
 linePoints?:{label:string;value:number}[];recordIds?:string[];
