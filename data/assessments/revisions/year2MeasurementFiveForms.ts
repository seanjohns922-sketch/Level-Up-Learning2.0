...
function make(form:typeof YEAR2_MEASUREMENT_FORMS[number],f:number){
 ...
 add('How many days pass from planting to checking?',String(5+f%2),v('calendar',`Planting: ${start} ${months[month]}. Checking: ${start+5+f%2} ${months[month]}. Count the one-day jumps from planting to checking.`,{month,dates:[start,start+5+f%2]}));
 ...
 const later=start+8;add(`What date is four days after ${later} ${months[month]}?`,String(later+4),v('calendar',`Start at ${later} ${months[month]}. Count forward four one-day jumps. Enter the date number.`,{month,dates:[later]}));
 ...
 const twoWeeks=3+f;add(`What date is two weeks after ${twoWeeks} ${months[month]}?`,String(twoWeeks+14),v('calendar',`Start at ${twoWeeks} ${months[month]}. Count forward two weeks. Enter the date number.`,{month,dates:[twoWeeks]}));
 ...
 return items.map((q,i)=>{const [code,skill,difficulty]=YEAR2_MEASUREMENT_BLUEPRINT[i];return {...q,id:`y2-measurement-review-${form}-${String(i+1).padStart(2,'0')}`,version:'1.0.0-review.1',form,type:q.options?'mcq':'numeric',options:q.options?[...q.options.slice((f+i)%q.options.length),...q.options.slice(0,(f+i)%q.options.length)]:undefined,readAloudText:[2,7,13].includes(i)?`${q.prompt} ${q.visual.description}`:q.prompt,primaryDescriptorCode:code,curriculumCodes:[code],skillId:`y2-measurement-slot-${i+1}`,skillLabel:skill,structureKey:`y2-measurement-slot-${i+1}`,difficulty,statistics:createUncalibratedItemStatistics(difficulty),scoring:{kind:'exact',correctResponse:q.correctAnswer},inputMode:'numeric',strand:'Measurement'};});
}
