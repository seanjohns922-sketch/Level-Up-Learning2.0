...
function make(form:StatisticaForm,f:number,slot:number):StatsItem{
 ...
 case 4:record('tally');q.sourceLabel='Draw';q.context='Counter experiment';q.prompt='Record the results with tally marks.';q.instruction='A counter was drawn and put back each time. Record the ten results shown.';break;
 ...
 case 16:{q.graphKind='columns';const high=counts.indexOf(10),low=counts.indexOf(5);q.prompt=`How many more chose ${name(high)} than ${name(low)}?`;q.instruction='Read the column heights.';choose(['5','15','10','3']);break;}
 ...
 case 20:dual();q.prompt='Which finding matches both graphs?';choose([`${name(most)} was chosen more than ${name(least)}.`,`${name(least)} was chosen more than ${name(most)}.`,'All four groups have equal counts.',`No one chose ${name(least)}.`]);break;
 }
 return q;
}
