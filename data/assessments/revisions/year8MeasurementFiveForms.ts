...
export type Measurement8Visual={type:'measurement_year8_panel';task:'composite'|'prism'|'circle'|'timezone'|'rate'|'rightTriangle'|'model'|'house'|'diagonal'|'packing';orientation?:number;values:number[];description:string;labels?:string[];unit?:string;variant?:string;unknown?:'height'|'hypotenuse';rows?:{label:string;offset:number;detail:string}[]};
const slots=[
...
function make(form:typeof YEAR8_MEASUREMENT_FORMS[number],f:number){
 ...
 const cartonL=62+20*f;
 add('How many whole tissue boxes fit in this shipping carton?',Math.floor(cartonL/20)*Math.floor(34/10)*Math.floor(27/8),v('packing','Keep all boxes upright in straight rows. Match length to length, width to width and height to height; do not turn the boxes. Carton dimensions are internal and tissue-box dimensions are external.',[cartonL,34,27,20,10,8],{unit:'cm'}),'boxes');
 ...
 return items.map((q,i)=>{const[code,skill,difficulty]=YEAR8_MEASUREMENT_BLUEPRINT[i];return {...q,visual:{...q.visual,orientation:f%4},id:`y8-measurement-review-${form}-${String(i+1).padStart(2,'0')}`,version:'1.1.0-review.1',form,type:q.options?'mcq':'numeric',options:q.options?[...q.options.slice((f+i)%q.options.length),...q.options.slice(0,(f+i)%q.options.length)]:undefined,readAloudText:q.prompt,primaryDescriptorCode:code,curriculumCodes:[code],skillId:`y8-measurement-slot-${i+1}`,skillLabel:skill,structureKey:`y8-measurement-slot-${i+1}`,difficulty,statistics:createUncalibratedItemStatistics(difficulty),scoring:{kind:'exact',correctResponse:q.correctAnswer},inputMode:'decimal' as const,strand:'Measurement'};});
}
