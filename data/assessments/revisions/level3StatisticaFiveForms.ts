...
function make(form:StatisticaForm,f:number,slot:number):StatsItem{
 ...
 const counts=rotate(readTask?(numeric?[20,30,50,10,40]:[40,20,50,30]):[11,15].includes(slot)?[20,30,40,25]:slot===17?[4,6,10,2,8]:numeric?[3,4,5,2,4]:[6,4,3,5],f);
 const categories=(numeric?['0','1','2','3','4']:cat.names).map((name,i)=>({name,color:numeric?'#3279bd':colors[i]}));
 const q:StatsItem={id:`statistics-l3-v1-${form}-${String(slot).padStart(2,'0')}`,slot,skillLabel:labels[slot-1],code:slot<=7?'AC9M3ST01':slot<=14?'AC9M3ST03':'AC9M3ST02',week:weeks[slot-1],difficulty:[1,2,8,15].includes(slot)?'easy':[7,13,14,17,19,20].includes(slot)?'challenging':'moderate',context:numeric?c.title:cat.title,categories,counts,observations:observations(counts,f+slot),mode:'choice',source:'table',display:'frequency',options:[],answer:'',prompt:'',instruction:'',numerical:numeric,valueLabel:numeric?c.unit:'Group',axisLabel:'Children',sourceLabel:'Child',graphMax:readTask?60:12,graphStep:readTask?5:1};
 ...
 if([11,15,16].includes(slot)){q.graphMax=50;q.graphStep=5;q.responseMax=50;q.responseStep=5;}
 ...
 case 11:build('columns','table');q.prompt='Graph the investigation results.';q.instruction='Use the frequency table. The scale goes up in fives. Type each frequency or use + and −.';break;
 ...
 case 15:build('columns','table');q.prompt='Build a graph of these categories.';q.instruction='Set every column to match its frequency. The scale goes up in fives.';break;
 case 16:q.counts=rotate([15,0,30,20,10],f);q.observations=observations(q.counts,f);build('columns','table');q.prompt='Build a graph of these numerical answers.';q.instruction='Use the scale in fives. Keep all values from 0 to 4 on the bottom axis, even when no child gave that answer.';break;
 ...
 case 18:q.counts=rotate([20,50,30,40],f);q.graphMax=60;q.graphStep=5;q.source='graph';q.graphKind='columns';{const a=q.counts.indexOf(50),b=q.counts.indexOf(20);q.prompt=`How many more children chose ${categories[a].name} than ${categories[b].name}?`;q.instruction='The scale goes up in fives.';choose(['30','6','70','50']);}break;
