...
function make(form:StatisticaForm,f:number,slot:number):StatsItem{
 ...
 const counts=rotate(numeric?[2,3,4,1,2]:[5,3,2,4],f);
 const categories=(numeric?['0','1','2','3','4']:cat.names).map((name,i)=>({name,color:numeric?'#3279bd':colors[i]}));
 const q:StatsItem={id:`statistics-l3-v1-${form}-${String(slot).padStart(2,'0')}`,slot,skillLabel:labels[slot-1],code:slot<=7?'AC9M3ST01':slot<=14?'AC9M3ST03':'AC9M3ST02',week:weeks[slot-1],difficulty:[1,2,8,15].includes(slot)?'easy':[7,13,14,17,19,20].includes(slot)?'challenging':'moderate',context:numeric?c.title:cat.title,categories,counts,observations:observations(counts,f+slot),mode:'choice',source:'table',display:'frequency',options:[],answer:'',prompt:'',instruction:'',numerical:numeric,valueLabel:numeric?c.unit:'Group',axisLabel:'Children',sourceLabel:'Child'};
 ...
 case 4:q.prompt='How many children gave the answer 2?';q.instruction=`Dataset: ${c.question} Frequency means the number of children.`;choose([String(counts[2]),String(counts.reduce((a,b)=>a+b,0)),'2','5'].map((v,i,a)=>i>0&&a.indexOf(v)!==i?String(8+i):v));break;
 ...
 case 16:q.counts=rotate([2,0,4,3,1],f);q.observations=observations(q.counts,f);build('columns','table');q.prompt='Build a graph of these numerical answers.';q.instruction='Keep all values from 0 to 4 on the bottom axis, even when no child gave that answer.';break;
