...
export type GroundMeasurementVisual = {
 ...
 target?:string; scene?:string; art?:string; days?:string[]; context?:string; description:string;
};
...
const tasks=[['Drawing','Building'],['Painting','Reading'],['Clapping','Singing'],['Packing','Tidying'],['Dancing','Walking']];
...
function make(form:GroundMeasurementForm,f:number){
 ...
 add('Which pencil is shorter?',pair[leftLong?1:0],[...pair,'Same length'],visual('length',`Two pencils with their left ends lined up.`,{values:lengths,objects:['pencil','pencil']}));
 ...
 add('Why is the small object heavier?','Its pan is lower',['It looks bigger','Its pan is lower','Its colour is darker'],visual('mass',`A small ${names[0].toLowerCase()} and a larger ${names[1].toLowerCase()} are on a balance. The small object's pan is lower.`,{labels:names,values:[125,75],objects:names,scene:'size-trap'}));
 const source=f%2;
 ...
 const morningArt=['routine-3d/routine-wakeup.png','routine-3d/routine-breakfast.png','routine-3d/routine-school.png','routine-3d/routine-reading.png','routine-3d/routine-art.png'][f];
 const nightArt=['routine-3d/routine-bed.png','timeofday-3d/reading-bed.png','routine-3d/routine-bath.png','routine-3d/routine-dinner.png','timeofday-3d/sleeping.png'][f];
 const morningScene=['wakes up','eats breakfast','gets ready for school','reads a story','draws a picture'][f];
 const nightScene=['gets ready for bed','reads in bed','has a bedtime bath','eats dinner','sleeps in bed'][f];
 add('Which activity lasts longer?',events[leftLong?0:1],[...events,'Same time'],visual('duration','Both activities start together. Each strip ends when that activity finishes.',{labels:events,values:lengths,objects:events}));
 add('Which activity finishes sooner?',events[leftLong?1:0],[...events,'Same time'],visual('duration','Both activities start together. Each strip ends when that activity finishes.',{labels:events,values:lengths,objects:events}));
 add('What time of day is shown?','Morning',['Morning','Afternoon','Night'],visual('daypart',`${children[f]} ${morningScene} as the sun rises.`,{target:'morning',context:children[f],scene:`${children[f]} ${morningScene}.`,art:morningArt}));
 add('What time of day is shown?','Night',['Morning','Lunchtime','Night'],visual('daypart',`${children[f]} ${nightScene} under a dark sky with the moon and stars.`,{target:'night',context:children[f],scene:`${children[f]} ${nightScene}.`,art:nightArt}));
 const day=f;
