...
export type Measurement7Visual={type:'measurement_year7_panel';task:'triangle'|'parallelogram'|'rectPrism'|'triPrism'|'circle'|'parallel'|'triangleAngles'|'quadAngles'|'ratio'|'pairTriangles'|'pairPrisms';values:number[];description:string;labels?:string[];unit?:string;unknown?:'height'|'length';relation?:'corresponding'|'alternate'|'cointerior';circleMeasure?:'radius'|'diameter';angleSumModel?:boolean;};
export const YEAR7_MEASUREMENT_BLUEPRINT=[
 ...
 ['AC9M7M01','Find a parallelogram height from area','challenging'],['AC9M7M02','Convert a triangular prism volume to capacity','challenging'],['AC9M7M03','Calculate circumference from radius','moderate'],['AC9M7M03','Find diameter from circumference','challenging'],['AC9M7M04','Justify corresponding angles','moderate'],['AC9M7M04','Calculate co-interior angles','moderate'],['AC9M7M05','Explain a triangle angle calculation','moderate'],['AC9M7M05','Derive a quadrilateral angle sum','moderate'],['AC9M7M06','Adjust a mixture to restore a ratio','challenging'],['AC9M7M06','Scale a recipe to a target total','challenging'],
] as const;
function make(form:typeof YEAR7_MEASUREMENT_FORMS[number],f:number){
 ...
 const pgBase=18+2*f,pgHeight=7+f;
 add('What is the perpendicular height of this parallelogram?',pgHeight,v('parallelogram','Find the unknown perpendicular height using the labelled area and base.',[pgBase,pgHeight],{unknown:'height',unit:'m',labels:[`Area: ${pgBase*pgHeight} m²`]}),'m');
 const tankBase=20+4*f,tankHeight=10+2*f,tankLength=30+2*f;
 add('How many litres of water can this triangular-prism tank hold?',tankBase*tankHeight*tankLength/2000,v('triPrism','These are the inside dimensions. The tank is full. 1000 cubic centimetres equals 1 litre.',[tankBase,tankHeight,tankLength],{unit:'cm'}),'L');
 const wheelRadius=15+5*f;
 add('What is the circumference of this circular wheel?',Number((2*wheelRadius*3.14).toFixed(2)),v('circle','The marked length is the radius. Use 3.14 for pi. Give your answer to two decimal places.',[wheelRadius],{circleMeasure:'radius',unit:'cm'}),'cm');
 const inverseDiameter=10+4*f,circ=Number((inverseDiameter*3.14).toFixed(2));
 add('What is the diameter of this circle?',inverseDiameter,v('circle','Find the unknown diameter d. Use 3.14 for pi.',[inverseDiameter],{circleMeasure:'diameter',labels:['symbolic',`Circumference: ${circ} cm`],unit:'cm'}),'cm');
 const corresponding=[110,65,120,70,115][f],correspondingAnswer=`${corresponding}°, because corresponding angles are equal.`;
 add('Which answer gives the size of x and a valid reason?',correspondingAnswer,v('parallel','The two marked lines are parallel.',[corresponding],{relation:'corresponding'}),undefined,[correspondingAnswer,`${180-corresponding}°, because corresponding angles add to 180°.`,`${corresponding}°, because every angle on a transversal is equal.`]);
 const interior=[115,125,110,130,105][f];
 add('What is the size of the co-interior angle marked x?',180-interior,v('parallel','The two marked lines are parallel.',[interior],{relation:'cointerior'}),'°');
 const givenA=35+f*5,givenB=70+f*5,missing=180-givenA-givenB,triangleReason=`${missing}°, because the three interior angles add to 180°.`;
 add('Which answer gives the missing angle and explains the calculation?',triangleReason,v('triangleAngles','Find the top angle marked x and choose the correct reason.',[givenA,givenB,missing]),undefined,[triangleReason,`${360-givenA-givenB}°, because the angles add to 360°.`,`${givenA+givenB}°, because x equals the sum of the other two angles.`]);
 const quadA=55+f*5,quadB=65+f*5;
 add('Why do the four interior angles of this quadrilateral add to 360°?','The diagonal makes two triangles, each with an angle sum of 180°.',v('quadAngles','A diagonal joins two opposite corners. Consider the angles in the two triangles.',[quadA,quadB,180-quadB,180-quadA],{angleSumModel:true}),undefined,['The diagonal makes two triangles, each with an angle sum of 180°.','Every quadrilateral must have four right angles.','The diagonal makes four triangles, each with an angle sum of 180°.']);
 const recipeWater=3+f,baseConcentrate=100+25*f,extraConcentrate=50+10*f;
 add('How many millilitres of water must be added to restore the recipe ratio?',extraConcentrate*recipeWater,v('ratio','The drink starts in the correct ratio. Extra concentrate is added. Add only water to restore the original ratio.',[1,recipeWater,baseConcentrate,baseConcentrate*recipeWater,extraConcentrate],{labels:[`Concentrate : water = 1 : ${recipeWater}`,`Starting drink: ${baseConcentrate} mL concentrate and ${baseConcentrate*recipeWater} mL water`,`Extra concentrate added: ${extraConcentrate} mL`],unit:'mL'}),'mL');
 const targetParts=4+f,targetLitres=(targetParts+1)*(1+f/2);
 add('How many millilitres of concentrate are needed to make the full amount?',1000*(1+f/2),v('ratio','Keep the recipe ratio when making the larger amount. 1 litre equals 1000 millilitres.',[1,targetParts,targetLitres],{labels:[`Concentrate : water = 1 : ${targetParts}`,`Total drink needed: ${targetLitres} litres`],unit:'mL'}),'mL');
 return items.map((q,i)=>{const[code,skill,difficulty]=YEAR7_MEASUREMENT_BLUEPRINT[i];return {...q,id:`y7-measurement-review-${form}-${String(i+1).padStart(2,'0')}`,version:'1.1.0-review.1',form,type:q.options?'mcq':'numeric',options:q.options?[...q.options.slice((f+i)%q.options.length),...q.options.slice(0,(f+i)%q.options.length)]:undefined,readAloudText:q.prompt,primaryDescriptorCode:code,curriculumCodes:[code],skillId:`y7-measurement-slot-${i+1}`,skillLabel:skill,structureKey:`y7-measurement-slot-${i+1}`,difficulty,statistics:createUncalibratedItemStatistics(difficulty),scoring:{kind:'exact',correctResponse:q.correctAnswer},inputMode:'decimal' as const,strand:'Measurement'};});
}
