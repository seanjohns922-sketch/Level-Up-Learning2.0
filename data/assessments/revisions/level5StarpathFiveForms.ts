...
function nets(f:number):NetTask[]{
 ...
 return [
  {mode:'choice',prompt:'Which object can be folded from this net?',instruction:'Imagine joining the outside edges. Ignore glue tabs.',solid,angle:f*90,options:opts([name[solid],...(['triPrism','pyramid','cuboid'] as const).filter(k=>k!==solid).map(k=>name[k]),'Cylinder'],f),correctId:'o0'},
