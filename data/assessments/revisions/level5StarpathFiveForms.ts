...
function nets(f:number):NetTask[]{
 ...
 const omittedFace=order(valid.map((_,i)=>i).filter(i=>i!==marked&&i!==opposite),f)[0];
 const netOptions=order([valid,...order(['block','ell','plusStub'],f).map((id,i)=>orient(HEXOMINOES[id],f+i))].map((cells,i)=>({id:`o${i}`,label:`Net ${String.fromCharCode(65+i)}`,cells})),f);
 ...
 return [
  {mode:'choice',prompt:'Which object can be folded from this net?',instruction:'Imagine joining the outside edges. Ignore glue tabs.',solid,angle:f*90,options:opts([name[solid],...(['triPrism','pyramid','cuboid'] as const).filter(k=>k!==solid).map(k=>name[k]),'Triangular pyramid'],f),correctId:'o0'},
  ...
  {mode:'choice',prompt:`Which face will be opposite face ${String.fromCharCode(65+marked)} when this net folds?`,instruction:'Opposite faces do not share an edge on the cube.',cells:valid,marked,options:order(valid.map((_,i)=>({id:`o${i}`,label:`Face ${String.fromCharCode(65+i)}`})).filter(o=>o.id!==`o${marked}`&&o.id!==`o${omittedFace}`),f),correctId:`o${opposite}`},
  {mode:'choice',prompt:'Why can this arrangement not make a closed cube?',instruction:'All six squares are the same size. Imagine folding along their shared edges.',cells:invalid,options:opts(['Some faces would overlap, leaving part of the cube uncovered.','A cube needs eight square faces, so two squares are missing.','Opposite faces must already share an edge in the flat net.','Every cube net must have four squares in a straight row.'],f+1),correctId:'o0'},
...
function plots(f:number):PlotTask[]{
 ...
 return [
  ...
  {mode:'choice',plainDiagram:true,prompt:'Which description explains the move from A to B?',instruction:'Compare the position, size and facing of the two shapes.',shape,image,options:opts(['A slides up and right, keeping its size and facing.','A flips over a mirror line, reversing its facing.','A makes a half turn around its own centre, keeping that centre in place.','A makes a quarter turn, changing the direction its sides face.'],f),correctId:'o0'},
