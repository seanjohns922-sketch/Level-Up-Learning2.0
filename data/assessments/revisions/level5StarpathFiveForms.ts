...
export type NetTask={mode:'choice'|'complete'|'build';prompt:string;instruction:string;cells?:Cell[];solid?:SolidKind;secondSolid?:SolidKind;angle?:number;options?:Option[];correctId?:string;marked?:number;seeds?:Cell[];example?:Cell[]};
export type PlotTask={mode:'choice'|'point'|'pair'|'axes'|'route'|'shape';prompt:string;instruction:string;points?:{label:string;point:Point}[];shape?:Shape;image?:Shape;answer?:Point;expected?:Shape;options?:Option[];correctId?:string;start?:Point;goal?:Point;via?:Point;blocked?:Point[];example?:MoveDir[];maxMoves?:number;mirror?:MirrorLine;centre?:Point;dx?:number;dy?:number;operation?:'translate'|'reflect'|'rotate'|'combine';axisValues?:number[]};
...
function nets(f:number):NetTask[]{
 ...
 return [
  ...
  {mode:'complete',prompt:'Add one square to complete a working cube net.',instruction:'Keep the five given faces. Tap one empty square. The six faces must fold without overlapping.',seeds:example.filter((_,i)=>i!==missing),example},
...
function plots(f:number):PlotTask[]{
 ...
 return [
  ...
  {mode:'choice',prompt:'How does shape A translate to image B?',instruction:'Compare corresponding vertices. One grid interval is one unit.',shape,image,options:opts(['2 units right and 2 units up.','2 units left and 2 units up.','4 units right and 4 units up.'],f),correctId:'o0'},
