...
function composites(f:number):CompositeTask[]{
 ...
 const approximation=[
  ['A tall rectangle with a triangle above it and a triangular fin on each side near the bottom.','A tall rectangle with a triangle above it and both triangular fins on the same side near the bottom.','A tall rectangle with a triangle above it and just one triangular fin near the bottom.'],
  ['A wide rectangle below an upright mast, with a triangular sail beside the mast.','A wide rectangle below an upright mast, with the triangular sail inside the hull.','A wide rectangle below an upright mast, with a triangular sail on each side of the mast.'],
  ['Two overlapping triangles above a narrow upright rectangle.','Two triangles side by side above a narrow upright rectangle, with no overlap.','One triangle above a narrow upright rectangle.'],
  ['A smaller rectangle above a wider rectangle, with two circles underneath.','A smaller rectangle above a wider rectangle, with both circles underneath the same end.','A smaller rectangle above a wider rectangle, with three circles underneath.'],
  ['Four triangles arranged on both sides of a narrow upright rectangle.','Four triangles arranged on one side of a narrow upright rectangle.','Two triangles arranged one on each side of a narrow upright rectangle.'],
 ][f];
 const simplifications=[
  ['Remove the small window; keep the body, pointed nose and both fins.','Split each fin into two smaller triangles; keep the body, nose and window in place.','Move both fins above the nose; keep all the same pieces.'],
  ['Remove the small flag; keep the hull, mast and sail in their positions.','Split the sail into two smaller triangles; keep the hull, mast and flag in place.','Move the hull above the sail; keep all the same pieces.'],
  ['Use one large triangle for the leaves above the rectangular trunk.','Split each triangle into two smaller triangles; keep the trunk and leaves in place.','Move the trunk above the leaves and keep both triangles.'],
  ['Use one rectangle for the body and cabin; keep the two wheels underneath.','Split the body rectangle into two rectangles; keep the cabin and wheels in place.','Move both wheels above the cabin and keep both rectangles.'],
  ['Use one larger triangle on each side of the narrow body.','Split each wing triangle into two smaller triangles; keep the body and wings in place.','Remove the body and arrange the wing triangles in a straight row.'],
 ][f];
 return [
...
function grids(f:number):GridTask[]{
 ...
 return [
  ...
  {...common,mode:'choice',prompt:`Which route log takes you from ${ref(start)} to ${ref(checkpoint)}?`,instruction:'Each reference in a route log must be in the next neighbouring square. Follow the logs on the map.',start,goal:checkpoint,options:options([moves(['up','up','right']),moves(['right','right','up']),moves(['up','right','right'])].map(ds=>routeCells(start,ds).map(ref).join(' → ')),f),correctId:'o0'},
 ];
}
...
function symmetry(f:number):SymTask[]{
 ...
 return [
  {...common,mode:'choice',prompt:'Does this pattern have symmetry across the dashed line?',line,seeds:valid?full:full.slice(0,-1),expected:full,options:options(['Yes, every tile has a same-coloured partner the same distance across the line.','No, at least one tile has no matching partner across the line.','Yes, because any pattern with tiles on both sides has line symmetry.','No, because tiles must touch the mirror line to have reflection symmetry.'],f),correctId:valid?'o0':'o1'},
  ...
  {...common,mode:'choice',prompt:'Does this design match after a half turn about the centre?',turn,seeds:turnValid?turned:turned.slice(0,-1),expected:turned,options:options(['Yes, every tile lands on a same-coloured tile after turning 180°.','No, at least one tile lands on an empty square after turning 180°.','Yes, because a half turn brings every pattern back to its starting position.','No, because a half turn must swap the top row with the left column.'],f+1),correctId:turnValid?'o0':'o1'},
