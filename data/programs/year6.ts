import { buildProgram, type ProgramRow } from "./buildProgram";
import type { LessonActivity } from "./types";

function makeActivity(
  activityType: LessonActivity["activityType"],
  weight: number,
  config: Record<string, unknown>
): LessonActivity {
  return { activityType, weight, config };
}

const year6Rows: ProgramRow[] = [
  {
    week: 1,
    weekTopic: "Decimal Foundations",
    focus: "Read, break, build, and compare decimals to thousandths",
    lesson: 1,
    topic: "Decimal Place Value to Thousandths",
    activity: "Decimals are precise numbers. Read, build, and compare decimals to thousandths.",
    curriculum: ["AC9M6N04"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_place_value_fluency",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_flexibility",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_reasoning_check",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 1,
    focus: "Solve decimal problems using efficient strategies",
    lesson: 2,
    topic: "Add & Subtract Decimals",
    activity: "Not all decimals should be solved the same way. Add and subtract decimals accurately while choosing efficient strategies.",
    curriculum: ["AC9M6N04"],
    activities: [
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_direct_calculation",
        rotationRole: "fast_thinking",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_strategy_selection",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_precision_under_pressure",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 1,
    focus: "Quickly estimate decimal answers, judge correctness, and catch decimal mistakes",
    lesson: 3,
    topic: "Estimate & Check with Decimals",
    activity: "You do not need to redo every calculation. Estimate quickly, check answers, and catch decimal mistakes fast.",
    curriculum: ["AC9M6N04", "AC9M6N08"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_quick_estimate",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_is_it_right",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_spot_error",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 2,
    weekTopic: "Number Properties",
    focus: "Classify numbers as prime or composite and connect this to factor structure",
    lesson: 1,
    topic: "Prime vs Composite Numbers",
    activity: "Some numbers break down easily. Others do not. Identify prime and composite numbers using smart checks.",
    curriculum: ["AC9M6N02"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_prime_quick_classify",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_prime_structure_check",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_prime_eliminate_fast",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 2,
    focus: "Use factor lists and multiples to compare number relationships",
    lesson: 2,
    topic: "Factors & Multiples",
    activity: "Factors and multiples reveal patterns. Use them to solve problems faster, not list them out.",
    curriculum: ["AC9M6N02"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_factor_quick_recognition",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_factor_relationship_thinking",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_factor_fast_application",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 2,
    focus: "Build square-number fluency and recognise numeric patterns",
    lesson: 3,
    topic: "Square Numbers & Patterns",
    activity: "Square numbers follow patterns. Spot them instantly and predict what comes next.",
    curriculum: ["AC9M6N02"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_square_recognition",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_square_pattern_building",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_square_apply_fast",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 3,
    weekTopic: "Decimal Scaling & Application",
    focus: "Scale decimals efficiently by powers of 10",
    lesson: 1,
    topic: "Multiply Decimals by 10, 100, 1000",
    activity: "Multiplying by 10, 100, and 1000 scales numbers. Watch what happens to the digits.",
    curriculum: ["AC9M6N06"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_scale_quick",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_scale_thinking",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_scale_apply",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 3,
    focus: "Scale decimals back efficiently by powers of 10",
    lesson: 2,
    topic: "Divide Decimals by 10, 100, 1000",
    activity: "Dividing by 10, 100, and 1000 scales numbers down. Watch what happens to the digits.",
    curriculum: ["AC9M6N06"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_divide_quick",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_divide_thinking",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_divide_apply",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 3,
    focus: "Apply decimal scaling in mixed problems and estimate whether answers fit",
    lesson: 3,
    topic: "Mixed Decimal Scaling Problems",
    activity: "Sometimes numbers scale up. Sometimes they scale down. Decide what to do and check if it makes sense.",
    curriculum: ["AC9M6N06", "AC9M6N08"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_mixed_choose",
        rotationRole: "fast_thinking",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_decimal_mixed_calculate",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_decimal_mixed_reason",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 4,
    weekTopic: "Integers & Number Lines",
    focus: "Compare and order integers using position and direction",
    lesson: 1,
    topic: "Integers on Number Lines",
    activity: "Compare and order integers using position and direction.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_integer_position_order" }),
      makeActivity("number_order", 1, { mode: "y6_integer_order_compare" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_apply_think" }),
    ],
  },
  {
    week: 4,
    focus: "Add and subtract integers by reasoning about movement on the number line",
    lesson: 2,
    topic: "Operations with Integers",
    activity: "Start at the first number, move right to increase, and move left to decrease.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_integer_follow_movement" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_equations_movement" }),
      makeActivity("multiple_choice", 1, { mode: "y6_integer_contexts_apply" }),
    ],
  },
  {
    week: 4,
    focus: "Interpret, compare, and solve multi-step integer changes in real-world contexts",
    lesson: 3,
    topic: "Integer Contexts",
    activity: "Interpret changes first, then solve harder integer contexts with movement, reversals, and multi-step thinking.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_integer_context_interpret" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_context_change" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_context_multistep" }),
    ],
  },
  {
    week: 5,
    weekTopic: "Fractions – Understanding & Comparison",
    focus: "Recognise, build, simplify, and justify equivalent fractions",
    lesson: 1,
    topic: "Equivalent Fractions",
    activity: "Build, identify, and justify equivalent fractions without comparing their size.",
    curriculum: ["AC9M6N03"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_equivalent_build_strict" }),
      makeActivity("equivalent_fraction_match", 1, {}),
      makeActivity("equivalent_fraction_yes_no", 1, { mode: "y6_equivalent_visual_strict" }),
    ],
  },
  {
    week: 5,
    focus: "Order fractions using shared denominators, related denominators, and benchmarks",
    lesson: 2,
    topic: "Ordering Fractions",
    activity: "Compare and order fractions using visual models, equivalence, and benchmark values.",
    curriculum: ["AC9M6N03"],
    activities: [
      makeActivity("fraction_compare", 1, {}),
      makeActivity("multiple_choice", 1, { mode: "y6_fraction_order_strategy" }),
      makeActivity("number_order", 1, { mode: "y6_fraction_order_reasoning" }),
    ],
  },
  {
    week: 5,
    focus: "Move flexibly between improper fractions and mixed numbers",
    lesson: 3,
    topic: "Mixed Numbers & Improper Fractions",
    activity: "Convert between mixed numbers and improper fractions and compare their size.",
    curriculum: ["AC9M6N03"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_mixed_improper_convert" }),
      makeActivity("number_order", 1, { mode: "y6_mixed_improper_compare" }),
      makeActivity("multiple_choice", 1, { mode: "y6_mixed_improper_apply" }),
    ],
  },
  {
    week: 6,
    weekTopic: "Fraction Operations",
    focus: "Choose efficient common denominators using multiples and factor reasoning",
    lesson: 1,
    topic: "Common Denominators (Efficient Strategies)",
    activity: "A common denominator is any shared multiple, but the smartest choice is usually the lowest common multiple.",
    curriculum: ["AC9M6N05"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_common_denominator_find_smart" }),
      makeActivity("typed_response", 1, { mode: "y6_common_denominator_build_equivalents" }),
      makeActivity("multiple_choice", 1, { mode: "y6_common_denominator_choose_strategy" }),
    ],
  },
  {
    week: 6,
    focus: "Add and subtract fractions by choosing efficient denominators and simplifying results",
    lesson: 2,
    topic: "Add & Subtract Fractions",
    activity: "Choose efficient denominators, solve accurately, and decide which fraction strategies are worth using.",
    curriculum: ["AC9M6N05"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_fraction_add_subtract_core" }),
      makeActivity("typed_response", 1, { mode: "y6_fraction_add_subtract_mixed" }),
      makeActivity("multiple_choice", 1, { mode: "y6_fraction_add_subtract_reason" }),
    ],
  },
  {
    week: 6,
    focus: "Apply fraction addition and subtraction in multi-step real-world contexts",
    lesson: 3,
    topic: "Fraction Word Problems",
    activity: "Read the context, choose the operation, solve efficiently, and check whether the result makes sense.",
    curriculum: ["AC9M6N05"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_fraction_word_problem_single" }),
      makeActivity("typed_response", 1, { mode: "y6_fraction_word_problem_multi" }),
      makeActivity("multiple_choice", 1, { mode: "y6_fraction_word_problem_reason" }),
    ],
  },
  {
    week: 7,
    weekTopic: "Fractions, Decimals & Percent Connections",
    focus: "Find a fraction of a quantity and connect it to multiplicative thinking",
    lesson: 1,
    topic: "Find a Fraction of a Quantity",
    activity: "Scale quantities forward, backward, and across multi-step situations using fractions.",
    curriculum: ["AC9M6N07"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_fraction_quantity_structure" }),
      makeActivity("typed_response", 1, { mode: "y6_fraction_quantity_nonclean" }),
      makeActivity("typed_response", 1, { mode: "y6_fraction_quantity_multistep" }),
    ],
  },
  {
    week: 7,
    focus: "Find percentages of quantities using efficient benchmark methods",
    lesson: 2,
    topic: "Find a Percentage of a Quantity",
    activity: "Use benchmark fractions, 10% thinking, and efficient part-building to calculate percentages of quantities.",
    curriculum: ["AC9M6N07"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_percent_benchmark" }),
      makeActivity("typed_response", 1, { mode: "y6_percent_break_apart" }),
      makeActivity("typed_response", 1, { mode: "y6_percent_real_multistep" }),
    ],
  },
  {
    week: 7,
    focus: "Connect fractions, decimals, and percentages and choose efficient representations",
    lesson: 3,
    topic: "Choose Efficient Forms",
    activity: "Choose the most efficient form before solving, then convert only if it helps.",
    curriculum: ["AC9M6N07"],
    activities: [
      makeActivity("fraction_decimal_percent_match", 1, {}),
      makeActivity("typed_response", 1, { mode: "y6_fdp_use_form" }),
      makeActivity("multiple_choice", 1, { mode: "y6_fdp_justify_decide" }),
    ],
  },
  {
    week: 8,
    weekTopic: "Mathematical Modelling",
    focus: "Use percentage reasoning to compare deals, combine discounts, and justify smart decisions",
    lesson: 1,
    topic: "Sales & Discounts — Making Smart Decisions",
    activity: "Compare real discounts, handle stacked percentage deals, and decide which option is actually better value.",
    curriculum: ["AC9M6N07", "AC9M6N08", "AC9M6N09"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_discount_compare_deals" }),
      makeActivity("typed_response", 1, { mode: "y6_discount_stacked" }),
      makeActivity("multiple_choice", 1, { mode: "y6_discount_budget_decide" }),
    ],
  },
  {
    week: 8,
    focus: "Compare value using unit rates and best-buy decisions",
    lesson: 2,
    topic: "Best-Buy & Unit Rate Decisions",
    activity: "Compare packs, rates, and offers to decide which option gives better value.",
    curriculum: ["AC9M6N08", "AC9M6N09"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_unit_rate_build" }),
      makeActivity("multiple_choice", 1, { mode: "y6_unit_rate_compare" }),
      makeActivity("multiple_choice", 1, { mode: "y6_unit_rate_decide" }),
    ],
  },
  {
    week: 8,
    focus: "Interpret and communicate modelling solutions clearly",
    lesson: 3,
    topic: "Interpret & Communicate Solutions",
    activity: "Work through short modelling tasks and explain which result or option makes most sense.",
    curriculum: ["AC9M6N09"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_interpret_spot" }),
      makeActivity("multiple_choice", 1, { mode: "y6_interpret_decide" }),
      makeActivity("typed_response", 1, { mode: "y6_interpret_explain" }),
    ],
  },
  {
    week: 9,
    weekTopic: "Rational Number Connections",
    focus: "Compare fractions and decimals using equivalent representations",
    lesson: 1,
    topic: "Compare Fractions and Decimals",
    activity: "Use place value and equivalence to compare rational numbers accurately.",
    curriculum: ["AC9M6N03", "AC9M6N04"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_fraction_order_fluency", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_decimal_flexibility", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_fraction_order_reasoning", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 9,
    focus: "Find fractions, decimals and percentages of quantities",
    lesson: 2,
    topic: "Find Parts of Quantities",
    activity: "Choose an efficient fraction, decimal or percentage form and calculate the required part.",
    curriculum: ["AC9M6N07"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_percent_benchmark", rotationRole: "fast_thinking" }),
      makeActivity("typed_response", 1, { mode: "y6_percent_break_apart", rotationRole: "reasoning" }),
      makeActivity("typed_response", 1, { mode: "y6_percent_real_multistep", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 9,
    focus: "Approximate rational-number results and judge reasonableness",
    lesson: 3,
    topic: "Approximate and Check",
    activity: "Select useful benchmarks, estimate results, and identify unreasonable answers.",
    curriculum: ["AC9M6N08"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_decimal_quick_estimate", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_decimal_is_it_right", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_decimal_spot_error", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 10,
    weekTopic: "Integers & Coordinates",
    focus: "Order and compare positive and negative integers",
    lesson: 1,
    topic: "Order Integers",
    activity: "Locate, compare, and order integers using distance and direction from zero.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_integer_position_order", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_integer_compare", rotationRole: "reasoning" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_apply_think", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 10,
    focus: "Interpret integers in practical contexts",
    lesson: 2,
    topic: "Integers in Context",
    activity: "Interpret signed values and changes in temperature, elevation and financial contexts.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_integer_contexts_apply", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_integer_context_interpret", rotationRole: "reasoning" }),
      makeActivity("typed_response", 1, { mode: "y6_integer_context_multistep", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 10,
    focus: "Read, plot and interpret coordinates across all quadrants",
    lesson: 3,
    topic: "Coordinates & Movement",
    activity: "Read, plot, and interpret coordinate positions and movements across the Cartesian plane.",
    curriculum: ["AC9M6N01"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_coord4q_find_plot", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_coord4q_read_quadrants", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_coord4q_movement", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 11,
    weekTopic: "Financial Decisions",
    focus: "Calculate and compare percentage discounts",
    lesson: 1,
    topic: "Discount Decisions",
    activity: "Calculate sale prices and compare single and successive discounts accurately.",
    curriculum: ["AC9M6N07", "AC9M6N08", "AC9M6N09"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_discount_compare_deals", rotationRole: "fast_thinking" }),
      makeActivity("typed_response", 1, { mode: "y6_discount_stacked", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_discount_budget_decide", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 11,
    focus: "Compare unit rates and best-buy options",
    lesson: 2,
    topic: "Unit Rates & Best Buys",
    activity: "Build equivalent unit rates and use them to justify the best-value decision.",
    curriculum: ["AC9M6N08", "AC9M6N09"],
    activities: [
      makeActivity("typed_response", 1, { mode: "y6_unit_rate_build", rotationRole: "fast_thinking" }),
      makeActivity("multiple_choice", 1, { mode: "y6_unit_rate_compare", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_unit_rate_decide", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 11,
    focus: "Interpret and communicate a financial model",
    lesson: 3,
    topic: "Justify a Financial Decision",
    activity: "Interpret constraints, compare options, and communicate a defensible financial conclusion.",
    curriculum: ["AC9M6N09"],
    activities: [
      makeActivity("multiple_choice", 1, { mode: "y6_interpret_spot", rotationRole: "fast_thinking" }),
      makeActivity("typed_response", 1, { mode: "y6_interpret_explain", rotationRole: "reasoning" }),
      makeActivity("multiple_choice", 1, { mode: "y6_interpret_decide", rotationRole: "apply_create" }),
    ],
  },
  {
    week: 12,
    weekTopic: "Mixed Application & Review",
    focus: "Apply all number skills fluently",
    lesson: 1,
    topic: "Mixed Operations Challenge",
    activity: "Apply mixed Level 6 number strategies across decimals, fractions, percentages, integers and coordinates.",
    curriculum: ["AC9M6N01", "AC9M6N04", "AC9M6N05", "AC9M6N06", "AC9M6N07"],
    activities: [
      makeActivity("typed_response", 1, {
        mode: "y6_number_operations_fluency",
        rotationRole: "fast_thinking",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_number_operations_reasoning",
        rotationRole: "reasoning",
      }),
      makeActivity("typed_response", 1, {
        mode: "y6_number_operations_application",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 12,
    focus: "Apply maths to real-life contexts",
    lesson: 2,
    topic: "Real-World Modelling Project",
    activity: "Combine percentage, rate, and multi-step reasoning in short realistic modelling problems.",
    curriculum: ["AC9M6N07", "AC9M6N08", "AC9M6N09"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_modelling_choose_strategy",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_modelling_best_value",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_modelling_mini_project",
        rotationRole: "apply_create",
      }),
    ],
  },
  {
    week: 12,
    focus: "Consolidate all learning before assessment",
    lesson: 3,
    topic: "Final Review & Readiness",
    activity: "Tune up fluency, estimation, and mixed calculations before the post-test.",
    curriculum: ["AC9M6N01", "AC9M6N02", "AC9M6N03", "AC9M6N08"],
    activities: [
      makeActivity("multiple_choice", 1, {
        mode: "y6_number_review_fluency",
        rotationRole: "fast_thinking",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_number_review_reasoning",
        rotationRole: "reasoning",
      }),
      makeActivity("multiple_choice", 1, {
        mode: "y6_number_review_application",
        rotationRole: "apply_create",
      }),
    ],
  },
];

export const YEAR6_PROGRAM = buildProgram(6, "Number", year6Rows);
