...
// A deterministic first-quadrant point (away from the axes) that varies by round.
function pt(round: number, salt = 0): Point {
  const x = 1 + ((round * 2 + salt) % 5);
  let y = 1 + ((round * 3 + salt + 2) % 5);
  if (y === x) y = 1 + (y % 5);
  return { x, y };
}
...
const base = (mode: CoordTask["mode"], render: CoordTask["render"], round: number, target: number): Pick<CoordTask, "kind" | "mode" | "render" | "target" | "bounds"> =>
  ({ kind: "starpathCoordinate", mode, render, target, bounds: BOUNDS });

// W4 L1 — read across then up; reject the swapped pair.
export function orderTask(round: number, target: number): CoordTask {
  const p = pt(round, 1);
  const { options, correctId } = coordOptions(p, [{ x: p.y, y: p.x }, { x: p.x, y: (p.y % 5) + 1 }], round);
  return {
    ...base("order", "options", round, target),
...
export function originTask(round: number, target: number): CoordTask {
  return {
    ...base("origin", "tap", round, target),
...
export function plotTask(round: number, target: number): CoordTask {
  const p = pt(round, 3);
  return {
    ...base("plot", "tap", round, target),
...
export function readTask(round: number, target: number): CoordTask {
  const p = pt(round, 5);
  const { options, correctId } = coordOptions(p, [{ x: p.y, y: p.x }, { x: (p.x % 5) + 1, y: p.y }], round);
  return {
    ...base("read", "options", round, target),
...
export function errorTask(round: number, target: number): CoordTask {
  const p = pt(round, 7);
  const wrong = round % 2 === 0 ? { x: p.y, y: p.x } : { x: p.x, y: (p.y % 5) + 1 };
  const { options, correctId } = coordOptions(p, [wrong, { x: (p.x % 5) + 1, y: p.y }], round);
  return {
    ...base("error", "options", round, target),
...
export function moveTask(round: number, target: number): CoordTask {
  const start = pt(round, 9);
...
export function moveAxisTask(round: number, target: number): CoordTask {
  const start = pt(round, 11);
  const dir: MoveDir = round % 2 === 0 ? "up" : "right";
  const steps = 2 + (round % 3);
  ...
  return {
    ...base("moveAxis", "options", round, target),
...
export function followTask(round: number, target: number): CoordTask {
...
export function commandsTask(round: number, target: number): CoordTask {
  const start = { x: 0 + (round % 2), y: 0 + ((round + 1) % 2) };
  const goal = { x: 3 + (round % 3), y: 3 + ((round + 2) % 3) };
  return {
    ...base("commands", "commands", round, target),
...
export function routeTask(round: number, target: number): CoordTask {
  const start = { x: 0, y: round % 3 };
  const goal = { x: 5, y: 5 - (round % 2) };
  const blocked = [{ x: 2, y: goal.y }, { x: 3, y: start.y }];
  return {
    ...base("route", "commands", round, target),
