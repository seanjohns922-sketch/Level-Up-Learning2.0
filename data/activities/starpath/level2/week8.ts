...
import { debugMapTask, followMapTask, missionMapTask } from "./navWeeks";
...
export function createPathwayMasterTaskSet(): RealmLessonTaskSet {
  ...
  let c = 0;
  return {
    teaching: teaching("masterPathway", "Pathway Master", "Read, follow and plan pathways.", "Read the map, follow a path, then plan your own mission route to a place."),
    activities: [
      () => whatIsHereTask(a++, ++target),
      () => followMapTask(b++ + 1, ++target),
      () => missionMapTask(c++ + 2, ++target),
    ],
  };
}
...
export function createMasterMissionTaskSet(): RealmLessonTaskSet {
  ...
  return {
    ...
    activities: [
      ...
      () => missionMapTask(b++ + 1, ++target),
      () => debugMapTask(c++ + 2, ++target),
    ],
  };
}
...
export const PATHWAY_MASTER_CONTENT = content("Pathway Master", "Read the map, follow a path, then plan your own mission route to a place.", ["read the map", "follow a path", "plan a mission"], ["Read", "Follow", "Plan"], "How did you master pathways?", ["I read the map", "I followed a path", "I planned a mission"], ["Read a map", "Follow a path", "Plan a mission"], "Master Mission", createPathwayMasterTaskSet);
