...
export const LEVEL_SIX_ARTWORK = "/images/starpath-home-bg-y6.png";

type TaskGenerator = (round: number, target: number) => PracticeTask;

// Expands as later Level 6 blocks land (coordinates, transformations, integration).
type L6Variant = "l6Cross";
...
export function lessonContent(params: {
  ...
  return {
    ...
    artworkSrc: LEVEL_SIX_ARTWORK,
    teaching: { title: params.title, durationMinutes: 1, taskKind: "starpathShapeIntro" },
    activities: [
      { key: "a1", title: params.activities[0], description: params.activities[0], taskKinds: [params.kinds?.[0] ?? "starpathCrossSection"] },
      { key: "a2", title: params.activities[1], description: params.activities[1], taskKinds: [params.kinds?.[1] ?? "starpathCrossSection"] },
      { key: "a3", title: params.activities[2], description: params.activities[2], taskKinds: [params.kinds?.[2] ?? "starpathCrossSection"] },
    ],
