...
function buildConstructionTask(): PrecisionTask {
  ...
  return {
    ...
    scene: "problem",
    attribute: "length",
    prompt: `You need a board at least ${fmtMixed(need, "length")} long. Which board is long enough?`,
    speakText: `You need a shelf at least ${fmtMixed(need, "length")} long. Which board is long enough?`,
    badgeLabel: "Construction Challenge",
    object: { label: "timber board", emoji: "🪵", context: "at the timber yard" },
    note: `You need at least ${fmtMixed(need, "length")}.`,
    options,
...
function buildMissionTask(asJustify = false): PrecisionTask {
  ...
  if (asJustify) {
    return {
      ...
      scene: "problem",
      attribute: "capacity",
      prompt: `You chose the ${fmtMixed(need + 300, "capacity")} bottle for ${fmtMixed(need, "capacity")} of juice. Why is that a good choice?`,
      speakText: `Why is the bigger bottle a good choice for the juice?`,
      badgeLabel: "Precision Mission",
      object: { label: "juice bottle", emoji: "🧃", context: "packing for the trip" },
  ...
  return {
    ...
    scene: "problem",
    attribute: "capacity",
    prompt: `You need to pour ${fmtMixed(need, "capacity")} of juice. Which container holds enough?`,
    speakText: `You need to pour ${fmtMixed(need, "capacity")} of juice. Which container holds enough?`,
    badgeLabel: "Precision Mission",
    object: { label: "juice", emoji: "🧃", context: "packing for the trip" },
    note: `You need at least ${fmtMixed(need, "capacity")}.`,
    options,
