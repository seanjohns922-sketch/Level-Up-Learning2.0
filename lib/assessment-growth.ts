...
function comparisonGroup(realmId: string, year: string, questions: readonly { id: string }[]): string | null {
  ...
  if(realmId==="pattern" && questions.length>0 && questions.every(q=>/^y[3-8]-pattern-(pretest|posttest)-\d{2}-v1$/.test(q.id)))return `pattern-${year}-2026-09-23-v1`;
  if(realmId==="chance" && questions.length>0 && questions.every(q=>/^y[3-8]-chance-(pretest|posttest)-\d{2}-v1$/.test(q.id)))return `chance-${year}-2026-09-24-v1`;
  ...
  if (realmId==="measurement" && year==="Year 6" && questions.length===20 && questions.every(q=>/^y6-measurement-(pre|post)-\d{2}-v4$/.test(q.id))) return "year6-measurement-2026-09-16-v4";
  if (realmId==="measurement" && year==="Year 3" && questions.length===20 && questions.every(q=>/^y3-measurement-(pre|post)-\d{2}-v4$/.test(q.id))) return "year3-measurement-2026-09-17-v4";
  if (realmId==="measurement" && year==="Year 4" && questions.length===20 && questions.every(q=>/^y4-measurement-(pre|post)-\d{2}-v4$/.test(q.id))) return "year4-measurement-2026-09-17-v4";
  if (realmId==="measurement" && year==="Year 7" && questions.length===30 && questions.every(q=>/^y7-measurement-(pre|post)-\d{2}-v4$/.test(q.id))) return "year7-measurement-2026-09-17-v4";
  if (realmId==="measurement" && year==="Year 8" && questions.length===30 && questions.every(q=>/^y8-measurement-(pre|post)-\d{2}-v4$/.test(q.id))) return "year8-measurement-2026-09-17-v4";
