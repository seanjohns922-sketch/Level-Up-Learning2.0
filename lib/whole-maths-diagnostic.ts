...
export function diagnosticQuestionCount(strand: string, level: string): number {
  return ["number","measurement","space","statistics","algebra"].includes(strand) && ["Year 7", "Year 8"].includes(level) ? 30 : DIAGNOSTIC_QUESTIONS_PER_LEVEL;
}
