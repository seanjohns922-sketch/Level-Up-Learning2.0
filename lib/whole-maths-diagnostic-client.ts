...
export type PendingStudentDiagnostic = {
  ...
  measurement_ground_bank_version?: 3 | 4;
  measurement_level1_bank_version?:3|4;
  measurement_level2_bank_version?:3|4;
  ...
  space_release_version?:0|1;
  statistics_release_version?:0|1;
  pattern_release_version?:0|1;
  chance_release_version?:0|1;
...
export async function fetchPendingStudentDiagnostic(studentId: string, includeClosed = false) {
  ...
  let { data, error } = await supabase.rpc("get_pending_whole_math_diagnostic_chance_full", {
    p_student_id: studentId,
  });
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_pattern_full",{p_student_id:studentId})); }
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_statistica_full",{p_student_id:studentId})); }
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_starpath_full",{p_student_id:studentId})); }
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_starpath_ground",{p_student_id:studentId})); }
  ...
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_measurement1",{p_student_id:studentId})); }
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_measurement",{p_student_id:studentId})); }
  if(error && (error.code === "PGRST202" || error.code === "42883")) { ({data,error}=await supabase.rpc("get_pending_whole_math_diagnostic_number7",{p_student_id:studentId})); }
  ...
  if (error && (error.code === "PGRST202" || error.code === "42883")) {
    ({ data, error } = await supabase.rpc("get_pending_whole_math_diagnostic_level4", { p_student_id: studentId }));
  }
  if (error && (error.code === "PGRST202" || error.code === "42883")) {
    ({ data, error } = await supabase.rpc("get_pending_whole_math_diagnostic_level2", { p_student_id: studentId }));
  }
  if (error && (error.code === "PGRST202" || error.code === "42883")) {
    ({ data, error } = await supabase.rpc("get_pending_whole_math_diagnostic_released", { p_student_id: studentId }));
  }
  if (error && (error.code === "PGRST202" || error.code === "42883")) {
    ({ data, error } = await supabase.rpc("get_pending_whole_math_diagnostic_versioned", { p_student_id: studentId }));
  }
  if (error && (error.code === "PGRST202" || error.code === "42883")) {
