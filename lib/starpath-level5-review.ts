...
export type Level5Response={folded?:boolean;selected:string;cells:Cell[];points:Point[];pair:{x:string;y:string};labels:Record<string,string>;moves:MoveDir[]};
...
export function level5ResponseReady(q:Level5Item,a:Level5Response){const t=q.task;if(t.mode==='choice')return !!a.selected;if(q.kind==='net')return (!q.task.foldRequired||!!a.folded)&&a.cells.length>(q.task.seeds?.length??0);switch(q.task.mode){case 'pair':return !!a.pair.x.trim()&&!!a.pair.y.trim();case 'point':case 'shape':return a.points.length>0;case 'axes':return q.task.axisValues!.every(v=>['x','y'].every(axis=>!!a.labels[axis+v]?.trim()));case 'route':return a.moves.length>0;default:return false;}}
export function scoreLevel5Response(q:Level5Item,a:Level5Response){
 ...
 if(q.kind==='net')return (!q.task.foldRequired||!!a.folded)&&a.cells.length===6&&new Set(a.cells.map(cellKey)).size===6&&a.cells.every(p=>Number.isInteger(p.r)&&Number.isInteger(p.c)&&p.r>=0&&p.r<5&&p.c>=0&&p.c<5)&&(q.task.seeds??[]).every(p=>a.cells.some(c=>cellKey(c)===cellKey(p)))&&foldNet(a.cells).valid&&(t.mode!=='build'||netSignature(a.cells)!==netSignature(q.task.cells!));
 const p=q.task;
