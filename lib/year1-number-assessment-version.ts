...
export function savedYear1NumberVersion(ids: readonly string[] | undefined): 2 | 3 | 5 | null {
  ...
  if (ids.every(id => /^y1-number-(pre|post)-\d{2}-v5$/.test(id))) return 5;
  return null;
}

/** Unknown historical baselines stay on the original pair; never relabel their evidence. */
export function year1NumberPostVersion(attempts: readonly NormalizedAssessmentAttempt[], draftIds?: readonly string[]): 2 | 3 | 5 {
  ...
  if (evidence?.comparison_group === "paired-number-Year 1-2026-09-15-v5") return 5;
