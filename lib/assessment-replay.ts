import {readReleasedChance,chanceReleaseVisual} from '@/lib/chance-release-response';
import {readReleasedPattern,patternReleaseVisual} from '@/lib/pattern-release-response';
...
export function buildAssessmentQuestionSnapshots(
  ...
  return questions.map((question, index) => {
    ...
    return {
      schema_version: 1,
      scorer_version: chanceReleaseVisual(question) ? "chance-release-2026-09-24-v1" : patternReleaseVisual(question) ? "pattern-release-2026-09-23-v1" : statisticaReleaseVisual(question) ? "statistica-release-2026-09-23-v3" : starpathReleaseVisual(question) ? "starpath-release-2026-09-22-v1" : "assessment-2026-09-14-v1",
      ...
      student_answer: cloneJsonValue(readReleasedChance(question.id,studentAnswer) ?? readReleasedPattern(question.id,studentAnswer) ?? readReleasedStatistica(question.id,studentAnswer) ?? readReleasedStarpath(question.id,studentAnswer) ?? decodeAssessmentResponse(studentAnswer)?.response ?? decodeStarpathResponse(studentAnswer)?.response ?? studentAnswer),
      correct,
