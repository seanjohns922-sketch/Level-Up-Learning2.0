...
import { hasComparableAssessmentGrowth } from "@/lib/assessment-growth";
...
export async function teacherChangeStartingLevel(
  ...
  const normalizedEntryMode = assignedLevel === "Prep" && !hasComparableAssessmentGrowth(realmId, assignedLevel)
    ? "ground_week1"
...
export async function teacherChangeStartingLevels(
  ...
  const { data, error } = await supabase.rpc("teacher_change_starting_levels", {
    ...
    p_placements: placements.map((placement) => ({
      ...
      entry_mode: placement.assignedLevel === "Prep" && !hasComparableAssessmentGrowth(realmId, placement.assignedLevel)
        ? "ground_week1"
...
export async function fetchNumberExtensionAssessments(studentId: string, level: 7 | 8 = 7): Promise<NormalizedAssessmentAttempt[]> {
  const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:studentId,p_realm_id:'number',p_working_level:`Year ${level}`});
...
/** Extension assessments have evidence without an active weekly-program summary. */
export async function fetchMeasurementExtensionAssessments(studentId: string, level: 7 | 8 = 7): Promise<NormalizedAssessmentAttempt[]> {
  const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:studentId,p_realm_id:'measurement',p_working_level:`Year ${level}`});
  if(error)throw new Error(error.message);
  return ((data??[]) as RealmAssessmentRow[]).sort((a,b)=>Date.parse(a.completed_at)-Date.parse(b.completed_at)).map((row,i)=>normalizeAssessmentAttempt(row,i+1));
}
...
export async function fetchStatisticsExtensionAssessments(studentId: string, level: 7 | 8 = 7): Promise<NormalizedAssessmentAttempt[]> {
  const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:studentId,p_realm_id:'statistics',p_working_level:`Year ${level}`});
  if(error)throw new Error(error.message);
  return ((data??[]) as RealmAssessmentRow[]).sort((a,b)=>Date.parse(a.completed_at)-Date.parse(b.completed_at)).map((row,i)=>normalizeAssessmentAttempt(row,i+1));
}
