...
export function getDiagnosticQuestions(
  ...
  const pretest = getPretestForYearLabel(level, definition.realmId, 2, 1, 2, 2);
  const posttest = getPosttestForYearLabel(level, definition.realmId, 2, 1, 2, 2)?.questions ?? [];
