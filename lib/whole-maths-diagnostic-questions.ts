...
export function getDiagnosticQuestions(
  ...
  statisticsReleaseVersion:0|1=0,
  patternReleaseVersion:0|1=0,
  chanceReleaseVersion:0|1=0,
  ...
  const pretest = getPretestForYearLabel(level, definition.realmId, 2, 1, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 0, 0, 0, 0, 0);
  const posttest = getPosttestForYearLabel(level, definition.realmId, 2, 1, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 0, 0, 0, 0, 0)?.questions ?? [];
  const levelTest = strand==='probability' && chanceReleaseVersion===1 ? releasedChanceQuestions(level,checkpoint==='ad_hoc'?'start':checkpoint) : strand==='algebra' && patternReleaseVersion===1 ? releasedPatternQuestions(level,checkpoint==='ad_hoc'?'start':checkpoint) : strand==='statistics' && statisticsReleaseVersion===1 ? releasedStatisticaQuestions(level,checkpoint==='ad_hoc'?'start':checkpoint) : strand==='space' && spaceReleaseVersion===1 ? releasedStarpathQuestions(level,checkpoint==='ad_hoc'?'start':checkpoint) : strand === "space" && level === "Prep" && spaceGroundVersion === 3
