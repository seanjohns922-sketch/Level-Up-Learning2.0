-- Deploy the web support first. Preserve existing diagnostic cycles and results.
begin;

alter table public.whole_math_diagnostic_sittings
  add column space_ground_bank_version integer not null default 0
  check (space_ground_bank_version in (0,3));
alter table public.whole_math_diagnostic_sittings alter column space_ground_bank_version set default 3;

create function public.pin_ground_starpath_diagnostic_version()
returns trigger language plpgsql security definer set search_path=public as $$
declare previous integer;
begin
  select s.space_ground_bank_version into previous
  ...
  new.space_ground_bank_version:=coalesce(previous,3);
  return new;
end;
$$;
revoke all on function public.pin_ground_starpath_diagnostic_version() from public,anon,authenticated;
create trigger trg_pin_ground_starpath_diagnostic_version
before insert on public.whole_math_diagnostic_sittings
for each row execute function public.pin_ground_starpath_diagnostic_version();

create function public.prepare_ground_starpath_diagnostic()
returns trigger language plpgsql security definer set search_path=public as $$
declare original_level text;
begin
  if new.strand='space' and exists (
    select 1 from public.whole_math_diagnostic_sittings s
    where s.id=new.sitting_id and s.space_ground_bank_version=3
  ) then
    select coalesce(
      (select p.working_level from public.student_realm_progress p
       where p.student_id=new.student_id and p.realm_id='space' and p.is_current
       order by p.updated_at desc nulls last limit 1),
      (select p.assigned_start_level from public.student_realm_placement p
       where p.student_id=new.student_id and p.realm_id='space'),
      s.school_year_level,s.year_level
    ) into original_level from public.students s where s.id=new.student_id;
    if original_level in ('Prep','Foundation','Year 0','Ground') then
      new.starting_level:='Prep';
    end if;
  end if;
  return new;
end;
$$;
revoke all on function public.prepare_ground_starpath_diagnostic() from public,anon,authenticated;
create trigger trg_prepare_ground_starpath_diagnostic
before insert on public.whole_math_diagnostic_strand_results
for each row execute function public.prepare_ground_starpath_diagnostic();

create function public.get_pending_whole_math_diagnostic_starpath_ground(p_student_id uuid)
returns table(sitting_id uuid,checkpoint text,strand text,starting_level text,status text,
  active_level text,draft_answers jsonb,draft_probes jsonb,draft_index integer,access_open boolean,
  number_level1_bank_version integer,number_ground_bank_version integer,number_level2_bank_version integer,
  number_level4_bank_version integer,number_level5_bank_version integer,number_level6_bank_version integer,
  number_level3_bank_version integer,number_maximum_level integer,measurement_ground_bank_version integer,
  measurement_level1_bank_version integer,measurement_level2_bank_version integer,
  measurement_level5_bank_version integer,measurement_level6_bank_version integer,measurement_release_version integer,
  space_ground_bank_version integer)
language plpgsql security definer set search_path=public as $$
begin
  perform public.assert_student_access(p_student_id);
  return query select p.*,s.space_ground_bank_version
  from public.get_pending_whole_math_diagnostic_measurement_full(p_student_id) p
  join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end;
$$;
revoke all on function public.get_pending_whole_math_diagnostic_starpath_ground(uuid) from public,anon,authenticated;
grant execute on function public.get_pending_whole_math_diagnostic_starpath_ground(uuid) to anon,authenticated;
...
begin
  ...
  if jsonb_typeof(p_probe_scores)<>'array' or jsonb_array_length(p_probe_scores)<1 or jsonb_array_length(p_probe_scores)>(case when p_strand='space' and v_sitting.space_ground_bank_version=3 then 7 when p_strand='number' and v_sitting.number_ground_bank_version=3 then v_sitting.number_maximum_level+1 when p_strand='measurement' and v_sitting.measurement_release_version=1 then 9 when p_strand='measurement' and v_sitting.measurement_ground_bank_version=4 then 7 else 6 end) then
