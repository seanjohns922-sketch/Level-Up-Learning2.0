-- Approved Level 1 Measurement. Existing academic-year cycles retain v3.
begin;
alter table public.whole_math_diagnostic_sittings add column measurement_level1_bank_version integer not null default 3 check(measurement_level1_bank_version in (3,4));
alter table public.whole_math_diagnostic_sittings alter column measurement_level1_bank_version set default 4;
create function public.pin_year1_measurement_diagnostic_version() returns trigger language plpgsql security definer set search_path=public as $$
declare previous integer;
begin
 select s.measurement_level1_bank_version into previous from public.whole_math_diagnostic_sittings s
 where s.student_id=new.student_id and s.academic_year=new.academic_year order by s.created_at,s.id limit 1;
 new.measurement_level1_bank_version:=coalesce(previous,4);return new;
end;$$;
revoke all on function public.pin_year1_measurement_diagnostic_version() from public,anon,authenticated;
create trigger trg_pin_year1_measurement_diagnostic_version before insert on public.whole_math_diagnostic_sittings for each row execute function public.pin_year1_measurement_diagnostic_version();
CREATE OR REPLACE FUNCTION public.get_pending_whole_math_diagnostic_measurement1(p_student_id uuid)
 RETURNS TABLE(sitting_id uuid, checkpoint text, strand text, starting_level text, status text, active_level text, draft_answers jsonb, draft_probes jsonb, draft_index integer, access_open boolean, number_level1_bank_version integer, number_ground_bank_version integer, number_level2_bank_version integer, number_level4_bank_version integer, number_level5_bank_version integer, number_level6_bank_version integer, number_level3_bank_version integer, number_maximum_level integer, measurement_ground_bank_version integer, measurement_level1_bank_version integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  return query select p.*, s.measurement_level1_bank_version
  from public.get_pending_whole_math_diagnostic_measurement(p_student_id) p
  join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end;
$function$
;
revoke all on function public.get_pending_whole_math_diagnostic_measurement1(uuid) from public,anon;
grant execute on function public.get_pending_whole_math_diagnostic_measurement1(uuid) to authenticated;
