-- Deploy web support before enabling new Statistica sittings. Existing cycles stay pinned.
begin;
alter table public.whole_math_diagnostic_strand_results drop constraint whole_math_diagnostic_measured_range;
alter table public.whole_math_diagnostic_strand_results add constraint whole_math_diagnostic_measured_range check(measured_level is null or (measured_level>=0 and measured_level<=case when strand in ('number','measurement','space','statistics') then 8 else 6 end));
alter table public.whole_math_diagnostic_strand_results drop constraint whole_math_diagnostic_draft_index_range;
alter table public.whole_math_diagnostic_strand_results add constraint whole_math_diagnostic_draft_index_range check(draft_question_index>=0 and draft_question_index<=case when strand in ('number','measurement','space','statistics') and active_level in ('Year 7','Year 8') then 29 else 19 end);
alter table public.whole_math_diagnostic_sittings add column statistics_release_version integer not null default 0 check(statistics_release_version in (0,1));
alter table public.whole_math_diagnostic_sittings alter column statistics_release_version set default 1;
create function public.pin_statistica_release_version() returns trigger language plpgsql security definer set search_path=public as $$
declare previous integer;
begin
 select s.statistics_release_version into previous from public.whole_math_diagnostic_sittings s
 where s.student_id=new.student_id and s.academic_year=new.academic_year order by s.created_at,s.id limit 1;
 new.statistics_release_version:=coalesce(previous,1);
 return new;
end;
$$;
revoke all on function public.pin_statistica_release_version() from public,anon,authenticated;
create trigger trg_pin_statistica_release_version before insert on public.whole_math_diagnostic_sittings for each row execute function public.pin_statistica_release_version();


CREATE OR REPLACE FUNCTION public.get_pending_whole_math_diagnostic_statistica_full(p_student_id uuid)
 RETURNS TABLE(sitting_id uuid, checkpoint text, strand text, starting_level text, status text, active_level text, draft_answers jsonb, draft_probes jsonb, draft_index integer, access_open boolean, number_level1_bank_version integer, number_ground_bank_version integer, number_level2_bank_version integer, number_level4_bank_version integer, number_level5_bank_version integer, number_level6_bank_version integer, number_level3_bank_version integer, number_maximum_level integer, measurement_ground_bank_version integer, measurement_level1_bank_version integer, measurement_level2_bank_version integer, measurement_level5_bank_version integer, measurement_level6_bank_version integer, measurement_release_version integer, space_ground_bank_version integer, space_release_version integer, statistics_release_version integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  return query select p.*,s.statistics_release_version
  from public.get_pending_whole_math_diagnostic_starpath_full(p_student_id) p
  join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end;
$function$
;

revoke all on function public.get_pending_whole_math_diagnostic_statistica_full(uuid) from public,anon,authenticated;
grant execute on function public.get_pending_whole_math_diagnostic_statistica_full(uuid) to anon,authenticated;
...
begin
  ...
  for v_probe in select value from jsonb_array_elements(p_probe_scores) probe(value) loop
    ...
    if v_level is null or v_level<v_minimum or v_level>(case when p_strand='number' then v_sitting.number_maximum_level when p_strand='statistics' and v_sitting.statistics_release_version=1 then 8 when p_strand='space' and v_sitting.space_release_version=1 then 8 when p_strand='measurement' and v_sitting.measurement_release_version=1 then 8 else 6 end) or v_total<>(case when p_strand in ('number','measurement','space','statistics') and v_level in (7,8) then 30 else 20 end) or v_score<0 or v_score>v_total
...
begin
  ...
  if p_strand in ('number','measurement','space','statistics') and v_recommended>6 then v_recommended:=6; v_flag:='extension_ready_to_bridge'; end if;
...
begin
  ...
  if p_strand not in ('number','measurement','space','statistics','algebra','probability')
    or (p_active_level not in ('Prep','Year 1','Year 2','Year 3','Year 4','Year 5','Year 6') and not (p_strand in ('number','measurement','space','statistics') and p_active_level in ('Year 7','Year 8') and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=p_sitting_id and s.student_id=p_student_id and public.number_diagnostic_level_value(p_active_level)<=case when p_strand='number' then s.number_maximum_level when p_strand='statistics' and s.statistics_release_version=1 then 8 when p_strand='space' and s.space_release_version=1 then 8 when p_strand='measurement' and s.measurement_release_version=1 then 8 else 6 end)))
...
begin
  ...
  loop
    v_total:=case when v_result.realm_id in ('number','measurement','space','statistics') and v_result.starting_level in ('Year 7','Year 8') then 30 else 20 end;
    ...
    if v_percent>=85 and v_level<(case when v_result.realm_id='number' then v_sitting.number_maximum_level when v_result.realm_id='statistics' and v_sitting.statistics_release_version=1 then 8 when v_result.realm_id='space' and v_sitting.space_release_version=1 then 8 when v_result.realm_id='measurement' and v_sitting.measurement_release_version=1 then 8 else 6 end) then
      ...
      v_measured:=greatest(0,least(case when v_result.realm_id='number' then v_sitting.number_maximum_level when v_result.realm_id='statistics' and v_sitting.statistics_release_version=1 then 8 when v_result.realm_id='space' and v_sitting.space_release_version=1 then 8 when v_result.realm_id='measurement' and v_sitting.measurement_release_version=1 then 8 else 6 end,case when v_percent>=85 then v_level
...
CREATE OR REPLACE FUNCTION public.prepare_statistica_release_level()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
 if new.strand='statistics' and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=new.sitting_id and s.statistics_release_version=1) then
  select coalesce((select p.working_level from public.student_realm_progress p where p.student_id=new.student_id and p.realm_id='statistics' and p.is_current order by p.updated_at desc nulls last limit 1),
   (select p.assigned_start_level from public.student_realm_placement p where p.student_id=new.student_id and p.realm_id='statistics'),s.school_year_level,s.year_level) into original_level from public.students s where s.id=new.student_id;
...
revoke all on function public.prepare_statistica_release_level() from public,anon,authenticated;
create trigger trg_prepare_statistica_release_level before insert on public.whole_math_diagnostic_strand_results for each row execute function public.prepare_statistica_release_level();

CREATE OR REPLACE FUNCTION public.complete_statistics_extension_assessment(p_student_id uuid, p_assessment_type text, p_completion_key uuid, p_attempt jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
 ...
 if not exists(select 1 from public.student_realm_progress where student_id=p_student_id and realm_id='statistics' and is_current and working_level in ('Year 6','Year 7','Year 8')) then raise exception 'Extension assessments require a Level 6 or higher Statistics placement'; end if;
 ...
 if jsonb_array_length(p_attempt->'question_results')<>30 or exists(
  select 1 from generate_series(1,30) n where not exists(select 1 from jsonb_array_elements(p_attempt->'question_results') q where q->>'question_id'='y'||right(target_level,1)||'-statistica-'||p_assessment_type||'-'||lpad(n::text,2,'0')||'-v3')
 ...
 perform pg_advisory_xact_lock(hashtextextended(p_student_id::text||':statistics:'||target_level,0));
 if p_assessment_type='posttest' and not exists(select 1 from public.student_realm_assessments where student_id=p_student_id and realm_id='statistics' and working_level=target_level and assessment_type='pretest') then raise exception 'Complete this level’s pre-test first'; end if;
 insert into public.student_completion_receipts(student_id,realm_id,activity_type,completion_key) values(p_student_id,'statistics',p_assessment_type,p_completion_key) on conflict do nothing;
 ...
 values(p_student_id,actual_student.class_id,'statistics',public.realm_program_key(target_level,'statistics'),actual_student.school_year_level,target_level,p_assessment_type,total_correct,30,round(total_correct*100.0/30),total_correct>=26,
 coalesce(p_attempt->'placement_result','{}'::jsonb)||jsonb_build_object('assessment_evidence',jsonb_build_object('realm','statistics','working_level',target_level,'comparison_group','statistica-'||target_level||'-2026-09-23-v3','comparability_status','blueprint_matched_uncalibrated','calibration_status','uncalibrated','bank_versions',jsonb_build_array('3'),'baseline_only',false)),p_attempt->'question_results',now());
 return true;
end; $function$
;

revoke all on function public.complete_statistics_extension_assessment(uuid,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.complete_statistics_extension_assessment(uuid,text,uuid,jsonb) to anon,authenticated;
