-- Number Level 6 five-form release. Deploy the web release first, then apply.
-- Existing Level 6 diagnostic cycles keep v2; new cycles use v3 and later checkpoints inherit the cycle's version.
begin;
alter table public.whole_math_diagnostic_sittings
  add column if not exists number_level6_bank_version integer not null default 2
  check (number_level6_bank_version in (2,3));
alter table public.whole_math_diagnostic_sittings alter column number_level6_bank_version set default 3;

create or replace function public.pin_number_level6_diagnostic_version()
returns trigger language plpgsql security definer set search_path=public as $$
declare previous integer;
begin
  select s.number_level6_bank_version into previous
  ...
  new.number_level6_bank_version:=coalesce(previous,3);
  return new;
end;
$$;
revoke all on function public.pin_number_level6_diagnostic_version() from public,anon,authenticated;
drop trigger if exists trg_pin_number_level6_diagnostic_version on public.whole_math_diagnostic_sittings;
create trigger trg_pin_number_level6_diagnostic_version before insert on public.whole_math_diagnostic_sittings
for each row execute function public.pin_number_level6_diagnostic_version();

create or replace function public.get_pending_whole_math_diagnostic_level6(p_student_id uuid)
returns table (
  ...
  number_level2_bank_version integer, number_level4_bank_version integer, number_level5_bank_version integer, number_level6_bank_version integer
)
language plpgsql security definer set search_path=public as $$
begin
  -- The released RPC keeps assert_student_access, deferred follow-ups and the earlier pinned versions.
  return query select p.*, s.number_level6_bank_version
  from public.get_pending_whole_math_diagnostic_level5(p_student_id) p
  join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end;
$$;
revoke all on function public.get_pending_whole_math_diagnostic_level6(uuid) from public,anon,authenticated;
grant execute on function public.get_pending_whole_math_diagnostic_level6(uuid) to anon,authenticated;

-- Recent pre/post evidence is adopted only when its released question version matches the cycle.
