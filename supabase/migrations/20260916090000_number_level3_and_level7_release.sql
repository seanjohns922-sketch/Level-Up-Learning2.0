-- Approved Number Level 3 and Level 7 forms. Existing cycles retain their banks and ceiling.
begin;
alter table public.whole_math_diagnostic_sittings
 add column number_level3_bank_version integer not null default 2 check(number_level3_bank_version in (2,3)),
 add column number_maximum_level integer not null default 6 check(number_maximum_level in (6,7));
alter table public.whole_math_diagnostic_sittings alter column number_level3_bank_version set default 3;
alter table public.whole_math_diagnostic_sittings alter column number_maximum_level set default 7;
...
begin
 ...
 new.number_maximum_level:=coalesce(previous.number_maximum_level,7);
 return new;
end; $$;
revoke all on function public.pin_number_extension_cycle() from public,anon,authenticated;
create trigger trg_pin_number_extension_cycle before insert on public.whole_math_diagnostic_sittings
 for each row execute function public.pin_number_extension_cycle();
-- The primary whole-maths aggregate remains capped at 6; only the Number strand extends.
alter table public.whole_math_diagnostic_strand_results drop constraint whole_math_diagnostic_measured_range;
alter table public.whole_math_diagnostic_strand_results add constraint whole_math_diagnostic_measured_range
 check(measured_level is null or (measured_level>=0 and measured_level<=case when strand='number' then 7 else 6 end));
CREATE OR REPLACE FUNCTION public.number_diagnostic_level_value(value text)
 RETURNS integer
 LANGUAGE sql
 IMMUTABLE
AS $function$
 select case when value='Prep' then 0 when value ~ '^Year [1-7]$' then substring(value from '[0-9]+')::integer else null end;
$function$
;
CREATE OR REPLACE FUNCTION public.get_pending_whole_math_diagnostic_number7(p_student_id uuid)
 RETURNS TABLE(sitting_id uuid, checkpoint text, strand text, starting_level text, status text, active_level text, draft_answers jsonb, draft_probes jsonb, draft_index integer, access_open boolean, number_level1_bank_version integer, number_ground_bank_version integer, number_level2_bank_version integer, number_level4_bank_version integer, number_level5_bank_version integer, number_level6_bank_version integer, number_level3_bank_version integer, number_maximum_level integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  return query select p.*, s.number_level3_bank_version, s.number_maximum_level
  from public.get_pending_whole_math_diagnostic_level6(p_student_id) p
  join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end;
$function$
;
revoke all on function public.get_pending_whole_math_diagnostic_number7(uuid) from public,anon,authenticated;
grant execute on function public.get_pending_whole_math_diagnostic_number7(uuid) to anon,authenticated;
...
begin
  ...
  for v_probe in select value from jsonb_array_elements(p_probe_scores) probe(value) loop
    ...
    if v_level is null or v_level<v_minimum or v_level>(case when p_strand='number' then v_sitting.number_maximum_level else 6 end) or v_total<>(case when p_strand='number' and v_level=7 then 30 else 20 end) or v_score<0 or v_score>v_total
...
begin
  ...
  if p_strand not in ('number','measurement','space','statistics','algebra','probability')
    or (p_active_level not in ('Prep','Year 1','Year 2','Year 3','Year 4','Year 5','Year 6') and not (p_strand='number' and p_active_level='Year 7' and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=p_sitting_id and s.student_id=p_student_id and s.number_maximum_level=7)))
    ...
    or jsonb_array_length(p_probe_scores)>(case when p_strand='number' and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=p_sitting_id and s.number_ground_bank_version=3) then 8 else 6 end) or p_question_index not between 0 and (case when p_strand='number' and p_active_level='Year 7' then 29 else 19 end) then
...
-- Preserve Level 7 Number evidence in the per-strand tracker cache.
alter table public.student_live_maths_progression drop constraint student_live_maths_progression_official_level_check;
alter table public.student_live_maths_progression add constraint student_live_maths_progression_official_level_check check(official_level>=0 and official_level<=case when realm_id='number' then 7 else 6 end);
alter table public.student_live_maths_progression drop constraint student_live_maths_progression_predicted_level_check;
alter table public.student_live_maths_progression add constraint student_live_maths_progression_predicted_level_check check(predicted_level>=0 and predicted_level<=case when realm_id='number' then 7 else 6 end);
alter table public.student_live_maths_progression drop constraint student_live_maths_progression_checkpoint_level_check;
alter table public.student_live_maths_progression add constraint student_live_maths_progression_checkpoint_level_check check(checkpoint_level>=0 and checkpoint_level<=case when realm_id='number' then 7 else 6 end);
...
begin
  ...
  select
    ...
    case when p_realm_id='number' and assessment.working_level='Year 7' then 7 else public.maths_progression_level_number(assessment.working_level) end,
  ...
  if found and v_assessment_level is not null and v_assessment_score is not null
    ...
    v_checkpoint := greatest(0, least(case when p_realm_id='number' then 7 else 6 end, case
  ...
  v_predicted := least(case when p_realm_id='number' then 7 else 6 end, round((v_checkpoint + v_week_equivalents / v_total_weeks)::numeric, 2));
  v_confidence := least(
    v_max_confidence,
...
$function$
;
-- Extension pre/post evidence uses the existing assessment history and cycle trigger,
-- without assigning an unpublished weekly programme.
...
declare actual_student public.students%rowtype; inserted_count integer; total_correct integer;
begin
 perform public.assert_student_access(p_student_id);
 ...
 if not exists(select 1 from public.student_realm_progress where student_id=p_student_id and realm_id='number' and is_current and working_level in ('Year 6','Year 7')) then raise exception 'Level 7 assessments require a Level 6 or Level 7 Number placement'; end if;
 ...
 if jsonb_array_length(p_attempt->'question_results')<>30 or exists(
  select 1 from generate_series(1,30) n where not exists(select 1 from jsonb_array_elements(p_attempt->'question_results') q where q->>'question_id'='y7-number-'||(case when p_assessment_type='pretest' then 'pre' else 'post' end)||'-'||lpad(n::text,2,'0')||'-v1')
 ) then raise exception 'Question set does not match the released Level 7 form'; end if;
 ...
 perform pg_advisory_xact_lock(hashtextextended(p_student_id::text||':number:Year 7',0));
 if p_assessment_type='posttest' and not exists(select 1 from public.student_realm_assessments where student_id=p_student_id and realm_id='number' and working_level='Year 7' and assessment_type='pretest') then raise exception 'Complete the Level 7 pre-test first'; end if;
 ...
 values(p_student_id,actual_student.class_id,'number',public.realm_program_key('Year 7','number'),actual_student.school_year_level,'Year 7',p_assessment_type,total_correct,30,round(total_correct*100.0/30),total_correct>=26,
 coalesce(p_attempt->'placement_result','{}'::jsonb)||jsonb_build_object('assessment_evidence',jsonb_build_object('realm','number','working_level','Year 7','comparison_group','paired-number-Year 7-2026-09-16-v1','comparability_status','blueprint_matched_uncalibrated','calibration_status','uncalibrated','bank_versions',jsonb_build_array('1'),'baseline_only',false)),p_attempt->'question_results',now());
 return true;
end; $$;
revoke all on function public.complete_number_extension_assessment(uuid,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.complete_number_extension_assessment(uuid,text,uuid,jsonb) to anon,authenticated;
