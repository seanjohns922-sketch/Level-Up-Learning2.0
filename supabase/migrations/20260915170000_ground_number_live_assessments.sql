-- Deploy web first, then apply. Existing Number diagnostic cycles retain their floor/version.
begin;
alter table public.whole_math_diagnostic_sittings add column if not exists number_ground_bank_version integer not null default 1 check(number_ground_bank_version in (1,3));
alter table public.whole_math_diagnostic_sittings alter column number_ground_bank_version set default 3;
create or replace function public.pin_ground_number_diagnostic_version() returns trigger language plpgsql security definer set search_path=public as $$
declare previous integer;
begin
 select s.number_ground_bank_version into previous from public.whole_math_diagnostic_sittings s
 where s.student_id=new.student_id and s.academic_year=new.academic_year order by s.created_at,s.id limit 1;
 new.number_ground_bank_version:=coalesce(previous,3); return new;
end; $$;
revoke all on function public.pin_ground_number_diagnostic_version() from public,anon,authenticated;
drop trigger if exists trg_pin_ground_number_diagnostic_version on public.whole_math_diagnostic_sittings;
create trigger trg_pin_ground_number_diagnostic_version before insert on public.whole_math_diagnostic_sittings for each row execute function public.pin_ground_number_diagnostic_version();

create or replace function public.prepare_ground_number_diagnostic() returns trigger language plpgsql security definer set search_path=public as $$
declare original_level text;
begin
 if new.strand='number' and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=new.sitting_id and s.number_ground_bank_version=3) then
  select coalesce((select p.working_level from public.student_realm_progress p where p.student_id=new.student_id and p.realm_id='number' and p.is_current order by p.updated_at desc nulls last limit 1),s.school_year_level,s.year_level) into original_level from public.students s where s.id=new.student_id;
  if original_level in ('Prep','Foundation','Year 0','Ground') then new.starting_level:='Prep'; end if;
 end if;
 return new;
end; $$;
revoke all on function public.prepare_ground_number_diagnostic() from public,anon,authenticated;
drop trigger if exists trg_prepare_ground_number_diagnostic on public.whole_math_diagnostic_strand_results;
create trigger trg_prepare_ground_number_diagnostic before insert on public.whole_math_diagnostic_strand_results for each row execute function public.prepare_ground_number_diagnostic();

create or replace function public.number_diagnostic_level_value(value text) returns integer language sql immutable as $$
 select case when value='Prep' then 0 when value ~ '^Year [1-6]$' then substring(value from '[0-9]+')::integer else null end;
$$;
create or replace function public.number_diagnostic_level_label(value integer) returns text language sql immutable as $$
 select case when value=0 then 'Prep' else 'Year '||value end;
$$;
create or replace function public.get_pending_whole_math_diagnostic_released(p_student_id uuid)
returns table(sitting_id uuid,checkpoint text,strand text,starting_level text,status text,active_level text,draft_answers jsonb,draft_probes jsonb,draft_index integer,access_open boolean,number_level1_bank_version integer,number_ground_bank_version integer)
language plpgsql security definer set search_path=public as $$
begin
 return query select p.*,s.number_ground_bank_version from public.get_pending_whole_math_diagnostic_versioned(p_student_id) p join public.whole_math_diagnostic_sittings s on s.id=p.sitting_id;
end; $$;
revoke all on function public.get_pending_whole_math_diagnostic_released(uuid) from public,anon,authenticated;
grant execute on function public.get_pending_whole_math_diagnostic_released(uuid) to anon,authenticated;
...
begin
  ...
  if p_strand not in ('number','measurement','space','statistics','algebra','probability')
    or p_active_level not in ('Prep','Year 1','Year 2','Year 3','Year 4','Year 5','Year 6')
    ...
    or jsonb_array_length(p_probe_scores)>(case when p_strand='number' and exists(select 1 from public.whole_math_diagnostic_sittings s where s.id=p_sitting_id and s.number_ground_bank_version=3) then 7 else 6 end) or p_question_index not between 0 and 19 then
...
begin
  ...
  if jsonb_typeof(p_probe_scores)<>'array' or jsonb_array_length(p_probe_scores)<1 or jsonb_array_length(p_probe_scores)>(case when p_strand='number' and v_sitting.number_ground_bank_version=3 then 7 else 6 end) then
  ...
  for v_probe in select value from jsonb_array_elements(p_probe_scores) probe(value) loop
    ...
    if v_level is null or v_level<v_minimum or v_level>6 or v_total<>20 or v_score<0 or v_score>20
  ...
  v_recommended:=case when v_result.placement_protected then v_result.starting_level else public.number_diagnostic_level_label(v_terminal_level) end;
...
begin
  ...
  for v_probe in select value from jsonb_array_elements(p_probe_scores) probe(value) loop
    ...
    v_score := nullif(v_probe->>'score', '')::integer;
    v_total := nullif(v_probe->>'total', '')::integer;
    if v_level is null or v_level<(case when p_strand='number' and v_sitting.number_ground_bank_version=3 then 0 when p_strand in ('algebra','probability') then 3 else 1 end) or v_level>6 or v_level <> v_expected or v_total < 10 or v_total > 20
