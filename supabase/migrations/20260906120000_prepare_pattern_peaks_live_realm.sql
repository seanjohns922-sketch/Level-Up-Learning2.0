...
-- Pattern Peaks release foundation.
-- This migration is intentionally additive and must be applied while Pattern Peaks
-- remains hidden. It widens the proven canonical realm contracts; it does not
-- activate the realm in the application.
...
alter table public.student_completion_receipts
  ...
  check (realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern'));
...
begin
  ...
  if p_class_id is distinct from actual_class_id or p_realm_id not in ('number', 'measurement', 'space', 'statistics', 'pattern') then
...
begin
  perform public.assert_student_access(p_student_id);
  if p_realm_id not in ('number', 'measurement', 'space', 'statistics', 'pattern') then raise exception 'Invalid realm'; end if;
  if not exists (
...
alter table public.realmie_catalogue
  add constraint realmie_catalogue_realm_id_check
    check (realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern', 'global')),
  ...
  add constraint realmie_catalogue_asset_path_check
    check (
      asset_path is null
      or asset_path ~ '^/realmies/(number|measurement|space|statistics|pattern|global)/[a-z0-9-]+\.(png|webp)$'
    ),
  add constraint realmie_catalogue_silhouette_asset_path_check
    check (
      silhouette_asset_path is null
      or silhouette_asset_path ~ '^/realmies/(number|measurement|space|statistics|pattern|global)/[a-z0-9-]+-silhouette\.(png|webp)$'
    );
...
alter table public.realmie_unlock_receipts
  add constraint realmie_unlock_receipts_canonical_realm_id_check
    check (
      ...
      or canonical_realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern', 'global')
    );
...
alter table public.realmie_product_events
  ...
  add constraint realmie_product_events_realm_id_check
    check (
      realm_id is null
      or realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern', 'global')
    );
...
begin
  ...
  if p_realm_id is not null
    and p_realm_id not in ('number', 'measurement', 'space', 'statistics', 'pattern', 'global') then
  ...
  from (
    ...
    from (
      ...
      where attempt.student_id = p_student_id
        ...
        and attempt.realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern')
...
begin
  if new.completed
    and new.realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern')
...
begin
  for v_student in
    ...
    from (
      ...
      where completed
        and realm_id in ('number', 'measurement', 'space', 'statistics', 'pattern')
...
begin
  ...
  if p_realm_id = 'pattern'
    and p_assigned_level = 'Year 3'
    and v_entry = 'pretest' then
    raise exception 'Pattern Peaks Year 3 starts directly without a pre-test';
  end if;
...
begin
  ...
  if p_realm_id = 'pattern'
    and p_working_level = 'Year 3'
    and p_assessment_type = 'pretest' then
    raise exception 'Pattern Peaks Year 3 does not use a pre-test';
  end if;
...
-- Stage Algebra as the fifth Whole-Maths Diagnostic strand. The official
-- Whole-Maths result remains locked because Probability is not released.
...
begin
  ...
  -- During development, an explicit subset of the five available strands is
  -- required. Remove this guard only when Probability has a real
  ...
  if exists (
    select 1 from unnest(v_requested) requested(strand)
    where requested.strand = 'probability'
  ) then
    raise exception 'Probability diagnostics are not available yet';
  end if;
  if exists (
  ...
  foreach v_strand in array v_requested loop
    ...
    if v_realm_id is null then
      ...
      if v_strand = 'algebra' and v_starting_level not in ('Year 3', 'Year 4', 'Year 5', 'Year 6') then
...
alter table public.student_live_maths_progression
  ...
  add constraint student_live_maths_progression_strand_check
    check (strand in ('number', 'measurement', 'space', 'statistics', 'algebra'));
...
begin
  if p_realm_id not in ('number', 'measurement', 'space', 'statistics', 'pattern') then return; end if;
  ...
  insert into public.student_live_maths_progression (
    ...
    case when p_realm_id = 'pattern' then 'algebra' else p_realm_id end,
