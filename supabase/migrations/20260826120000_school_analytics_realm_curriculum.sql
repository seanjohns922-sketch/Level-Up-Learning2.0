-- Phase SA1.1: add realm_id to the curriculum evidence rollup so school
-- analytics can attach AC9 strand + achievement band per topic.
-- Regenerated from 20260825110000_school_analytics_sa1.sql (curriculum block only).
...
begin
  ...
  select jsonb_build_object(
    ...
    'curriculum', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'averageAccuracy', average_accuracy
    ) order by year_level, topic_focus) from (
      select coalesce(cl.topic_focus, 'Unlabelled lesson evidence') as topic_focus,
        co.year_level, cl.realm_id as realm_id, count(distinct cl.student_id) as students, count(*) as evidence_count,
        round(avg(cl.accuracy_percent), 1) as average_accuracy
      from canonical_lessons cl join cohort co on co.student_id = cl.student_id
      group by coalesce(cl.topic_focus, 'Unlabelled lesson evidence'), co.year_level, cl.realm_id
    ) topics), '[]'::jsonb),
    ...
    ) order by student_name) from student_summary), '[]'::jsonb),
