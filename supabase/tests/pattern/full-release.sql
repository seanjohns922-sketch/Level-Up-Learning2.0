...
select parent_id,'pattern-verify-'||parent_id||'@example.invalid','authenticated','authenticated','{"role":"parent"}'::jsonb from verify_ids;
insert into public.user_profiles(user_id,email,display_name,status)
select parent_id,'pattern-verify-'||parent_id||'@example.invalid','Synthetic Pattern Verification','active' from verify_ids
...
create temporary table verify_student as select public.create_home_student_for_parent('Synthetic','Pattern Verification','Prep','Prep','2468') result;
...
set local role anon;
select pg_temp.check_true((select pattern_release_version=1 and space_release_version=1 and measurement_release_version=1 and number_maximum_level=8 and access_open from public.get_pending_whole_math_diagnostic_pattern_full((select (result->>'studentId')::uuid from verify_student))), 'Anon student login returns all four released banks and an open session');
do $$declare r text; probes jsonb; output jsonb; sid uuid:=(select (result->>'studentId')::uuid from verify_student); sitting uuid:=(select id from verify_sitting);begin
 for r in select unnest(array['number','measurement','space','statistics','algebra']) loop
...
do $$declare headers text:=current_setting('request.headers',true); sid uuid:=(select (result->>'studentId')::uuid from verify_student); denied boolean:=false;begin
 ...
 begin perform public.get_pending_whole_math_diagnostic_pattern_full(sid); exception when insufficient_privilege then denied:=true; end;
 perform pg_temp.check_true(denied,'Missing student session rejected');
 denied:=false;begin perform public.complete_pattern_extension_assessment(sid,'pretest',gen_random_uuid(),'{}'::jsonb); exception when insufficient_privilege then denied:=true;end;
 ...
 begin perform public.get_pending_whole_math_diagnostic_pattern_full(gen_random_uuid()); exception when insufficient_privilege then denied:=true; end;
 perform pg_temp.check_true(denied,'Cross-student session rejected');
 denied:=false;begin perform public.complete_pattern_extension_assessment(gen_random_uuid(),'pretest',gen_random_uuid(),'{}'::jsonb); exception when insufficient_privilege then denied:=true;end;
...
update public.whole_math_diagnostic_sittings set pattern_release_version=0 where id=(select id from legacy_cycle);
...
select pg_temp.check_true((select bool_and(pattern_release_version=0) from public.whole_math_diagnostic_sittings where student_id=(select (result->>'studentId')::uuid from verify_student) and academic_year=extract(year from now())::integer-1),'Existing academic-year bank remains pinned');
...
do $$declare sid uuid:=(select (result->>'studentId')::uuid from verify_student); cid uuid; n integer; form text; attempt jsonb; completion uuid;begin
 ...
 for n in 3..6 loop
  foreach form in array array['pretest','posttest'] loop
   select jsonb_build_object('correct_count',12,'total_questions',20,'score_percent',60,'question_results',jsonb_agg(jsonb_build_object('question_id','y'||n||'-pattern-'||form||'-'||lpad(q::text,2,'0')||'-v1','question_number',q,'correct',q<=12,'student_answer',jsonb_build_object('choice','option-0','values','[]'::jsonb,'touched',true)))) into attempt from generate_series(1,20) q;
   completion:=gen_random_uuid();
   perform pg_temp.check_true(public.complete_realm_assessment(sid,cid,'pattern','year'||n||'-pattern','Year 6','Year '||n,form,completion,attempt,'{"status":"ASSIGNED_PROGRAM","placement_complete":true,"current_week":1,"assigned_week":1,"required_weeks":[1,2,3,4,5,6]}'::jsonb),'Pattern Level '||n||' '||form||' saves through student session');
  end loop;
 end loop;
end$$;
reset role;
...
update public.student_realm_progress set is_current=false where student_id=(select (result->>'studentId')::uuid from verify_student) and realm_id='pattern';
...
select (s.result->>'studentId')::uuid,c.id,'pattern','year6-pattern','Year 6','Year 6',true,'ASSIGNED_PROGRAM' from verify_student s,verify_class c
...
do $$declare sid uuid:=(select (result->>'studentId')::uuid from verify_student); n integer; form text; attempt jsonb; completion uuid;begin
 for n in 7..8 loop
  foreach form in array array['pretest','posttest'] loop
   ...
   completion:=gen_random_uuid();
   perform pg_temp.check_true(public.complete_pattern_extension_assessment(sid,form,completion,attempt),'Pattern Level '||n||' '||form||' saves through student session');
   perform pg_temp.check_true(not public.complete_pattern_extension_assessment(sid,form,completion,attempt),'Extension duplicate completion is idempotent');
  end loop;
  perform pg_temp.check_true((select count(*)=2 from public.get_student_realm_assessments_secure(sid,'pattern','Year '||n)),'Extension history returns pre and post for Level '||n);
 end loop;
 perform pg_temp.check_true((select working_level='Year 6' from public.get_student_realm_progress_compat_secure(sid,'pattern') where is_current),'Extension leaves weekly placement unchanged');
end$$;
reset role;
