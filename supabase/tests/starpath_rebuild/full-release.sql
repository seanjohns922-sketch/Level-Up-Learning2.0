...
select v.id,(s.result->>'studentId')::uuid,r,r,'pending','Year 6',true from verify_sitting v,verify_student s,unnest(array['number','measurement','space','statistics']) r;
...
set local role anon;
select pg_temp.check_true((select space_release_version=1 and measurement_release_version=1 and number_maximum_level=8 and access_open from public.get_pending_whole_math_diagnostic_starpath_full((select (result->>'studentId')::uuid from verify_student))), 'Anon student login returns all three released banks and an open session');
do $$declare r text; probes jsonb; output jsonb; sid uuid:=(select (result->>'studentId')::uuid from verify_student); sitting uuid:=(select id from verify_sitting);begin
 for r in select unnest(array['number','measurement','space']) loop
...
do $$declare headers text:=current_setting('request.headers',true); sid uuid:=(select (result->>'studentId')::uuid from verify_student); denied boolean:=false;begin
 ...
 begin perform public.get_pending_whole_math_diagnostic_starpath_full(sid); exception when insufficient_privilege then denied:=true; end;
 ...
 begin perform public.get_pending_whole_math_diagnostic_starpath_full(gen_random_uuid()); exception when insufficient_privilege then denied:=true; end;
...
update public.student_realm_progress set is_current=false where student_id=(select (result->>'studentId')::uuid from verify_student) and realm_id='space';
...
select (s.result->>'studentId')::uuid,c.id,'space','year6-space','Year 6','Year 6',true,'ASSIGNED_PROGRAM' from verify_student s,verify_class c
...
do $$declare sid uuid:=(select (result->>'studentId')::uuid from verify_student); n integer; form text; attempt jsonb; completion uuid;begin
 for n in 7..8 loop
  foreach form in array array['pretest','posttest'] loop
   select jsonb_build_object('working_level','Year '||n,'total_questions',30,'correct_count',18,'question_results',jsonb_agg(jsonb_build_object('question_id','y'||n||'-starpath-'||form||'-'||lpad(q::text,2,'0')||'-v9','question_number',q,'correct',q<=18,'student_answer',jsonb_build_object('selected',jsonb_build_array('o1'))))) into attempt from generate_series(1,30) q;
   completion:=gen_random_uuid();
   perform pg_temp.check_true(public.complete_space_extension_assessment(sid,form,completion,attempt),'Space Level '||n||' '||form||' saves through student session');
   perform pg_temp.check_true(not public.complete_space_extension_assessment(sid,form,completion,attempt),'Extension duplicate completion is idempotent');
  end loop;
  perform pg_temp.check_true((select count(*)=2 from public.get_student_realm_assessments_secure(sid,'space','Year '||n)),'Extension history returns pre and post for Level '||n);
 end loop;
 perform pg_temp.check_true((select working_level='Year 6' from public.get_student_realm_progress_compat_secure(sid,'space') where is_current),'Extension leaves weekly placement unchanged');
end$$;
reset role;
