...
export default function IdentityCentreClient({ initialSnapshot }: { initialSnapshot: PlatformIdentityCentre }) {
  ...
  const previewConflicts = Array.isArray(preview?.preview.conflicts)
    ? preview.preview.conflicts as Array<{ code?: string; label?: string; detail?: string }>
    : [];
  const previewIsMergeable = preview?.preview.mergeable === true;
  ...
  return <div className="space-y-6">
    ...
    <section className="border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {preview ? <div className={`mt-5 border p-4 ${previewIsMergeable ? "border-amber-300 bg-amber-50" : "border-red-300 bg-red-50"}`}><h3 className={`font-black ${previewIsMergeable ? "text-amber-950" : "text-red-950"}`}>{previewIsMergeable ? "Final review required" : "Merge blocked"}</h3><p className={`mt-1 text-sm ${previewIsMergeable ? "text-amber-900" : "text-red-900"}`}>{previewIsMergeable ? "Only one identity owns state in each protected domain. Approval retires the duplicate and transfers that one-sided state." : "Both identities own canonical state in one or more protected domains. Resolve these conflicts outside the automatic merge before creating a new request."}</p>{previewConflicts.length ? <div className="mt-3 space-y-2">{previewConflicts.map((conflict) => <div key={conflict.code ?? conflict.label} className="border border-red-200 bg-white p-3"><p className="font-bold text-red-900">{conflict.label ?? conflict.code}</p><p className="mt-1 text-sm text-red-800">{conflict.detail}</p></div>)}</div> : null}<pre className="mt-3 max-h-56 overflow-auto bg-white p-3 text-xs">{JSON.stringify(preview.preview, null, 2)}</pre>{previewIsMergeable ? <><input value={reviewReason} onChange={(event) => setReviewReason(event.target.value)} className="mt-3 w-full border border-amber-300 px-3 py-2" placeholder="Final review reason"/><div className="mt-3 flex gap-2"><button type="button" disabled={busy || !reviewReason.trim()} onClick={() => void resolve(preview.requestId, true, reviewReason)} className="bg-red-700 px-4 py-2 font-bold text-white disabled:opacity-40">Approve merge</button><button type="button" onClick={() => setPreview(null)} className="border border-slate-300 bg-white px-4 py-2 font-bold">Cancel</button></div></> : <div className="mt-3 flex gap-2"><button type="button" onClick={() => setPreview(null)} className="border border-slate-300 bg-white px-4 py-2 font-bold">Close</button><button type="button" onClick={() => void resolve(preview.requestId, false, "Blocked by unresolved identity conflicts") } className="border border-red-300 bg-white px-4 py-2 font-bold text-red-800">Reject request</button></div>}</div> : null}
    </section>
