    ...
    <div className="space-y-7">
      ...
      <section className="border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 divide-y divide-slate-100 border border-slate-200">
          ...
          {detail.adminInvitations.filter((invitation) => invitation.status === "pending").map((invitation) => <div key={invitation.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{invitation.email}</p><p className="text-sm text-amber-700">Pending invitation · expires {invitation.expiresAt.slice(0, 10)} · code {detail.school.code}</p></div>{!archived ? <div className="flex flex-wrap gap-2"><a href={schoolAdminInviteMailto(invitation.email, detail.school.name, detail.school.code)} className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800">Draft email</a><button type="button" disabled={busy} onClick={() => manageAdmin("resend_invitation", undefined, invitation.id)} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-bold">Resend email</button><button type="button" disabled={busy} onClick={() => manageAdmin("revoke_invitation", undefined, invitation.id)} className="rounded-md border border-red-200 px-3 py-2 text-xs font-bold text-red-700">Revoke</button></div> : null}</div>)}
