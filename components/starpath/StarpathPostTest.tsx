...
import { ArrowLeft, ArrowRight, Check, Orbit, RotateCcw, Trophy, X } from "lucide-react";
import { REALM_QUIZ_THEMES } from "@/components/quiz/RealmWeeklyQuizChrome";
import { TaskRenderer } from "@/components/TaskRenderer";
import ReadAloudBtn from "@/components/ReadAloudBtn";
import { posttestPassed, ASSESSMENT_THRESHOLDS } from "@/lib/assessment-rules";
...
export type StarpathPostTestMeta = {
  /** Working level / year label used for persistence, e.g. "Foundation". */
  year: string;
  levelLabel: string;
  title: string;
  coverage: string;
...
export default function StarpathPostTest({
  meta,
  tasks,
}: {
  meta: StarpathPostTestMeta;
  tasks: PracticeTask[];
}) {
  ...
  const passed = posttestPassed(percent);

  function persist(finalCorrect: number) {
    const finalPercent = total > 0 ? Math.round((finalCorrect / total) * 100) : 0;
    const didPass = posttestPassed(finalPercent);
    const studentId = getActiveStudentIdentity().studentId;
    if (!studentId) return;
    // Same rules every realm uses: a passed post-test unlocks the realm Legend.
    const legend = getLegendForYear(meta.year, "starpath");
    const progress = { unlocked_legends: didPass && legend ? [legend.id] : [] };
    void saveRealmAssessment(
      studentId,
      meta.year,
      "posttest",
      { percent: finalPercent, score: finalCorrect, total, passed: didPass },
      newCompletionKey(),
      progress,
      "space",
    ).catch((error) => console.warn("[Starpath] Post-test persist failed", error));
  }
  ...
  return (
    <main className="relative isolate min-h-screen px-3 py-4 text-white sm:px-6">
      <div className="mx-auto w-full max-w-[1200px]">
        {/* Header */}
        <header
          className="flex flex-wrap items-center justify-between gap-3 rounded-t-lg border border-b-0 px-4 py-4 sm:px-7"
          style={{ background: "linear-gradient(120deg,#1e1b4b,#4c1d95 55%,#0e7490)", borderColor: theme.panelBorder }}
        >
          <button
            type="button"
            onClick={() => router.push(meta.programHref)}
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Back to Week 8
          </button>
          <div className="flex items-center gap-2 text-center">
            <Trophy className="h-5 w-5 text-amber-300" />
            <div>
              <div className="font-mono text-[11px] font-black uppercase tracking-[0.2em] text-cyan-200">
                {meta.levelLabel} · Post Test
              </div>
              <div className="text-base font-black">{meta.title}</div>
            </div>
            <ReadAloudBtn
              text={`${meta.title}. ${total} questions. Score ${ASSESSMENT_THRESHOLDS.posttestPassPercent} percent or more to graduate. ${meta.coverage}`}
              label="Read"
              size="sm"
              className="border-cyan-200/30 bg-white/10 text-cyan-100"
            />
          </div>
          <div className="flex items-center gap-2 font-mono text-xs font-black uppercase tracking-[0.16em] text-violet-200">
            <Orbit className="h-5 w-5 text-cyan-300" /> Starpath
          </div>
        </header>
            ...
            <div className="mx-auto max-w-xl text-center">
              ...
              <div
                className={[
                  ...
                  passed ? "bg-gradient-to-br from-amber-500 to-violet-600" : "bg-gradient-to-br from-slate-500 to-slate-700",
                ].join(" ")}
              >
                <span className="text-3xl font-black">{percent}%</span>
              </div>
              <h2 className="mt-5 text-3xl font-black text-slate-950">
                {passed ? "You graduated!" : "Keep training, Explorer!"}
              </h2>
              <p className="mt-2 text-base font-semibold text-slate-600">
                You answered {correct} of {total} correctly.
                {passed
                  ? ` That passes the ${ASSESSMENT_THRESHOLDS.posttestPassPercent}% mark — your Starpath Legend is unlocked!`
                  : ` You need ${ASSESSMENT_THRESHOLDS.posttestPassPercent}% to graduate. Try again to unlock your Legend.`}
              </p>
              <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
                ...
                >
                  Back to Program <ArrowRight className="h-5 w-5" />
                </button>
              </div>
            </div>
          )}
