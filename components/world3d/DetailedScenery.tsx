...
export function DetailedReward({ assetKey }: {
    ...
    if (["puppy_yard", "bunny_garden", "pony_paddock"].includes(assetKey))
        return <group>{[-1, 1].flatMap(side => [-11.95, -8, -4, 0, 4, 8, 11.95].map(x => <Pole key={`${side}-${x}`} a={[x, 0, side * 9.95]} b={[x, .8, side * 9.95]} radius={.055}/>))}{[-1, 1].flatMap(side => [.3, .65].map(y => <Pole key={`${side}-${y}`} a={[-11.95, y, side * 9.95]} b={[11.95, y, side * 9.95]} radius={.035}/>))}<group position={[-8, 0, -6]} scale={1.2}><Shelter variant="gazebo"/></group>{assetKey === "pony_paddock" ? <group position={[2, 0, 2]} scale={1.2}><Ball p={[0, .9, 0]} s={[.33, .4, .65]} c="#936c48"/><Ball p={[0, 1.4, .5]} s={[.22, .4, .23]} c="#936c48"/><Ball p={[0, 1.63, .69]} s={[.18, .2, .29]} c="#a27c55"/>{[-1, 1].flatMap(x => [-1, 1].map(z => <Pole key={`${x}-${z}`} a={[x * .23, .1, z * .4]} b={[x * .23, .86, z * .4]} radius={.07} c="#815c3e"/>))}{[-.1, .1].map(x => <Ball key={x} p={[x, 1.88, .58]} s={[.06, .15, .06]} c="#815c3e"/>)}<Pole a={[0, 1.1, -.55]} b={[0, .4, -.85]} radius={.08} c="#473c30"/><Ball p={[.17, 1.69, .69]} s={[.025, .025, .025]} c="#242923"/><Ball p={[-.17, 1.69, .69]} s={[.025, .025, .025]} c="#242923"/></group> : <group position={[2, 0, 2]} scale={1.35}><Animal variant={assetKey === "puppy_yard" ? "blue_heeler" : "bilby"}/></group>}<Ring p={[1.3, .1, -.8]} radius={.22} tube={.065} c="#8c958b"/></group>;
    ...
    if (assetKey === "cinema")
        return <group><Box p={[0,.04,0]} s={[22,.08,16]} c="#767d69"/><Box p={[0,5.5,-7]} s={[12,6,.3]} c="#3e4b46"/><Box p={[0,5.5,-6.83]} s={[11.4,5.4,.03]} c="#d3d7c9"/>{[-4,4].map(x=><Pole key={x} a={[x,0,-7]} b={[x,6,-7]} radius={.16} c={iron}/>)}
        {[-5,0,5].flatMap(x=>[1,6].map(z=><group key={`${x}-${z}`} position={[x,0,z]}><Box p={[0,.6,0]} s={[1.8,.7,3.8]} c={x<0?"#8b5c4c":"#749187"}/><Box p={[0,1.15,0]} s={[1.6,.65,1.8]} c="#adbbad"/>{[-1,1].flatMap(side=>[-1.15,1.15].map(axle=><Ball key={`${side}-${axle}`} p={[side*.86,.35,axle]} s={[.16,.35,.35]} c="#303732"/>))}</group>))}</group>;
