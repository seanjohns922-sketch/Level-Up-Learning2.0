...
export default function NumberAdventure3DWorld({access}:{access?:ExpeditionAccess}){
 ...
 <section className="summit-title"><p className="summit-eyebrow"><Mountain size={16}/> NEW CHAPTER UNLOCKED</p><h1>The Shattered<br/>Realms</h1><p className="summit-intro-copy">Six stolen Cores. Six paths. One final ascent.<br/>Follow your realm’s trail to Level 7 weekly lessons.<br/>Complete Level 7 to open the volcano.</p><button className="summit-primary" onClick={()=>setStarted(true)}>Begin your adventure <ArrowUpRight size={22}/></button><div className="summit-intro-note"><ReadAloudBtn text="The Shattered Realms. Six stolen Cores. Six paths. One final ascent. Six trails lead to six realm lesson gates. Complete a realm’s Level 7 to unlock the volcano and its Level 8 stronghold. "/><span>{preview?'Design preview · Explore all six approaches':'Your next adventure starts here'}</span></div></section></div>:<>
 <header className="summit-hud"><div className="summit-brand"><Mountain size={24}/><div><small>{onVolcano?'THE FINAL BATTLE · LEVEL 8':trailIndex===null?'SIXFOLD BASE CAMP':EXPEDITION_TRAILS[trailIndex].name.toUpperCase()}</small><strong>The Shattered Realms</strong></div></div><button className="summit-icon-button" aria-label="Open expedition menu" onClick={()=>open('menu')}><Compass size={21}/></button></header>
