...
export default function NumberAdventure3DWorld({access}:{access?:ExpeditionAccess}){
 ...
 <header className="summit-hud"><div className="summit-brand"><Mountain size={24}/><div><small>{onVolcano?'ASHEN PASS · LEVEL 8':trailIndex===null?'SIXFOLD BASE CAMP':EXPEDITION_TRAILS[trailIndex].name.toUpperCase()}</small><strong>The Shattered Realms</strong></div></div><button className="summit-icon-button" aria-label="Open expedition menu" onClick={()=>open('menu')}><Compass size={21}/></button></header>
