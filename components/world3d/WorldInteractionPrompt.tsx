  ...
  return (
    <div className="worldInteractionPrompt" role="group" aria-label={location}>
      ...
      <style>{`
        ...
        @media(max-width:700px){.worldInteractionPrompt{bottom:max(82px,calc(env(safe-area-inset-bottom) + 70px));gap:9px}.worldInteractionCopy strong{font-size:12px}.worldInteractionPrompt button:not(.worldVoiceButton){min-height:46px;padding:9px 12px}}
      `}</style>
    </div>
  );
}
