...
export function ConnectedBoundary({assetKey,placement,neighbours}:{assetKey:string;placement:CentralWorldPlacement;neighbours:CentralWorldPlacement[]}){
 ...
 const height=castle?3.05:stone?.86:1.1;
 return <group rotation={[0,-placement.rotation*Math.PI/180,0]}>
  ...
  {directions.map(([x,z])=><group key={x+":"+z} rotation={[0,Math.atan2(-z,x),0]}>
   {stone||castle?<group position={[.5,0,0]}><Wall width={1} height={height} depth={castle?.4:.3} battlements={castle} tint={tint}/></group>:
    <group>
