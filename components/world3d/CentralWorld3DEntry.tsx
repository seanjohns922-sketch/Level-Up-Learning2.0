...
export default function CentralWorld3DEntry({ teacherPreview = false }: { teacherPreview?: boolean }) {
  ...
  const [access] = useState(() => {
    ...
    return resolveRealm3DAccess({ realmId: "number", classId: profile?.classId, studentId: profile?.studentId, respectReducedMotion: true });
  });
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        ...
        console.warn("[CentralWorld3D] Canonical restore failed", error);
        setStatus("error");
      }
    };
    void restore();
