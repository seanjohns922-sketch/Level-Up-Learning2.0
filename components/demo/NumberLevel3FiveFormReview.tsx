...
import NumberLevel3QuestionCard from "@/components/assessment/NumberLevel3QuestionCard";
import ReadAloudBtn, { ReadAloudRateProvider } from "@/components/ReadAloudBtn";
import { isAssessmentAnswerCorrect } from "@/data/assessments/analysis";
import { level3SpokenPrompt, NUMBER_LEVEL3_FIVE_FORMS, NUMBER_LEVEL3_FORMS, NUMBER_LEVEL3_FORM_LABELS, type NumberLevel3Form } from "@/data/assessments/revisions/year3NumberFiveForms";
...
export default function NumberLevel3FiveFormReview() {
  const router = useRouter();
  const params = useSearchParams();
  const requestedForm = params.get("form");
  const form: NumberLevel3Form = NUMBER_LEVEL3_FORMS.find(f => f === requestedForm) ?? "posttest";
  ...
  const [finished,setFinished] = useState<Partial<Record<NumberLevel3Form,boolean>>>({});
  const questions = NUMBER_LEVEL3_FIVE_FORMS[form];
  ...
  function select(nextForm: NumberLevel3Form, nextIndex: number) {
    setShowAnswer(false);
    router.replace(`/demo-review/number-level-3?form=${nextForm}&question=${nextIndex+1}`, {scroll:false});
  }
  return <ReadAloudRateProvider>
    <div className="bg-[#001b18] px-4 pb-4 pt-20 text-white md:pt-4">
      <section className="mx-auto max-w-6xl space-y-3" aria-label="Five-form review controls">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-xl font-black">Number Nexus · Level 3 · Five-form review</h1>
          <span className="text-sm text-teal-100">Review only · Student results are not saved</span>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {NUMBER_LEVEL3_FORMS.map(f => <button key={f} type="button" aria-pressed={form===f} onClick={()=>select(f,index)} className={`${buttonClass} ${form===f ? "bg-teal-200 text-teal-950" : "bg-teal-950 text-teal-50"}`}>{NUMBER_LEVEL3_FORM_LABELS[f]}</button>)}
    ...
    <AssessmentShell
      testType={NUMBER_LEVEL3_FORM_LABELS[form]}
      year="Year 3" realmId="number" currentIndex={index} totalQuestions={questions.length}
      subtitle="Matched Level 3 forms · Manual review"
      questionPrompt={question.prompt}
      promptAction={<ReadAloudBtn text={level3SpokenPrompt(question)}/>}
      questionContent={<>
        <NumberLevel3QuestionCard key={question.id} question={question} value={value === "idk" ? "" : value} onChange={answer=>setAnswers(previous=>({...previous,[question.id]:answer}))}/>
        ...
        <details className="mt-5 rounded-lg border border-teal-700 p-4 text-teal-50">
          ...
          <p className="mt-3 text-sm">{question.curriculumCodes.join(", ")} · {question.skillLabel} · Intended difficulty: {question.difficulty}</p>
        ...
        {finished[form] ? <div role="status" className="mt-5 rounded-lg border border-teal-500 bg-teal-950 p-4 text-teal-50">
          <p className="font-bold">{NUMBER_LEVEL3_FORM_LABELS[form]} review: {correct}/20 ({correct*5}%)</p>
      ...
      onExit={()=>router.push("/demo-review?realm=number&year=Year%203")}
    />
