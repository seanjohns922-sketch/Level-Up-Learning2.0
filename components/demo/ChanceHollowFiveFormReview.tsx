...
import {CH_FORMS,CH_LABELS,LEVEL3_CHANCE_FORMS,chEmpty,chReady,chScore,chSpeech,type CHForm,type CHResponse} from '@/data/assessments/revisions/level3ChanceHollowFiveForms';
...
export default function ChanceHollowFiveFormReview(){
 const router=useRouter(),params=useSearchParams(),form=CH_FORMS.find(f=>f===params.get('form'))??'posttest',items=LEVEL3_CHANCE_FORMS[form];
 ...
 const select=(f:CHForm,i:number)=>{stopSpeaking();setShow(false);router.replace(`/demo-review/chance-level3?form=${f}&question=${i+1}`,{scroll:false});};
 ...
 const trial=()=>{if(r.trials.length>=q.target!)return;const u=new Uint32Array(1);crypto.getRandomValues(u);const outcome=q.outcomes![u[0]/4294967296<(q.experiment==='coin'?.5:.75)?0:1];update({...r,trials:[...r.trials,outcome],values:[]});};
 ...
 <header className={styles.header}><div><p>CHANCE HOLLOW · LEVEL 3</p><h1>Level 3 assessments</h1></div><Link href='/demo-review?realm=chance&year=Year%203'>Back to review</Link></header>
 ...
 <div className={styles.question}><h2 id='ch-question'>{q.prompt}</h2><ReadAloudBtn text={[q.prompt,q.instruction].filter(Boolean).join('. ')} label='Read question' className={styles.voice}/></div>{q.instruction&&<p className={styles.instruction}>{q.instruction}</p>}
 ...
 <p className={ch.caption}>{q.caption}</p>
 {q.apparatus&&<div className={ch.apparatus}><ChanceVisual visual={q.apparatus}/></div>}
 {q.mode==='experiment'&&<div className={ch.experiment}><button className={styles.primary} disabled={r.trials.length===q.target} onClick={trial}>{q.experiment==='coin'?'Toss coin':'Spin'}</button><span aria-live='polite'>{r.trials.length}/{q.target} trials{r.trials.length>0?` · Latest: ${r.trials.at(-1)}`:''}</span></div>}
