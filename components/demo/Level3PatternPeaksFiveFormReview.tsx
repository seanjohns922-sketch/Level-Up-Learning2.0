...
import {LEVEL3_PP_FORMS,PP_FORMS,PP_LABELS,PP_OPERATIONS,ppEmpty,ppReady,ppScore,ppVisualSpeech,type PPForm,type PPVisual,type PPResponse} from '@/data/assessments/revisions/level3PatternPeaksFiveForms';
...
function Visual({v}:{v:PPVisual}){
 ...
 v.kind==='array'?<><h4>{v.rows} rows of {v.columns} counters</h4><div className={pp.array}>{Array.from({length:v.rows},(_,r)=><div className={pp.arrayRow} key={r}>{Array.from({length:v.columns},(_,c)=><span className={pp.counter} data-pp-counter key={c}/>)}</div>)}</div></>:
...
export default function Level3PatternPeaksFiveFormReview(){
 const router=useRouter(),params=useSearchParams();const form=PP_FORMS.find(f=>f===params.get('form'))??'posttest';
 const raw=Number(params.get('question')??1)-1,index=Number.isInteger(raw)?Math.max(0,Math.min(19,raw)):0,items=LEVEL3_PP_FORMS[form],item=items[index];
 ...
 const select=(f:PPForm,i:number)=>{stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/pattern-level3?form=${f}&question=${i+1}`,{scroll:false});};
 ...
 const expected=item.mode==='choice'?item.options[item.correct]:item.mode==='partition'?`Any two positive whole numbers totalling ${item.total}.`:item.mode==='algorithm'?`Any two steps that multiply each input by ${item.multiplier}.`:item.answers.join(', ');
 ...
 <header className={styles.header}><div><p>PATTERN PEAKS · LEVEL 3</p><h1>Level 3 assessments</h1></div><Link href='/demo-review?realm=pattern&year=Year%203'>Back to review</Link></header>
