...
import { LEVEL1_STATISTICA_FORMS } from "@/data/assessments/revisions/level1StatisticaFiveForms";
...
import Level1StatisticaAssessmentCard from "@/components/statistica/Level1StatisticaAssessmentCard";
...
export default function Level1StatisticaFiveFormReview() {
  ...
  const items = LEVEL1_STATISTICA_FORMS[form];
  ...
  const select = (nextForm: StatisticaForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/statistica-level1?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  ...
  return <main className={styles.page} style={{"--light":theme.chipText,"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={styles.header}><div><p>STATISTICA · LEVEL 1</p><h1>Level 1 assessments</h1></div><Link href="/demo-review?realm=statistics&year=Year%201">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="stats-question-heading" data-assessment-task-kind={`statisticaLevel1V2-${item.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {STATISTICA_FORM_LABELS[form]}</p>
        <Level1StatisticaAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
