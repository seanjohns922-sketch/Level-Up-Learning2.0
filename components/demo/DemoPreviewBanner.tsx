...
export default function DemoPreviewBanner() {
  ...
  function exitDemoMode() {
    if (reviewingAssessment) {
      ...
      const year = pathname === "/demo-review/measurement-level4" ? "Year 4" : pathname === "/demo-review/measurement-level3" ? "Year 3" : pathname === "/demo-review/measurement-level2" ? "Year 2" : pathname === "/demo-review/measurement-level1" ? "Year 1" : /^\/demo-review\/number-level-[1-8]$/.test(pathname) ? `Year ${pathname.slice(-1)}` : searchParams.get("year") ?? searchParams.get("level") ?? "Prep";
