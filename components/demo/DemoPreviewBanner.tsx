...
export default function DemoPreviewBanner() {
  ...
  function exitDemoMode() {
    if (reviewingAssessment) {
      ...
      const year = /^\/demo-review\/number-level-[78]$/.test(pathname) ? "Year 6" : /^\/demo-review\/number-level-[1-8]$/.test(pathname) ? `Year ${pathname.slice(-1)}` : searchParams.get("year") ?? searchParams.get("level") ?? "Prep";
