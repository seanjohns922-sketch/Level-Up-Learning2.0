...
import { LEVEL6_STARPATH_FORMS } from "@/data/assessments/revisions/level6StarpathFiveForms";
import { emptyLevel6Response, level6ResponseReady, scoreLevel6Response, level6Expected, type Level6Response } from "@/lib/starpath-level6-review";
...
type RecordValue = { response: Level6Response; submitted: boolean; skipped: boolean };
export default function Level6StarpathFiveFormReview() {
  ...
  const items = LEVEL6_STARPATH_FORMS[form];
  ...
  const response = saved?.response ?? emptyLevel6Response(item);
  const theme = getRealmTheme("space");
  const select = (nextForm: GroundStarpathForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/starpath-level6?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  const update = (next:Level6Response) => {setRecords(old=>({...old,[item.id]:{response:next,submitted:level6ResponseReady(item,next),skipped:false}}));setFinished(old=>({...old,[form]:false}));};
  ...
  const correct = items.filter(q=>records[q.id]?.submitted&&!records[q.id].skipped&&scoreLevel6Response(q,records[q.id].response)).length;
  const expected = level6Expected(item);
  return <main className={styles.page} style={{"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={`${styles.header} ${styles.fullReviewHeader}`}><div><p>STARPATH · LEVEL 6</p><h1>Five redesigned assessments</h1></div><Link href="/demo-review?realm=space&year=Year%206">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="ground-question-heading" data-assessment-task-kind={`starpathLevel6V6-${item.task.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {GROUND_STARPATH_LABELS[form]}</p>
        <Level6StarpathAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
