...
import { LEVEL1_STARPATH_FORMS } from "@/data/assessments/revisions/level1StarpathFiveForms";
import { emptyLevel1Response, level1ResponseReady, scoreLevel1Response, level1Expected, type Level1Response } from "@/lib/starpath-level1-review";
...
type RecordValue = { response: Level1Response; submitted: boolean; skipped: boolean };
export default function Level1StarpathFiveFormReview() {
  ...
  const items = LEVEL1_STARPATH_FORMS[form];
  ...
  const response = saved?.response ?? emptyLevel1Response(item);
  const theme = getRealmTheme("space");
  const select = (nextForm: GroundStarpathForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/starpath-level1?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  const update = (next:Level1Response) => {setRecords(old=>({...old,[item.id]:{response:next,submitted:level1ResponseReady(item,next),skipped:false}}));setFinished(old=>({...old,[form]:false}));};
  ...
  const correct = items.filter(q=>records[q.id]?.submitted&&!records[q.id].skipped&&scoreLevel1Response(q,records[q.id].response)).length;
  const expected = level1Expected(item);
  return <main className={styles.page} style={{"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={`${styles.header} ${styles.fullReviewHeader}`}><div><p>STARPATH · LEVEL 1</p><h1>Five redesigned assessments</h1></div><Link href="/demo-review?realm=space&year=Year%201">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="ground-question-heading" data-assessment-task-kind={`starpathLevel1V5-${item.task.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {GROUND_STARPATH_LABELS[form]}</p>
        <Level1StarpathAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
