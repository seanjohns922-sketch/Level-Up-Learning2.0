...
export default function PrepNumberCandidateReview() {
  ...
  return <ReadAloudRateProvider>
    ...
    <AssessmentShell
      testType={formLabel} year="Prep" realmId="number"
      ...
      questionContent={<>
        ...
        <details className="mt-4 rounded-xl border border-teal-300/20 p-3 text-sm text-teal-100">
          ...
          <div className="mt-3 flex flex-wrap gap-3">
            ...
            <button type="button" onClick={()=>{setAnswers(a=>{const next={...a};delete next[item.id];return next;});setSubmitted(s=>({...s,[item.id]:false}));}} className="min-h-12 rounded-xl border border-teal-300/40 px-4">Reset question</button>
          </div>
      ...
      onSubmit={()=>setSubmitted(s=>({...s,[item.id]:true}))}
