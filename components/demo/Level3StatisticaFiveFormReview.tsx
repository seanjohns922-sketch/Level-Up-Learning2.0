...
export default function Level3StatisticaFiveFormReview() {
  ...
  const items = LEVEL3_STATISTICA_FORMS[form];
  ...
  const select = (nextForm: StatisticaForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/statistica-level3?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  ...
  return <main className={styles.page} style={{"--light":theme.chipText,"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={styles.header}><div><p>STATISTICA · LEVEL 3</p><h1>Level 3 assessments</h1></div><Link href="/demo-review?realm=statistics&year=Year%202">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="stats-question-heading" data-assessment-task-kind={`statisticaLevel3V1-${item.mode}`} data-question-id={item.id}>
