...
import { LEVEL2_STARPATH_FORMS } from "@/data/assessments/revisions/level2StarpathFiveForms";
import { emptyLevel2Response, level2ResponseReady, scoreLevel2Response, level2Expected, type Level2Response } from "@/lib/starpath-level2-review";
...
type RecordValue = { response: Level2Response; submitted: boolean; skipped: boolean };
export default function Level2StarpathFiveFormReview() {
  ...
  const items = LEVEL2_STARPATH_FORMS[form];
  ...
  const response = saved?.response ?? emptyLevel2Response(item);
  const theme = getRealmTheme("space");
  const select = (nextForm: GroundStarpathForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/starpath-level2?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  const update = (next:Level2Response) => {setRecords(old=>({...old,[item.id]:{response:next,submitted:level2ResponseReady(item,next),skipped:false}}));setFinished(old=>({...old,[form]:false}));};
  ...
  const correct = items.filter(q=>records[q.id]?.submitted&&!records[q.id].skipped&&scoreLevel2Response(q,records[q.id].response)).length;
  const expected = level2Expected(item);
  return <main className={styles.page} style={{"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={`${styles.header} ${styles.fullReviewHeader}`}><div><p>STARPATH · LEVEL 2</p><h1>Five redesigned assessments</h1></div><Link href="/demo-review?realm=space&year=Year%202">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="ground-question-heading" data-assessment-task-kind={`starpathLevel2V5-${item.task.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {GROUND_STARPATH_LABELS[form]}</p>
        <Level2StarpathAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
