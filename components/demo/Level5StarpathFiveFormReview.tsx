...
import { LEVEL5_STARPATH_FORMS } from "@/data/assessments/revisions/level5StarpathFiveForms";
import { emptyLevel5Response, level5ResponseReady, scoreLevel5Response, level5Expected, type Level5Response } from "@/lib/starpath-level5-review";
...
type RecordValue = { response: Level5Response; submitted: boolean; skipped: boolean };
export default function Level5StarpathFiveFormReview() {
  ...
  const items = LEVEL5_STARPATH_FORMS[form];
  ...
  const response = saved?.response ?? emptyLevel5Response(item);
  const theme = getRealmTheme("space");
  const select = (nextForm: GroundStarpathForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/starpath-level5?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  const update = (next:Level5Response) => {setRecords(old=>({...old,[item.id]:{response:next,submitted:level5ResponseReady(item,next),skipped:false}}));setFinished(old=>({...old,[form]:false}));};
  ...
  const correct = items.filter(q=>records[q.id]?.submitted&&!records[q.id].skipped&&scoreLevel5Response(q,records[q.id].response)).length;
  const expected = level5Expected(item);
  return <main className={styles.page} style={{"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={`${styles.header} ${styles.fullReviewHeader}`}><div><p>STARPATH · LEVEL 5</p><h1>Five redesigned assessments</h1></div><Link href="/demo-review?realm=space&year=Year%205">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="ground-question-heading" data-assessment-task-kind={`starpathLevel5V5-${item.task.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {GROUND_STARPATH_LABELS[form]}</p>
        <Level5StarpathAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
