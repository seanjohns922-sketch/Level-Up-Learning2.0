...
import { GROUND_STARPATH_REDESIGNED_FORMS } from "@/data/assessments/revisions/groundStarpathRedesignedForms";
import { emptyGroundResponse, groundResponseReady, scoreGroundResponse, type GroundResponse } from "@/lib/starpath-ground-redesign";
...
export default function GroundStarpathFiveFormReview() {
  ...
  const items = GROUND_STARPATH_REDESIGNED_FORMS[form];
  ...
  const expected = item.task.mode==="draw" ? `Any valid ${item.task.drawShape}.` : item.task.mode==="compose" ? "Two pieces joined into a square, in any orientation." : item.task.mode==="place" ? `Any space ${item.task.relation} the reference object.` : `${correctOptions}${item.task.reasons?` · ${item.task.reasons.find(r=>r.id===item.task.correctReason)?.label}`:""}`;
