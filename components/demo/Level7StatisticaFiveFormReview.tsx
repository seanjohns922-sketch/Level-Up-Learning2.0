...
import { LEVEL7_STATISTICA_FORMS } from "@/data/assessments/revisions/level7StatisticaFiveForms";
...
export default function Level7StatisticaFiveFormReview() {
  ...
  const items = LEVEL7_STATISTICA_FORMS[form];
  ...
  const select = (nextForm: StatisticaForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/statistica-level7?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  ...
  return <main className={styles.page} style={{"--light":theme.chipText,"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={styles.header}><div><p>STATISTICA · LEVEL 7</p><h1>Level 7 assessments</h1></div><Link href="/demo-review?realm=statistics&year=Year%207">Back to review</Link></header>
      <div className={styles.reviewBar}><div className={styles.formTabs} aria-label="Assessment forms">{STATISTICA_FORMS.map(f=><button key={f} aria-pressed={f===form} onClick={()=>select(f,index)}>{STATISTICA_FORM_LABELS[f]}</button>)}</div><select aria-label="Review question" value={index} onChange={e=>select(form,Number(e.target.value))}>{items.map((q,i)=><option key={q.id} value={i}>{i+1} — {q.skillLabel}</option>)}</select><span>Review only · 30 questions per form · Student results are not saved</span><span>{answered}/30 recorded</span></div>
      <div className="mb-4">
      ...
      <section className={styles.card} aria-labelledby="stats-question-heading" data-assessment-task-kind={`statisticaLevel7V1-${item.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 30 · {STATISTICA_FORM_LABELS[form]}</p>
      ...
      <details className={styles.notes} key={item.id}><summary>Review details</summary><p>{item.code} · {item.skillLabel} · Intended difficulty: {item.difficulty} · Curriculum group {item.week}</p><button onClick={()=>setShowAnswer(v=>!v)}>{showAnswer?"Hide answer":"Show answer"}</button>{showAnswer&&<p>Expected response: {expected}{saved?.submitted?` · Your answer: ${saved.skipped?"I don’t know":scoreStatsResponse(item,response)?"correct":"incorrect"}`:""}</p>}<p>All five forms use the same skill sequence. New Level 7 questions are isolated from existing student attempts while this bank is reviewed.</p></details>
      {finished[form]&&<div className={styles.notes} role="status">{STATISTICA_FORM_LABELS[form]} review: {correct}/30 correct · {answered}/30 recorded. Nothing was saved to a student record.</div>}
    </div>
  </main>;
}
