...
export default function PatternPeaksFiveFormReview({level,forms}:{level:3|4|5;forms:Record<PPForm,PPItem[]>}){
 ...
 const expected=(item.mode==='choice'||item.mode==='testerChoice')?item.options[item.correct]:item.mode==='product'?`Any two whole numbers from 2 to ${item.maxFactor} whose product is ${item.total}.`:item.mode==='filterBuilder'?`Two divisors whose lowest common multiple is ${item.targetMultiple}.`:item.mode==='equivalent'?`Two numbers from 1 to ${item.equivalence!.max} satisfying ${item.equivalence!.left} + first = ${item.equivalence!.right} + second.`:item.mode==='partition'?`Any two positive whole numbers totalling ${item.total}.`:item.mode==='algorithm'?`Any two steps that multiply each input by ${item.multiplier}.`:item.answers.join(', ');
 ...
 <div className={styles.activity}><div className={styles.source}><div className={styles.panelHeader}><h3>{item.visual.kind==='decision'?'Number pathway':item.visual.kind==='table'?'Number machine':'Look at the model'}</h3><ReadAloudBtn className={styles.voice} label='Read diagram' text={ppVisualSpeech(item.visual)}/></div><Visual v={item.visual}/>{item.mode==='testerChoice'&&<DivisibilityTester key={item.id} item={item} response={response} update={update}/>}</div>
 <div className={styles.answer}>
 {(item.mode==='choice'||item.mode==='testerChoice')?item.options.map((o,i)=><div className={styles.option} key={i}><button data-pp-option={i} aria-pressed={!response.skipped&&response.choice===i} onClick={()=>update({...ppEmpty(),tests:response.tests,choice:i})}>{o}</button><ReadAloudBtn className={styles.voice} kind='option' text={o}/></div>):(item.mode==='select'||item.mode==='filterBuilder')?<div className={pp.builder}>
 ...
 </div>:item.mode==='algorithm'?<div className={pp.builder}>
