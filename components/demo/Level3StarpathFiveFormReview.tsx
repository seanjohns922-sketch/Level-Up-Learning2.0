...
import { LEVEL3_STARPATH_FORMS } from "@/data/assessments/revisions/level3StarpathFiveForms";
import { emptyLevel3Response, level3ResponseReady, scoreLevel3Response, level3Expected, type Level3Response } from "@/lib/starpath-level3-review";
...
type RecordValue = { response: Level3Response; submitted: boolean; skipped: boolean };
export default function Level3StarpathFiveFormReview() {
  ...
  const items = LEVEL3_STARPATH_FORMS[form];
  ...
  const response = saved?.response ?? emptyLevel3Response(item);
  const theme = getRealmTheme("space");
  const select = (nextForm: GroundStarpathForm,nextIndex:number) => {stopSpeaking();setShowAnswer(false);router.replace(`/demo-review/starpath-level3?form=${nextForm}&question=${nextIndex+1}`,{scroll:false});};
  const update = (next:Level3Response) => {setRecords(old=>({...old,[item.id]:{response:next,submitted:level3ResponseReady(item,next),skipped:false}}));setFinished(old=>({...old,[form]:false}));};
  ...
  const correct = items.filter(q=>records[q.id]?.submitted&&!records[q.id].skipped&&scoreLevel3Response(q,records[q.id].response)).length;
  const expected = level3Expected(item);
  return <main className={styles.page} style={{"--accent":theme.accentText,"--selected":theme.ctaFrom,"--ring":theme.borderRing,"--cta":theme.ctaGradientCss,"--surface":theme.cardSurface,"--tint":theme.surfaceTint} as CSSProperties}>
    <div className={styles.shell}>
      <header className={`${styles.header} ${styles.fullReviewHeader}`}><div><p>STARPATH · LEVEL 3</p><h1>Five redesigned assessments</h1></div><Link href="/demo-review?realm=space&year=Year%203">Back to review</Link></header>
      ...
      <section className={styles.card} aria-labelledby="ground-question-heading" data-assessment-task-kind={`starpathLevel3V5-${item.task.mode}`} data-question-id={item.id}>
        <p className={styles.counter}>Question {index+1} of 20 · {GROUND_STARPATH_LABELS[form]}</p>
        <Level3StarpathAssessmentCard key={item.id} item={item} response={response} onChange={update}/>
