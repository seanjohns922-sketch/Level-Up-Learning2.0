...
export default function ChanceCompareToolsCard({ task, onCorrect, onWrong }: { task: Task; onCorrect: () => void; onWrong: (answer?: string) => void }) {
  ...
  return (
    <div className="chance-compare-task space-y-4">
      <div className="chance-compare-heading flex items-start gap-2">
      ...
      <div className="chance-compare-layout">
        {/* Tools side by side */}
        <div className="chance-compare-tools flex flex-wrap items-start justify-center gap-4 rounded-xl border border-[#e4d8f5] bg-[#faf7ff] p-4">
          {task.tools.map((tool, i) => (
            <div key={i} className="chance-compare-tool flex flex-col items-center gap-1">
        ...
        {/* Options */}
        <div className="chance-compare-options grid gap-2 sm:grid-cols-2">
