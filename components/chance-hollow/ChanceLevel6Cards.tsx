  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-hidden rounded-lg border-2 border-[#c89cff] bg-gradient-to-br from-[#25143a] via-[#322051] to-[#102b3e] p-5 text-white shadow-xl">
      ...
      <div className="px-2 pb-3 pt-6">
        <div className="mb-4 flex items-center justify-center gap-2 text-2xl font-black"><Sparkles className="text-cyan-300" /><span>{label(selected)}</span><OptionReadAloudButton text={label(selected)} /></div>
        ...
        <div className="mt-3 flex justify-between">{marks.map((value) => { const markLabel = task.displayMode === "decimal" ? label(value) : `${Math.round(value * 100)}%`; return <div key={value} className="flex flex-col items-center gap-1"><button type="button" onClick={() => setSelected(value)} className="text-[10px] font-black text-white/80 sm:text-xs">{markLabel}</button><OptionReadAloudButton text={markLabel} /></div>; })}</div>
      </div>
...
export function ChanceFormMatchCard({ task, onCorrect, onWrong }: CardProps<FormMatchTask>) {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-hidden rounded-lg border-2 border-[#c89cff] bg-gradient-to-br from-[#25143a] via-[#322051] to-[#102b3e] p-5 text-white shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{task.options.map((option, index) => <button key={index} type="button" onClick={() => toggle(index)} className={`flex items-center justify-between gap-2 rounded-lg border-2 p-4 text-xl font-black transition ${selected.has(index) ? "border-amber-300 bg-[#4b2861] shadow-[0_0_18px_rgba(217,70,239,.4)]" : "border-white/20 bg-white/5 hover:border-cyan-300"}`}><span>{option.label}</span><OptionReadAloudButton text={option.label} /></button>)}</div>
