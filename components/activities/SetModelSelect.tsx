    ...
    <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      {questionData.mode === "tap_fraction" ? (
        <div className="mt-6 rounded-2xl border border-sky-100 bg-sky-50 p-4">
          <CounterGrid total={questionData.totalObjects} selected={selected} interactive onToggle={toggle} />
          <button
            type="button"
      ...
      {questionData.mode === "pick_set" ? (
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {questionData.options?.map((option) => (
            ...
            >
              <CounterGrid total={questionData.totalObjects} highlightedCount={option.highlightedCount} />
            </button>
          ))}
          <button
            type="button"
      ...
      {questionData.mode === "complete_sentence" ? (
        <div className="mt-6 rounded-2xl border border-sky-100 bg-sky-50 p-4">
          <CounterGrid total={questionData.totalObjects} highlightedCount={questionData.highlightedIndices?.length ?? 0} />
