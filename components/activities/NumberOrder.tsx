...
export default function NumberOrder({
  ...
  const [selected, setSelected] = useState<string[]>(() =>
    isUnsupportedFractionOrder ? Array.from({ length: correctOrder.length }, () => "") : []
  );
  ...
  function selectValue(value: string) {
    ...
    if (isUnsupportedFractionOrder) {
      const emptyIndex = selected.findIndex((entry) => entry === "");
  ...
  function resetAll() {
    setSelected(isUnsupportedFractionOrder ? Array.from({ length: correctOrder.length }, () => "") : []);
    setCheckingComplete(false);
    setSubmitted(false);
    setSubmittedCorrect(null);
  }
  ...
  const availableCards = isUnsupportedFractionOrder
    ? cards.filter((value) => !selected.includes(value))
    : cards;

  function placeDraggedIntoSlot(slotIndex: number) {
    if (!draggedValue) return;
    const next = [...selected];
    const existingIndex = next.findIndex((value) => value === draggedValue);
    if (existingIndex >= 0) {
      next[existingIndex] = "";
    }
    const displaced = next[slotIndex];
    next[slotIndex] = draggedValue;
    if (displaced) {
      const firstEmpty = next.findIndex((value, index) => value === "" && index !== slotIndex);
      if (firstEmpty >= 0) {
        next[firstEmpty] = displaced;
      }
    }
    setSelected(next);
    setSubmitted(false);
  ...
  function submitUnsupportedOrder() {
    if (!isUnsupportedFractionOrder) return;
    ...
    const isCorrect = selected.every((value, index) => value === correctOrder[index]);
    setSubmitted(true);
      ...
      {isUnsupportedFractionOrder ? (
        <>
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            {selected.map((value, index) => (
              <div
                ...
                onDrop={() => placeDraggedIntoSlot(index)}
                className={[
                  "flex min-h-[96px] items-center justify-center rounded-2xl border-2 border-dashed bg-slate-50 px-4 py-5 transition-all",
                  submitted && submittedCorrect === false
                    ? "animate-[pulse_0.28s_ease-in-out_1] border-rose-300"
                    : "border-slate-200",
                ].join(" ")}
              >
                {value ? (
                  <div
                    draggable
                    onDragStart={() => setDraggedValue(value)}
                    className="flex w-full cursor-move items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 py-4 text-2xl font-black text-slate-900 shadow-sm"
                  >
                    {renderValue(value)}
                  </div>
                ) : (
                  <span className="text-sm font-bold uppercase tracking-[0.16em] text-slate-300">
                    Drop here
                  </span>
                )}
              </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {availableCards.map((value, idx) => (
              <button
                ...
                className="rounded-2xl border border-slate-200 bg-white p-6 text-xl font-black text-slate-900 shadow-sm transition-all duration-150 hover:-translate-y-[1px] hover:border-emerald-300 hover:bg-emerald-50"
              >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={submitUnsupportedOrder}
              className="rounded-xl bg-teal-600 px-5 py-3 text-sm font-black text-white hover:bg-teal-500"
            >
              Check order
            </button>
            <button
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {selected.length > 0 ? (
                ...
                <div className="text-sm font-bold text-teal-700">
                  {isFractionOrder ? "Place each fraction in order." : "Tap the cards in order."}
                </div>
              )}
            </div>
