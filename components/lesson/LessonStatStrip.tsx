...
import { BarChart3, CheckCircle, Compass, HelpCircle, Orbit, Sparkles, Target } from "lucide-react";
...
export function LessonStatStrip({
  ...
  const stats = [
    {
      label: isMeasurement || isStarpath ? "Challenges" : isStatistics ? "Data Tasks" : "Questions",
      value: questionsAnswered,
      icon: isMeasurement ? Compass : isStarpath ? Orbit : isStatistics ? BarChart3 : HelpCircle,
      iconColor: isMeasurement ? "text-amber-300" : isStarpath ? "text-violet-200" : isStatistics ? "text-[#f2bc45]" : "text-cyan-300",
      glow: isMeasurement ? "rgba(200,160,48,0.5)" : isStarpath ? "rgba(167,139,250,0.55)" : isStatistics ? "rgba(242,188,69,0.5)" : "rgba(34,211,238,0.45)",
    },
    {
      label: isMeasurement || isStarpath || isStatistics ? "Solved" : "Correct",
      value: correctAnswers,
      icon: CheckCircle,
      iconColor: isMeasurement ? "text-amber-200" : isStarpath ? "text-cyan-200" : isStatistics ? "text-[#8fbf7f]" : "text-emerald-300",
      glow: isMeasurement ? "rgba(232,200,120,0.55)" : isStarpath ? "rgba(103,232,249,0.55)" : isStatistics ? "rgba(143,191,127,0.5)" : "rgba(16,185,129,0.5)",
    },
    {
      label: isMeasurement ? "Mastery" : isStarpath ? "Mission Accuracy" : isStatistics ? "Data Accuracy" : "Accuracy",
      value: `${accuracy}%`,
      icon: isMeasurement || isStarpath || isStatistics ? Sparkles : Target,
      iconColor: isMeasurement ? "text-amber-200" : isStarpath ? "text-fuchsia-200" : isStatistics ? "text-[#f06b64]" : "text-teal-200",
      glow: isMeasurement ? "rgba(200,160,48,0.5)" : isStarpath ? "rgba(240,171,252,0.5)" : isStatistics ? "rgba(240,107,100,0.5)" : "rgba(94,234,212,0.5)",
    },
  ];
  ...
  const labelColor = isMeasurement ? "rgba(240,210,150,0.75)" : isStarpath ? "rgba(207,250,254,0.88)" : isStatistics ? "rgba(255,240,199,0.86)" : "rgba(94,234,212,0.8)";
      ...
      {stats.map((s) => (
        <div key={s.label} className="relative">
          ...
          <div
            ...
            style={{
              clipPath: isMeasurement || isStarpath || isStatistics
                ? undefined
                : "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)",
              borderRadius: isMeasurement || isStarpath || isStatistics ? 10 : undefined,
              background: bezelBg,
            }}
          />
          {/* Inner plate */}
          <div
            ...
            style={{
              ...
              borderRadius: isMeasurement || isStarpath || isStatistics ? 8 : undefined,
              background: plateBg,
