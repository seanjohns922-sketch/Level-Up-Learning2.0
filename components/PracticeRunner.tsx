...
function formatPracticeTopicLabel(kind: PracticeTask["kind"]) {
  ...
  if (kind === "starpathShapeClassify") return "Shape Families";
  if (kind === "starpathViewpoint") return "Objects and Views";
  if (kind === "starpathTurnMove") return "Direction Words";
  if (kind === "starpathRouteDebug") return "Fix the Route";
  if (kind === "starpathRouteBuild") return "Give the Route";
  if (kind === "starpathRouteRecord") return "Send the Directions";
  if (kind === "starpathShapeHunt") return "Shape Hunt";
  if (kind === "starpathShapeWorkshop") return "Shape Workshop";
  ...
  if (kind === "starpathMapCreate") return "Map Creator";
  if (kind === "starpathShapeFeature") return "Shape Features";
  if (kind === "starpathMapRoute") return "Map Route";
  ...
  if (kind === "chanceScalePortal") return "Probability Portal";
  if (kind === "chanceFormMatch") return "Match the Forms";
  if (kind === "chanceProbabilityForge") return "Probability Forge";
  if (kind === "chanceSimulationLab") return "Simulation Lab";
  if (kind === "chanceModelDebugger") return "Machine Diagnostic";
  if (kind === "chanceMasterTrial") return "Master Trial";
...
function getPracticeTaskCorrectAnswer(task: PracticeTask) {
  ...
  if (task.kind === "starpathShapeCompare") return task.answer;
...
export function PracticeRunner({
  ...
  const hasGroundFeedback =
    ...
    task.kind === "starpathShapeCompare" ||
