          ...
          <div className="grid grid-cols-3 lg:grid-cols-2 gap-2 mt-1">
            {plan.map((w) => {
              ...
              return (
                ...
                >
                  <div className={`text-[11px] font-extrabold uppercase tracking-wider ${active ? "text-stone-700" : "text-[#94A3B8]"}`}>
                    Week {w.week}
                  </div>
                  ...
                  <div className="mt-2 h-1.5 rounded-full bg-slate-100 overflow-hidden">
                    <div
                      className="h-full rounded-full bg-stone-1000 transition-all"
                      style={{ width: `${pct}%` }}
                    />
                  </div>
            ...
            {(week?.lessons ?? []).map((lsn) => {
              ...
              return (
                <button
                  ...
                  className="text-left bg-white rounded-2xl border border-[#E6E8EC] p-4 flex flex-col gap-3 hover:border-stone-400 hover:shadow-sm transition cursor-pointer"
                  title="Click to preview lesson content"
                >
                  <div className="flex items-center justify-between">
                    <span className="inline-flex items-center justify-center h-7 w-7 rounded-lg bg-stone-100 text-stone-700 text-xs font-black">
                      L{lsn.lesson}
                    </span>
                  ...
                  <span className="mt-auto w-full px-3 py-2 rounded-lg bg-stone-100 text-stone-700 text-xs font-extrabold text-center border border-stone-200">
                    Preview lesson →
                  </span>
                </button>
              );
...
function StatusPill({
  ...
  const map = {
    teal:  "bg-stone-100 text-stone-700 border-stone-200",
