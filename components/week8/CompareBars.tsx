  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-5">
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 grid gap-3">
        <div>
          <div className="text-xs font-bold text-slate-600 mb-2">Tom</div>
          <div className="h-6 rounded-lg bg-teal-400" style={{ width: `${(a / max) * 100}%` }} />
          <div className="text-xs text-slate-600 mt-1">{a}</div>
        </div>
        <div>
          <div className="text-xs font-bold text-slate-600 mb-2">Mia</div>
          <div className="h-6 rounded-lg bg-teal-200" style={{ width: `${(b / max) * 100}%` }} />
          <div className="text-xs text-slate-600 mt-1">{b}</div>
        </div>
        <div className="mt-2">
          <div className="text-xs font-bold text-slate-600 mb-2">Count by 1s</div>
          <div className="grid" style={{ gridTemplateColumns: `repeat(${max}, minmax(0, 1fr))`, gap: 4 }}>
            {Array.from({ length: max }).map((_, i) => (
              <div key={i} className="flex flex-col items-center">
                <div className="h-2 w-full rounded bg-slate-200" />
                <div className="mt-1 text-[10px] text-slate-600">{i + 1}</div>
              </div>
            ))}
          </div>
        </div>
