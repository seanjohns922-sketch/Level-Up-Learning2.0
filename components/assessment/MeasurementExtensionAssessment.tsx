...
const draftKey=(id:string,form:string,level:7|8)=>level===7?`measurement-extension-v4:${id}:${form}`:`measurement-extension-v4:${id}:8:${form}`;
...
export default function MeasurementExtensionAssessment({form,level=7}:{form:'pretest'|'posttest';level:7|8}){
 const router=useRouter();const year=`Year ${level}`;const questions=(level===8?YEAR8_MEASUREMENT_RELEASED_FORMS:YEAR7_MEASUREMENT_RELEASED_FORMS)[form];
 ...
 useEffect(()=>{let cancelled=false;async function load(){
  ...
  if(isDemoPreviewMode()){router.replace(`/demo-review/measurement-level${level}?form=${form}`);return;}
  if(!id){router.replace('/login');return;}
  const restored=await restoreStudentStateFromServer(id,'measurement');
  if(!['Year 6','Year 7','Year 8'].includes(restored.progress?.year??''))throw new Error('Extension assessments are available from Level 6 Measurement.');
  if(form==='posttest'){
   const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:id,p_realm_id:'measurement',p_working_level:year});
