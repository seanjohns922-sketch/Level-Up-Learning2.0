...
const draftKey=(id:string,form:string,level:7|8)=>level===7?`number-extension-v1:${id}:${form}`:`number-extension-v1:${id}:8:${form}`;
type Draft={answers:Record<string,string>;index:number;completionId:string;startedAt:string};
export default function NumberExtensionAssessment({form,level=7}:{form:'pretest'|'posttest';level:7|8}){
 const router=useRouter();const year=`Year ${level}`;const questions=(level===8?YEAR8_NUMBER_RELEASED_FORMS:YEAR7_NUMBER_RELEASED_FORMS)[form];
 ...
 useEffect(()=>{let cancelled=false;async function load(){
  ...
  if(isDemoPreviewMode()){router.replace(`/demo-review/number-level-${level}?form=${form}`);return;}
  if(!id){router.replace('/login');return;}
  const restored=await restoreStudentStateFromServer(id,'number');
  if(!['Year 6','Year 7','Year 8'].includes(restored.progress?.year??''))throw new Error('Extension assessments are available from Level 6 Number.');
  if(form==='posttest'){
   const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:id,p_realm_id:'number',p_working_level:year});
 ...
 if(done)return <main className="grid min-h-screen place-items-center bg-[#001b18] p-6 text-white"><div className="max-w-lg text-center"><h1 className="text-3xl font-black">Level {level} {form==='pretest'?'pre-test':'post-test'} saved</h1><p className="my-5">Your teacher can review your answers and progress.</p><button className="rounded-xl bg-teal-400 px-5 py-3 font-bold text-teal-950" onClick={()=>router.push('/world')}>Back to my world</button></div></main>;
