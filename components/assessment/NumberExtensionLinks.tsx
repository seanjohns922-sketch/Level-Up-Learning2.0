...
const eligible=()=>!isDemoPreviewMode()&&['Year 6','Year 7','Year 8'].includes(readProgress('number')?.year??'');
export default function NumberExtensionLinks(){
 const canOpen=useSyncExternalStore(subscribe,eligible,()=>false);
 if(!canOpen)return null;
 return <Link href="/number-extension" className="inline-flex min-h-11 shrink-0 items-center rounded-xl border border-teal-300/40 bg-teal-950 px-4 py-2 text-sm font-bold text-teal-50">Level 7–8 Number tests</Link>;
}
