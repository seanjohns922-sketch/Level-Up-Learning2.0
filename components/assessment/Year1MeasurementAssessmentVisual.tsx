...
export function measurement1Speech(v:Measurement1Visual){return [v.description,...v.labels.map((label,i)=>`${label==='?'?'Missing item':label}${v.values?.[i]!==undefined&&!['units','fair','string','balance'].includes(v.task)?`: ${v.values[i]} ${v.unit??''}`:''}`)].join('. ');}
 ...
 if(v.task==='units'||v.task==='fair'||v.task==='string'){
  ...
  content=<svg viewBox={`0 0 680 ${v.labels.length*110+20}`} className="mx-auto w-full max-w-3xl" role="img" aria-label={v.description}>{v.labels.map((label,i)=>{
   ...
   return <g key={label}><text x="5" y={y+25} fill="#3b2a14" fontSize="19" fontWeight="700">{label}</text>
    {v.task==='string'?<><path d={`M${x} ${y+22}H${x+objectWidth}`} stroke={i===0?'#b4793c':i===1?'#2c8794':'#9463b0'} strokeWidth={i===0?6:28} strokeLinecap={i===0?'round':'butt'}/>{i>0?<path d={`M${x} ${y+52}H${x+v.target!*step}`} stroke="#b4793c" strokeWidth="5" strokeDasharray="8 3"/>:null}</>:<>
    <rect x={x} y={y+4} width={v.task==='fair'?6*step:objectWidth} height="24" rx="4" fill={i%2?'#9463b0':'#2c8794'}/>
