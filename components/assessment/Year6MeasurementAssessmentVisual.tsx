...
export function measurement6Speech(v:Measurement6Visual){
 ...
 return [v.description,...v.labels??[],details,v.rows ? 'Timetable. All times use 24-hour time.' : '',...v.rows?.map(r=>`${r.service}. Departs ${measurement6Time(r.departs)}. Arrives ${measurement6Time(r.arrives)}.`)??[]].filter(Boolean).join(' ');
}
export default function Year6MeasurementAssessmentVisual({visual:v}:{visual:Measurement6Visual}){
 ...
 else if(v.task==='finish')content=<div className="flex flex-wrap justify-center gap-4">{v.labels!.map((label,i)=><div key={label} className="flex flex-col items-center rounded-xl border border-amber-200 bg-white p-5" style={{flex:'1 1 220px',maxWidth:310}}>{i===2?<img src="/images/measurelands/containers/lunchbox.png" alt="" style={{height:130,width:170,objectFit:'contain'}}/>:<TravelScene coach/>}<p className="text-center text-xl font-black text-amber-950">{label}</p></div>)}</div>;
 else content=<div className="mx-auto max-w-3xl"><div className="flex justify-center"><TravelScene/></div><div style={{overflowX:'auto'}}><table className="w-full border-collapse text-left text-amber-950" style={{minWidth:360}}><caption className="mb-3 font-bold">Timetable · 24-hour time</caption><thead><tr>{['Service','Departs','Arrives'].map(h=><th key={h} className="border border-amber-300 bg-amber-100 p-3">{h}</th>)}</tr></thead><tbody>{v.rows!.map(r=><tr key={r.service}>{[r.service,measurement6Time(r.departs),measurement6Time(r.arrives)].map((text,i)=><td key={i} className="border border-amber-200 bg-white p-3 font-bold tabular-nums">{text}</td>)}</tr>)}</tbody></table></div></div>;
