...
import {readyReleasedChance} from '@/lib/chance-release-response';
...
const draftKey=(id:string,form:string,level:7|8)=>level===7?`chance-extension-v1:${id}:${form}`:`chance-extension-v1:${id}:8:${form}`;
type Draft={answers:Record<string,string>;index:number;completionId:string;startedAt:string};
const theme=getRealmTheme('chance');
export default function ChanceExtensionAssessment({form,level=7}:{form:'pretest'|'posttest';level:7|8}){
 const router=useRouter();const year=`Year ${level}`;const questions=releasedChanceQuestions(year,form);
 ...
 useEffect(()=>{let cancelled=false;async function load(){
  ...
  if(isDemoPreviewMode()){router.replace(`/demo-review/chance-level${level}?form=${form}`);return;}
  if(!id){router.replace('/login');return;}
  const restored=await restoreStudentStateFromServer(id,'chance');
  if(!['Year 6','Year 7','Year 8'].includes(restored.progress?.year??''))throw new Error('Extension assessments are available from Level 6 Chance.');
  if(form==='posttest'){
   const {data,error}=await supabase.rpc('get_student_realm_assessments_secure',{p_student_id:id,p_realm_id:'chance',p_working_level:year});
 ...
 async function finish(next:Draft){
  if(saving||!studentId||questions.some(q=>!readyReleasedChance(q,next.answers[q.id])))return;
  ...
  try{
   const {error}=await supabase.rpc('complete_chance_extension_assessment',{p_student_id:studentId,p_assessment_type:form,p_completion_key:next.completionId,p_attempt:{working_level:year,correct_count:snapshots.filter(q=>q.correct).length,total_questions:questions.length,question_results:snapshots,placement_result:{replay_metadata:{started_at:next.startedAt,completed_at:completedAt}}}});
