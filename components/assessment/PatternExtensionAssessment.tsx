...
import {readyReleasedPattern} from '@/lib/pattern-release-response';
...
const draftKey=(id:string,form:string,level:7|8)=>level===7?`pattern-extension-v1:${id}:${form}`:`pattern-extension-v1:${id}:8:${form}`;
...
export default function PatternExtensionAssessment({form,level=7}:{form:'pretest'|'posttest';level:7|8}){
 const router=useRouter();const year=`Year ${level}`;const questions=releasedPatternQuestions(year,form);
 ...
 async function finish(next:Draft){
  if(saving||!studentId||questions.some(q=>!readyReleasedPattern(q,next.answers[q.id])))return;
