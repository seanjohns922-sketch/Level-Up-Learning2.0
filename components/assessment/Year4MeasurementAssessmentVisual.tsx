...
export default function Year4MeasurementAssessmentVisual({visual:v}:{visual:Measurement4Visual}){
 ...
 else if(v.task==='timeline'&&n.length===3)content=<div className="grid gap-4 sm:grid-cols-3">{v.labels!.map((label,i)=><div key={i} className="relative flex flex-col items-center justify-between gap-4 rounded-2xl border border-amber-200 bg-white p-5"><div className="flex h-36 w-full items-center justify-center" aria-hidden="true">{i===0?<ClockFace hour={Math.floor(n[0]/60)%12||12} minute={n[0]%60} size={140}/>:i===1?<svg viewBox="0 0 240 140" className="w-full max-w-60"><path d="M10 117H230" stroke="#d9c391" strokeWidth="4" strokeDasharray="10 8"/><rect x="25" y="25" width="185" height="78" rx="16" fill="#e6b853" stroke="#775421" strokeWidth="3"/><path d="M40 38H176Q195 38 195 53V70H40Z" fill="#cde8ed" stroke="#775421" strokeWidth="2"/><path d="M79 38V70M118 38V70M156 38V70" stroke="#775421" strokeWidth="3"/><path d="M164 79H194V102H164Z" fill="#ffe0a0" stroke="#775421" strokeWidth="2"/>{[62,175].map(x=><g key={x}><circle cx={x} cy="105" r="15" fill="#38434b"/><circle cx={x} cy="105" r="7" fill="#c5cdd1"/></g>)}<rect x="27" y="78" width="10" height="9" rx="3" fill="#fff1b7"/></svg>:<img src="/images/measurelands/containers/lunchbox.png" alt="" width={140} height={140} className="h-32 w-32 object-contain"/>}</div><p className="text-center text-xl font-black text-amber-950">{label}</p>{i<2?<ArrowRight aria-hidden="true" className="absolute z-10 hidden rounded-full bg-[#fffaf0] p-1 text-amber-700 sm:block" style={{right:-20,top:"50%",transform:"translateY(-50%)"}} size={36}/>:null}</div>)}</div>;
