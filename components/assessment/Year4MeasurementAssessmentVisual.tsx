...
export function measurement4Speech(v:Measurement4Visual){
 ...
 if(v.task==='scale')extra='Scale labels: zero to nine hundred grams, in steps of one hundred grams.';
...
function Perimeter({v}:{v:Measurement4Visual}){
 ...
 return <svg viewBox={`0 0 ${w*s+140} ${h*s+110}`} className="mx-auto max-h-80 w-full max-w-xl" role="img" aria-label={measurement4Speech(v)}><polygon points={points.map(([px,py])=>`${x+px*s},${y+py*s}`).join(' ')} fill="#d3e9bf" stroke="#638447" strokeWidth="4"/>{points.map(([px,py],i)=>{const[nx,ny]=points[(i+1)%points.length];const horizontal=py===ny;const distance=Math.abs(nx-px)+Math.abs(ny-py);return <text key={i} x={x+(px+nx)*s/2+(horizontal?0:20)} y={y+(py+ny)*s/2+(horizontal?-12:5)} textAnchor={horizontal?'middle':'start'} fontSize="19" fontWeight="bold" fill="#513323">{distance} m</text>;})}</svg>;
}
