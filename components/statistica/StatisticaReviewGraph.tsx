...
export default function StatisticaReviewGraph({categories,values,kind,order=categories.map((_,i)=>i),max=12,step=1,minorStep,axisLabel='Answers',valueLabel,numerical=false}:{categories:Category[];values:number[];kind:'pictures'|'columns';order?:number[];max?:number;step?:number;minorStep?:number;axisLabel?:string;valueLabel?:string;numerical?:boolean}){
 ...
 return <svg className={styles.reviewGraph} viewBox="0 0 360 290" role="img" aria-label={`${kind==='pictures'?'Picture':'Column'} graph. ${order.map(i=>`${categories[i].name}: ${values[i]}`).join('. ')}`}>
  ...
  {minorStep&&Array.from({length:Math.floor(max/minorStep)},(_,n)=>(n+1)*minorStep).filter(v=>v%step!==0).map(v=><line key={`minor-${v}`} x1="35" x2="350" y1={baseline-v*unit} y2={baseline-v*unit} stroke="currentColor" opacity=".12" strokeDasharray="2 3"/>)}
