...
export default function StatisticaReviewGraph({categories,values,kind,order=categories.map((_,i)=>i),max=12,min=0,step=1,minorStep,axisLabel='Answers',valueLabel,numerical=false}:{categories:Category[];values:number[];kind:'pictures'|'columns';order?:number[];max?:number;min?:number;step?:number;minorStep?:number;axisLabel?:string;valueLabel?:string;numerical?:boolean}){
 const baseline=234,unit=204/(max-min);
 return <svg className={styles.reviewGraph} viewBox="0 0 360 290" role="img" aria-label={`${kind==='pictures'?'Picture':'Column'} graph. Vertical scale starts at ${min}. ${order.map(i=>`${categories[i].name}: ${values[i]}`).join('. ')}`}>
  ...
  {order.map((i,k)=>{const c=categories[i],x=35+(k+.5)*315/order.length;return <g key={c.name} data-graph-category={c.name} data-graph-value={values[i]}>
   ...
   {min>0&&<text x={x} y={baseline-(values[i]-min)*unit-7} textAnchor="middle" fill="currentColor" fontSize="13" fontWeight="600">{values[i]}</text>}
