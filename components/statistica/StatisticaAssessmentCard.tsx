...
export default function StatisticaAssessmentCard({item,response,onChange}:{item:StatsItem;response:StatsResponse;onChange:(r:StatsResponse)=>void}){
 ...
 const sourceSpeech=item.source==='dual'?`${item.context}. Graph A uses pictures. One picture represents one answer. ${countSpeech}. Graph B uses columns, with a scale from zero to twelve in steps of one. ${ (item.secondaryOrder??[0,1,2,3]).map(i=>`${names[i]}: ${item.counts[i]}`).join('. ')}.`:item.source==='table'?`${item.context}. Frequency table. Frequency means how many answers. ${countSpeech}.`:item.source==='categories'?`Possible answers: ${names.join(', ')}.`:item.source==='graph'?`${item.context}. ${item.graphKind==='columns'?'Column graph. Scale from zero to twelve in steps of one.':'One picture stands for one answer.'} ${countSpeech}.`:`${item.context}. ${item.observations.map((v,i)=>`${observationLabel} ${i+1}: ${names[v]}`).join('. ')}.`;
 ...
 const table=(values:number[],editable=false)=><table className={styles.frequencyTable}><thead><tr><th scope="col">Group</th><th scope="col">Frequency</th></tr></thead><tbody>{values.map((n,i)=><tr key={i}><th scope="row"><span className={styles.category}><ObjectArt name={names[i]} color={item.categories[i].color} size={34}/>{names[i]}</span></th><td>{editable?controls(n,i):n}</td></tr>)}</tbody></table>;
 const graph=(values:number[],editable=false)=>{
  if(editable&&item.display==='frequency')return table(values,true);
  if(editable&&item.display==='columns')return <><ReviewGraph categories={item.categories} values={values} kind="columns"/><div className={styles.columnControls}>{values.map((n,i)=><div key={i}><strong>{names[i]}</strong>{controls(n,i)}</div>)}</div></>;
  if(!editable&&item.graphKind==='columns')return <ReviewGraph categories={item.categories} values={values} kind="columns"/>;
 ...
 return <>
  ...
  {item.mode==='sort'?<StatisticaSortActivity item={item} response={response} onChange={onChange}/>:<div className={`${styles.activity} ${item.source==='dual'?styles.wideActivity:''}`}>
   <div className={styles.source} data-stats-source><div className={styles.panelHeader}><h3>{item.source==='categories'?'Survey answers':item.context}</h3><ReadAloudBtn className={styles.voice} label="Read diagram" text={`${sourceSpeech} ${listSpeech}`}/></div>
    {item.source==='dual'?<div className={styles.twoGraphs}><div><h4>Graph A · Pictures</h4><ReviewGraph categories={item.categories} values={item.counts} kind="pictures"/></div><div><h4>Graph B · Columns</h4><ReviewGraph categories={item.categories} values={item.counts} kind="columns" order={item.secondaryOrder}/></div></div>:item.source==='table'?table(item.counts):item.source==='graph'?graph(item.counts):item.source==='categories'?<div className={styles.categoryChoices}>{item.categories.map(c=><div key={c.name}><ObjectArt name={c.name} color={c.color} size={80}/><span>{c.name}</span></div>)}</div>:<div className={styles.observations}>{item.observations.map((v,i)=><div key={i}><span>{observationLabel} {i+1}</span><ObjectArt name={names[v]} color={item.categories[v].color} size={58}/><strong>{names[v]}</strong></div>)}</div>}
