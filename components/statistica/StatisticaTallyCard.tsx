  ...
  return (
    <svg viewBox="0 0 34 34" className="h-9 w-9" aria-hidden="true">
      {Array.from({ length: Math.min(n, 4) }, (_, i) => (
        <line key={i} x1={5 + i * 7} y1={4} x2={5 + i * 7} y2={30} stroke="#67e8f9" strokeWidth={2.4} strokeLinecap="round" />
      ))}
      {n >= 5 ? <line x1={2} y1={30} x2={32} y2={4} stroke="#fcd34d" strokeWidth={2.6} strokeLinecap="round" /> : null}
    </svg>
  );
}
...
export default function StatisticaTallyCard({ task, onCorrect, onWrong }: { task: Task; onCorrect: () => void; onWrong: (answer?: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mx-auto flex min-h-[96px] max-w-md items-center justify-center rounded-3xl border border-cyan-300/25 bg-slate-950 p-4">
      ...
      {isRecord ? (
        <>
          <div className="flex items-center justify-center gap-3">
            <button type="button" onClick={() => !settled && setBuilt((v) => Math.max(0, v - 1))} disabled={settled} aria-label="remove a tally" className="grid h-11 w-11 place-items-center rounded-2xl border-2 border-white/15 bg-white/5 text-white/70 disabled:opacity-40"><Minus className="h-5 w-5" /></button>
            <div className="min-w-[3ch] text-center font-mono text-2xl font-black text-cyan-200 tabular-nums">{built}</div>
            <button type="button" onClick={() => !settled && setBuilt((v) => Math.min(20, v + 1))} disabled={settled} aria-label={`add a tally for ${task.label}`} className="grid h-11 w-14 place-items-center rounded-2xl border-2 border-cyan-300/40 bg-cyan-400/15 text-cyan-100 disabled:opacity-40"><Plus className="h-5 w-5" /></button>
          </div>
        ...
        <>
          <div className="mx-auto grid max-w-xs grid-cols-3 gap-2">
            {task.options?.map((option) => (
              <button key={option.id} type="button" onClick={() => !settled && setChosen(option.id)} className={["rounded-2xl border-2 p-3 text-center text-lg font-black transition", chosen === option.id ? "border-cyan-500 bg-cyan-50 text-teal-950 ring-2 ring-cyan-300" : "border-teal-200 bg-white text-teal-900 hover:border-cyan-400"].join(" ")}>{option.label}</button>
            ))}
          </div>
