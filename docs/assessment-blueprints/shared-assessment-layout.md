...
Number Nexus uses the shared `needsFullWidthAssessment` policy for number lines, fraction strips/comparisons, place-value charts, sequences, ordering tasks, algorithms and multi-panel contextual information. Number line interaction, tick marks and pointer geometry are unchanged. On wide screens, short numeric responses and skip controls sit in one row underneath full-width evidence; smaller screens stack them. The Number workspace retains teal surfaces and selection states.
