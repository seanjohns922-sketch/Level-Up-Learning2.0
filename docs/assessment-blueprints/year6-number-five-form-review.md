...
Reviewed against the supplied Australian Curriculum v9 F–6 Mathematics PDF, Year 6 Number (PDF pages 61–64), using the existing Level 6 post-test as the benchmark. All five forms sample the same nine Number descriptor codes and use the same skills and response formats in corresponding slots. Examples differ.

| Questions | Code | Shared demand |
| --- | --- | --- |
...
| 13–15 | AC9M6N07 | Find a quarter; calculate a 20% discounted price; calculate empty capacity from a tank's percentage full. |
| 16–17 | AC9M6N08 | Estimate a percentage amount using explicitly specified rounding; estimate a quarter-price discount. |
| 18–20 | AC9M6N09 | Model a decimal budget, compare exact whole-pack purchases, and maximise an affordable discounted purchase. |

The test includes accessible entry questions alongside fraction comparison, number-property and multi-step financial challenges. Intended difficulty is held by response format, number size, denominator relationships, calculation steps, visual support and language. These are author judgements, not statistically calibrated equivalent forms. In particular, familiarity with different prime factors can affect Q4 difficulty; the owner should check that challenge carefully.

## Audit refinements

- Coordinate reading replaces reflection: the target Number descriptor requires Cartesian coordinates, while reflection introduces a separate transformation skill. Four answer choices check sign/axis understanding without requiring comma entry on a numeric keyboard.
...
- A proportional tank illustration shows percentage full and total capacity, without displaying the required empty amount.

## Visuals and usability

Number Nexus styling, large mathematical notation, no shaded hints on fraction-ordering cards, a labelled Cartesian SVG, tank diagram, pack illustrations and clear purchase cards. Existing numbered question navigation and Back to Review behaviour are reused. Weekly lesson content and student progression are unchanged.
