"use client";
import {chanceReleaseVisual,readyReleasedChance} from "@/lib/chance-release-response";
import {patternReleaseVisual,readyReleasedPattern} from "@/lib/pattern-release-response";
import {statisticaReleaseVisual,readyReleasedStatistica} from "@/lib/statistica-release-response";
import {savedChanceReleaseVersion} from "@/lib/chance-release-version";
import {savedPatternReleaseVersion} from "@/lib/pattern-release-version";
import {savedStatisticaReleaseVersion} from "@/lib/statistica-release-version";
...
import ChanceExtensionAssessment from "@/components/assessment/ChanceExtensionAssessment";
import PatternExtensionAssessment from "@/components/assessment/PatternExtensionAssessment";
import StatisticsExtensionAssessment from "@/components/assessment/StatisticsExtensionAssessment";
...
import { savedYear6MeasurementVersion } from "@/lib/year6-measurement-assessment-version";
...
import { savedYear6NumberVersion } from "@/lib/year6-number-assessment-version";
...
function AssessmentRoute() {
  ...
  if (["Year 7","Year 8"].includes(params.get("year")??"") && params.get("realm_id")==="statistics") return <StatisticsExtensionAssessment key={`pretest-${params.get("year")}`} level={params.get("year")==="Year 8"?8:7} form="pretest"/>;
  if (["Year 7","Year 8"].includes(params.get("year")??"") && params.get("realm_id")==="chance") return <ChanceExtensionAssessment key={`pretest-${params.get("year")}`} level={params.get("year")==="Year 8"?8:7} form="pretest"/>;
  if (["Year 7","Year 8"].includes(params.get("year")??"") && params.get("realm_id")==="pattern") return <PatternExtensionAssessment key={`pretest-${params.get("year")}`} level={params.get("year")==="Year 8"?8:7} form="pretest"/>;
...
function PretestPage() {
  ...
  const [chanceReleaseVersion,setChanceReleaseVersion]=useState<0|1>(1);
  const [patternReleaseVersion,setPatternReleaseVersion]=useState<0|1>(1);
  const [statisticsReleaseVersion,setStatisticsReleaseVersion]=useState<0|1>(1);
  ...
  useEffect(() => {
    ...
    const level6Version = progressRealmId === "number" && year === "Year 6" && !isDemoPreviewMode() ? savedYear6NumberVersion(snapshot?.questionIds) ?? 3 : 3;
    ...
    const measurement1Version=progressRealmId==="measurement" && year==="Year 1" && !isDemoPreviewMode() && snapshot?.questionIds?.length ? savedYear1MeasurementVersion(snapshot.questionIds)??3 : 4;
    ...
    const measurement6Version=progressRealmId==="measurement" && year==="Year 6" && !isDemoPreviewMode() && snapshot?.questionIds?.length ? savedYear6MeasurementVersion(snapshot.questionIds)??3 : 4;
    ...
    const statsVersion=progressRealmId==="statistics"&&!isDemoPreviewMode()?savedStatisticaReleaseVersion(snapshot?.questionIds)??1:1;
    setStatisticsReleaseVersion(statsVersion);
    setChanceReleaseVersion(progressRealmId==="chance"&&!isDemoPreviewMode()?savedChanceReleaseVersion(snapshot?.questionIds)??1:1);
    setPatternReleaseVersion(progressRealmId==="pattern"&&!isDemoPreviewMode()?savedPatternReleaseVersion(snapshot?.questionIds)??1:1);
    const resumeQuestions = getPretestForYearLabel(year, progressRealmId, version,groundVersion,level2Version,level4Version,level5Version,level6Version,level3Version,snapshot?.questionIds?.length ? measurementVersion : 4,measurement1Version,measurement2Version,measurement5Version,measurement6Version,fullMeasurementVersion,spaceVersion,statsVersion);
  ...
  function restartAssessment() {
    ...
    setYear6MeasurementVersion(4);
    setMeasurementReleaseVersion(1);
  ...
  const isReady = question && chanceReleaseVisual(question) ? readyReleasedChance(question,selected) : question && patternReleaseVisual(question) ? readyReleasedPattern(question,selected) : question && statisticaReleaseVisual(question) ? readyReleasedStatistica(question,selected) : question && starpathReleaseVisual(question) ? readyReleasedStarpath(question,selected) : question?.type === "prepNumberTask" ? groundNumberHasAnswer(question,selected) :
    question?.type === "mab"
      ? mabHasSelection
    ...
    <>
      ...
      <ReadAloudRateProvider rate={realmId === "space" ? 0.85 : undefined}>
        <AssessmentShell
          ...
          onLogout={exitLogout}
          wideContent={isMeasurelandsTask || (!!chanceReleaseVisual(question)||!!patternReleaseVisual(question)||!!starpathReleaseVisual(question)||!!statisticaReleaseVisual(question))}
          hidePrompt={isMeasurelandsTask || (!!chanceReleaseVisual(question)||!!patternReleaseVisual(question)||!!starpathReleaseVisual(question)||!!statisticaReleaseVisual(question))}
          lightSurface={isMeasurelandsTask}
          answeredFlags={questions.map((q,i) => chanceReleaseVisual(q) ? readyReleasedChance(q,answers[i]) : patternReleaseVisual(q) ? readyReleasedPattern(q,answers[i]) : statisticaReleaseVisual(q) ? readyReleasedStatistica(q,answers[i]) : starpathReleaseVisual(q) ? readyReleasedStarpath(q,answers[i]) : q.type === "prepNumberTask" ? groundNumberHasAnswer(q,answers[i]) : answers[i] !== null && answers[i] !== undefined && answers[i] !== "")}
          onJump={(i) => setIndex(i)}
          realmId={realmId}
        />
