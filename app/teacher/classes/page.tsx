...
import { Archive, Check, Copy, Eye, EyeOff, FileUp, MoreHorizontal, Search, Trash2, UserPlus, Users, X } from "lucide-react";
...
export default function TeacherClassesPage() {
  ...
  const [directoryClass, setDirectoryClass] = useState<ClassRow | null>(null);
  const [directoryStudents, setDirectoryStudents] = useState<SchoolDirectoryStudent[]>([]);
  const [directoryQuery, setDirectoryQuery] = useState("");
  const [directoryYear, setDirectoryYear] = useState("");
  ...
  async function openSchoolDirectory(classRow: ClassRow) {
    if (!classRow.school_id) return;
    setDirectoryClass(classRow);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Student directory could not be loaded");
      }
      setDirectoryStudents(payload.students ?? []);
    } catch (error) {
      setDirectoryError(
        error instanceof Error ? error.message : "Student directory could not be loaded",
      );
    } finally {
  ...
  async function assignDirectoryStudents() {
    if (!directoryClass?.school_id || selectedDirectoryIds.size === 0) return;
    setDirectoryLoading(true);
    setDirectoryError("");
    try {
      const accessToken = await getTeacherAccessToken();
      const response = await fetch(
        `/api/school/${directoryClass.school_id}/command`,
        {
          ...
          body: JSON.stringify({
            ...
            studentIds: [...selectedDirectoryIds],
            idempotencyKey: crypto.randomUUID(),
          }),
        },
      );
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? "Students could not be added");
      }
      if (payload.errors?.length) {
        throw new Error(payload.errors.map((error) => error.message).join("; "));
      }
      setDirectoryClass(null);
      if (authUser) await loadClasses(authUser.id);
    } catch (error) {
      setDirectoryError(
        error instanceof Error ? error.message : "Students could not be added",
      );
    } finally {
      setDirectoryLoading(false);
    }
          ...
          >
            <div className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl">
              <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
                <div>
                  <h2 id="school-directory-title" className="text-xl font-bold text-slate-950">
                    Add existing students
                  </h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Choose students from the {directoryClass.school_name ?? "school"} directory for {directoryClass.name}.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setDirectoryClass(null)}
                  className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 text-slate-500"
                  aria-label="Close student directory"
                >
                  <X size={18} />
                </button>
              </div>
                ...
                <div className="flex overflow-x-auto">
                  {[{ value: "", label: "All" }, ...YEAR_LEVELS.map((year, index) => ({
                    ...
                    <button
                      key={year.value || "all"}
                      type="button"
                      onClick={() => setDirectoryYear(year.value)}
                      className={`min-w-14 border-b-2 px-4 py-3 text-sm font-bold ${
                        directoryYear === year.value
              ...
              <div className="min-h-64 flex-1 overflow-y-auto p-6">
                {directoryLoading && directoryStudents.length === 0 ? (
                  <p className="py-12 text-center text-sm text-slate-500">Loading school students...</p>
                ) : directoryError ? (
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {directoryStudents
                      ...
                      .filter((student) => !(student.classIds ?? []).includes(directoryClass.id))
                      .filter((student) => !directoryYear || student.yearLevel === directoryYear)
                      .filter((student) => {
                      ...
                      .map((student) => {
                        const selected = selectedDirectoryIds.has(student.id);
                        return (
                          <button
                            key={student.id}
                            type="button"
                            ...
                            className={`flex min-h-16 items-center justify-between rounded-md border px-4 py-3 text-left ${
                              selected
                          ...
                          >
                            <span>
                              <span className="block text-sm font-bold text-slate-950">{student.name}</span>
                              <span className="block text-xs text-slate-500">
                                {student.yearLevel ?? "Year not set"}
                                {student.classes.length ? ` · ${student.classes.join(", ")}` : " · Unassigned"}
                              </span>
                            </span>
              ...
              <div className="flex items-center justify-between border-t border-slate-200 px-6 py-4">
                <p className="text-sm font-semibold text-slate-600">
                  {selectedDirectoryIds.size} selected
                </p>
                <div className="flex gap-2">
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    disabled={directoryLoading || selectedDirectoryIds.size === 0}
                    onClick={() => void assignDirectoryStudents()}
                    className="rounded-md bg-emerald-700 px-4 py-2 text-sm font-bold text-white disabled:opacity-50"
                  >
                    {directoryLoading ? "Adding..." : `Add ${selectedDirectoryIds.size} students`}
                  </button>
                </div>
              </div>
            </div>
