...
const rows = GROUND_MEASURELANDS_INDEPENDENT_POSTTEST_ITEMS.map((item) => [item.id, "Post-Test", item.primaryDescriptorCode, item.prompt, responseLabels[item.responseMode] ?? label(item.responseMode), label(item.difficulty), label(item.cognitiveCategory), item.misconceptionTags.join(", "), item.correctAnswer, "", "", "", "", ""]);
if (rows.length !== 20 || new Set(GROUND_MEASURELANDS_INDEPENDENT_POSTTEST_ITEMS.map((item) => item.id)).size !== 20) throw new Error("Ground review sheet requires 20 unique items.");
if (rows.some((row) => row.length !== HEADERS.length)) throw new Error("Ground review rows do not match the approved columns.");
const outputPath = path.join(process.cwd(), "docs/measurelands/ground-educator-review-sheet.csv");
