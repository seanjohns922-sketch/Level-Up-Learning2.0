...
import { YEAR5_NUMBER_NEXUS_INDEPENDENT_POSTTEST_ITEMS, YEAR5_NUMBER_NEXUS_INDEPENDENT_PRETEST_ITEMS } from "../data/assessments/year5NumberNexusIndependentBanks";
...
const items = [...YEAR5_NUMBER_NEXUS_INDEPENDENT_PRETEST_ITEMS, ...YEAR5_NUMBER_NEXUS_INDEPENDENT_POSTTEST_ITEMS];
...
if (rows.length !== 40 || new Set(items.map((item) => item.id)).size !== 40) throw new Error("Year 5 review sheet requires 40 unique items.");
if (rows.some((row) => row.length !== HEADERS.length)) throw new Error("Year 5 review rows do not match the approved columns.");
const outputPath = path.join(process.cwd(), "docs/number-nexus/year-5-educator-review-sheet.csv");
