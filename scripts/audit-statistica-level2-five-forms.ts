...
for(const form of STATISTICA_FORMS){
 ...
 for(const q of bank){
  ...
  assert(q.counts.every(n=>Number.isInteger(n)&&n>=0&&n<=12));
  ...
  if(q.slot===4){assert.equal(q.observations.length,10);assert.equal(q.sourceLabel,'Draw');assert.equal(q.display,'tally');}
  ...
  if(q.slot===16)assert.equal(Number(label),Math.max(...q.counts)-q.counts.find(n=>n===5)!);
  ...
  if(q.slot===20){const most=q.categories[q.counts.indexOf(Math.max(...q.counts))].name,least=q.categories[q.counts.indexOf(Math.min(...q.counts))].name;assert.equal(label,`${most} was chosen more than ${least}.`);}
 }
}
for(let i=0;i<20;i++){
