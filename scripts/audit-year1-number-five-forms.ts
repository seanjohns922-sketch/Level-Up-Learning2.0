...
import { NUMBER_LEVEL1_FIVE_FORMS as forms, NUMBER_LEVEL1_FORMS as formNames } from "../data/assessments/revisions/year1NumberFiveForms";
...
const keys = {
  pretest:["58","42","30","15","13","4","12","star","109","5","35","25 groups of 5","8","13","7","30","star||star","96||108||120","No. Coin values matter.","Yes"],
  posttest:["73","64","25","16","11","5","10","robot","119","7","34","24 groups of 5","9","11","6","25","robot||robot","98||109||120","No. Coin values matter.","Yes"],
  start:["62","56","20","14","13","6","14","crystal","129","6","36","26 groups of 5","8","12","5","35","crystal||crystal","95||107||120","No. Coin values matter.","Yes"],
  mid:["85","79","35","15","13","3","16","star","139","5","33","27 groups of 5","7","13","4","40","star||star","94||106||120","No. Coin values matter.","Yes"],
  end:["47","83","15","13","14","7","18","robot","149","4","37","28 groups of 5","8","14","8","45","robot||robot","93||105||120","No. Coin values matter.","Yes"],
};
type Visual = { type:string; tens:number; ones:number; whole:number|null; parts:(number|null)[]; groups:number[]|number; groupSize:number; total:number; start:number; change:number; token:string; values:(number|null)[]; expression:string; sequence:string[]; answerSlots:number; amounts:number[]; choices:{count:number;size:number}[]; totals:number[] };
...
for(const form of formNames) {
  ...
  items.forEach((item,i)=>{
    ...
    assert.equal(item.type,benchmark[i].type);
    assert.equal(item.responseMode,benchmark[i].responseMode);
    assert.equal(item.difficulty,forms.posttest[i].difficulty);
    assert.equal(item.scoring.kind,benchmark[i].scoring.kind);
    ...
    assert.equal(v.type,(benchmark[i].visual as Visual).type);
    if(item.type==="mcq") {
    ...
    switch(i) {
      ...
      case 3: { const ns=numbers(item.prompt);assert.deepEqual(ns,numbers(v.expression));assert.ok(ns.every(n=>n<10));calculated=sum(ns);range(calculated,11,20);break; }
      case 4: assert.equal(v.token,"seedling");assert.deepEqual(numbers(item.prompt),[v.start,v.change]);assert.ok(v.start<10&&v.change<10);calculated=v.start+v.change;range(calculated,11,20);break;
      ...
      case 12: { const [a,b]=numbers(item.prompt);assert.deepEqual(numbers(v.expression),[a,b]);range(a,14,18);range(b,6,9);assert.ok(a%10<b,"Subtraction bridges ten in every form");calculated=a-b;break; }
      case 13: assert.deepEqual(numbers(item.prompt),v.amounts);assert.ok(v.amounts.every(n=>n<10));calculated=sum(v.amounts);range(calculated,11,14);break;
      ...
      case 18: { const groups=(item.visual as {groups:number[][]}).groups;assert.ok(groups[0].length>groups[1].length);assert.ok(groups[0].every(n=>n===1)&&groups[1].every(n=>n===2));assert.ok(sum(groups[0])<sum(groups[1]));calculated="No. Coin values matter.";break; }
      case 19: { const groups=v.groups as number[];assert.equal(groups.length,2);assert.equal(groups[0],groups[1]);range(groups[0],3,7);calculated="Yes";break; }
...
// Keep the owner's original post-test questions, not the simplified v3 variants.
...
console.log(`Number Level 1: 100 independent answer/visual checks, five distinct examples per slot, post-test benchmark, matched demand, scoring and protected review isolation pass. Counting workload: ${JSON.stringify(workload)}.`);
