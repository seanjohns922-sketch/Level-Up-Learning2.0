...
for(const [form,items]of Object.entries(forms)){
 ...
 for(const [i,q]of items.entries()){
  ...
  if(q.kind==='composite'){
   ...
   if(t.mode==='choice'){a.selected=[t.correctId!];assert.equal(t.correctId==='o0',independentSym(t,t.seeds));assert(!scoreLevel4Response(q,{...a,selected:[t.options!.find(o=>o.id!==t.correctId)!.id]}));}
