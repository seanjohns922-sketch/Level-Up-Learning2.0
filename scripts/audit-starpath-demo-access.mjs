...
assert.match(carousel, /restoreStudentStateFromServer\(identity\.studentId, availability\.progressRealmId\)/);
...
assert.doesNotMatch(serverPage, /getServerStarpathAccess/);
assert.match(starpathMap, /RealmDashboardShell/);
assert.doesNotMatch(starpathMap, /only: true/);
assert.match(starpathMap, /storageRealmId: "space"/);
assert.doesNotMatch(lessonPage, /getServerStarpathAccess/);
...
console.log("Starpath demo access audit passed: demo review remains isolated while production Starpath is live and canonical.");
