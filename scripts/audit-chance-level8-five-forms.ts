...
for(const form of CH_FORMS){const items=LEVEL8_CHANCE_FORMS[form];assert.equal(items.length,30);assert.deepEqual([...new Set(items.map(q=>q.code))].sort(),['AC9M8P01','AC9M8P02','AC9M8P03']);
 ...
 for(const q of items){assert(!ids.has(q.id));ids.add(q.id);assert(chSpeech(q));assert(!chReady(q,chEmpty()));assert(!chScore(q,{...chEmpty(),skipped:true}));assert(q.apparatus||q.displays||q.rows||q.tiles||q.scene||q.tree||q.venn||q.matrix,`${q.id}: missing visual`);
 ...
 if(q.venn){assert.equal(q.venn.counts.reduce((a,b)=>a+b,0),40);for(const label of q.venn.labels)assert(chSpeech(q).includes(label));}
 ...
 const [onlyA,both,onlyB,neither]=items[12].venn!.counts;close(num(13),both/40);close(num(14),(onlyA+both+onlyB)/40);close(num(15),(onlyA+onlyB)/40);close(num(16),neither/40);
