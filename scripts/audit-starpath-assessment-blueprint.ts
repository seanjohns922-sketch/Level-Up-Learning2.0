...
assert.equal(STARPATH_ASSESSMENT_BLUEPRINTS.reduce((sum, blueprint) => sum + blueprint.forms.length, 0), 14);
...
for (const blueprint of STARPATH_ASSESSMENT_BLUEPRINTS) {
  ...
  assert.equal(blueprint.forms.length, 2);
  ...
  for (const item of blueprint.descriptors) {
    ...
    assert.equal(item.curriculumMapping.implementationStatus, [2, 3, 4, 5, 6].includes(blueprint.level) ? "aligned" : "mapped-unverified");
  }
}
...
console.table(rows);
console.log(`Starpath assessment blueprint audit passed: 7 levels, 14 forms, 17 ACv9 Space descriptors and ${STARPATH_MISCONCEPTION_LIBRARY.length} canonical misconceptions.`);
