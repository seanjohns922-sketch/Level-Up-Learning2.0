...
const rows = [
  ...LEVEL3_STARPATH_INDEPENDENT_PRETEST_ITEMS.map((item) => [item, "Pre-Test"] as const),
  ...LEVEL3_STARPATH_INDEPENDENT_POSTTEST_ITEMS.map((item) => [item, "Post-Test"] as const),
].map(([item, form]) => [item.id, form, item.primaryDescriptorCode, item.prompt, label(item.responseMode), label(item.difficulty), label(item.cognitiveCategory), item.misconceptionTags.join(", "), "Validated interaction", "", "", "", "", ""]);

if (rows.length !== 40 || rows.some((row) => row.length !== HEADERS.length)) throw new Error("Year 3 Starpath review sheet requires 40 complete rows.");
const output = path.join(process.cwd(), "docs/starpath/level3-educator-review-sheet.csv");
