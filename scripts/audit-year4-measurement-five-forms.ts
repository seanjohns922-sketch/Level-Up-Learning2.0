...
import {toMeasurelandsTimeResponse,fromMeasurelandsTimeResponse} from '../data/assessments/measurelandsPresentation';
for(const bank of Object.values(forms)){
 ...
 bank.forEach((q,i)=>{
  ...
  switch(v.task){
   ...
   case 'area':expected=String(v.halfCells?(n[0]-1)*n[1]+n[1]/2:n.length===4?n[0]*n[1]-n[2]*n[3]:n[0]*n[1]);break;
