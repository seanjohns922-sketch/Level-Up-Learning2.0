/* Statistica Level 1 production audit (6-week / 18-lesson structure). Validates
 * answer correctness, renderer safety, interaction variety and repeated-call
 * variation. Run: npx tsx scripts/audit-statistica-level1.ts
 */
...
const inRange = (v: number) => v >= 1 && v <= 20; // Year 1 with Codex's raised ranges
...
function auditTask(lessonId: string, task: PracticeTask) {
  ...
  switch (task.kind) {
    ...
    case "statisticaGraph": {
      check(task.categories.length >= 2 && task.categories.every((c) => inRange(c.count)), `${lessonId}: graph counts must be 1..20`);
      if (task.mode !== "build") {
    ...
    case "statisticaTable": {
      check(task.rows.length >= 2 && task.rows.every((r) => inRange(r.count)), `${lessonId}: table counts must be 1..20`);
...
for (const lessonId of STATISTICA_LEVEL1_LESSON_IDS) {
  ...
  coverage.push({ lesson: lessonId.replace("y1-statistics-", ""), teach: family(teaching), a1: families[0]!, a2: families[1]!, a3: families[2]! });
  ...
  sample.activities.forEach((_, slot) => {
    const vs = getStatisticaLevel1TaskSet(lessonId)!;
...
check(STATISTICA_LEVEL1_LESSON_IDS.length === 18, `Level 1 should have 18 lessons (6 weeks), found ${STATISTICA_LEVEL1_LESSON_IDS.length}`);
...
console.log(`Statistica Level 1 audit passed: 18 lessons, ${taskCount} generated tasks validated.`);
