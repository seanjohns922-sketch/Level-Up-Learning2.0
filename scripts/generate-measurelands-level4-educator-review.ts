...
const items = [...YEAR4_MEASURELANDS_INDEPENDENT_PRETEST_ITEMS, ...YEAR4_MEASURELANDS_INDEPENDENT_POSTTEST_ITEMS];
...
if (rows.length !== 40 || new Set(items.map((item) => item.id)).size !== 40) throw new Error("Level 4 review sheet requires 40 unique items.");
if (rows.some((row) => row.length !== HEADERS.length)) throw new Error("Level 4 review rows do not match the approved columns.");
const outputPath = path.join(process.cwd(), "docs/measurelands/level-4-educator-review-sheet.csv");
