...
const compare=['4.04||4.24||4.42||4.6','5.04||5.24||5.42||5.6','6.04||6.24||6.42||6.6','7.04||7.24||7.42||7.6','8.04||8.24||8.42||8.6'];
const inverse=['107 × 8 = 856','108 × 8 = 864','109 × 8 = 872','106 × 8 = 848','104 × 8 = 832'];
const sequences=['39','44','34','33','37'];
const numericKeys=[
 ['0.06','3','3','0.3','3','1.25','4200','4274','296','700','150','85','182','95','141','16'],
 ['0.08','3','15','0.6','7','2.25','5300','5274','376','800','200','85','210','109','131','20'],
 ['0.04','3','21','0.7','11','3.25','6400','6274','456','900','250','85','238','81','121','24'],
 ['0.07','3','9','0.4','15','4.25','7500','7274','536','600','300','85','266','67','111','28'],
 ['0.03','3','18','0.8','19','5.25','8600','8274','616','500','350','85','154','53','101','32'],
];
...
for(const [f,form] of names.entries()){
 ...
 const expected:Record<number,string>={1:compare[f],3:'Odd',11:inverse[f],19:sequences[f]};
 ...
 for(const q of bank.filter(q=>q.type==='mcq')) assert.equal(q.options![q.selectedAnswerPosition!-1],q.correctAnswer);
 const equivalent=bank[4].visual as {left:number[];right:number[]};assert.equal(equivalent.right[1]/equivalent.left[1],3);
 const decimal=bank[5].visual as {numerator:number;denominator:number};assert.equal(decimal.denominator,10);assert.ok(decimal.numerator>0&&decimal.numerator<10);
