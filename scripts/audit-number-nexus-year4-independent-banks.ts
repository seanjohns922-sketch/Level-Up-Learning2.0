...
const blueprint = NUMBER_NEXUS_ASSESSMENT_BLUEPRINTS.find((entry) => entry.level === 4);
check(Boolean(blueprint), "Year 4 blueprint is missing.");
check(blueprint?.descriptors.every((entry) => entry.curriculumMapping.implementationStatus === "aligned") === true, "Year 4 curriculum is not fully aligned.");
...
for (let week = 1; week <= 12; week += 1) buildYear4NumberNexusWeeklyQuiz(week).forEach((question) => weeklyPrompts.add(question.prompt.trim().toLowerCase()));

const allItems = forms.flatMap((form) => form.bank) as Candidate[];
check(allItems.length === 40, "Year 4 banks must contain 40 items.");
...
for (const form of forms) {
  ...
  check(bank.every((item) => item.prompt.trim().split(/\s+/).length <= 16), `${form.kind} exceeds the Year 4 reading ceiling.`);
  ...
  const resultAt85 = analyzeAssessmentResult({ questions: replayQuestions, answers: answersAt85, yearLevel: 4, testType: form.kind === "pretest" ? "pre" : "post", passThreshold: threshold });
  const resultAt80 = analyzeAssessmentResult({ questions: replayQuestions, answers: answersAt80, yearLevel: 4, testType: form.kind === "pretest" ? "pre" : "post", passThreshold: threshold });
...
const bankSource = fs.readFileSync(path.join(process.cwd(), "data/assessments/year4NumberNexusIndependentBanks.ts"), "utf8");
...
const visualSource = fs.readFileSync(path.join(process.cwd(), "components/assessment/NumberNexusYear4AssessmentVisual.tsx"), "utf8");
...
if (failures.length) {
  console.error(`Year 4 Number Nexus bank audit failed: ${failures.length} failure(s), ${passed} checks passed.`);
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}
console.log(`Year 4 Number Nexus production bank audit passed: 40/40 items across 2 forms; ${passed} validation checks passed.`);
