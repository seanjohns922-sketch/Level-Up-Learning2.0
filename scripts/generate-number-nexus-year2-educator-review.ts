...
import { YEAR2_NUMBER_NEXUS_INDEPENDENT_POSTTEST_ITEMS, YEAR2_NUMBER_NEXUS_INDEPENDENT_PRETEST_ITEMS } from "../data/assessments/year2NumberNexusIndependentBanks";
...
const items = [...YEAR2_NUMBER_NEXUS_INDEPENDENT_PRETEST_ITEMS, ...YEAR2_NUMBER_NEXUS_INDEPENDENT_POSTTEST_ITEMS];
...
if (rows.length !== 40 || new Set(items.map((item) => item.id)).size !== 40) throw new Error("Year 2 review sheet requires 40 unique items.");
if (rows.some((row) => row.length !== HEADERS.length)) throw new Error("Year 2 review rows do not match the approved columns.");
const outputPath = path.join(process.cwd(), "docs/number-nexus/year-2-educator-review-sheet.csv");
