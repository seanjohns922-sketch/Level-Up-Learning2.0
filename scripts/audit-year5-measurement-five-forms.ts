...
for(const bank of Object.values(forms)){
 ...
 bank.forEach((q,i)=>{
  ...
  if(v.task==='precision')expected=['Millimetres','Centimetres','Centimetres','Metres','Millimetres'][n[0]];
  ...
  if(v.task==='combinedArea')expected=String(n[2]*n[3]+n[0]*n[1]);
  if(v.task==='rectangleArea')expected=String(n[0]*n[1]);
  if(v.task==='compositeArea'){assert.ok(n[2]<n[0]&&n[3]<n[1]);expected=String((n[0]-n[2])*n[3]+n[0]*(n[1]-n[3]));}
...
console.log('Level 5 Measurement: 100 answers, exact scoring and IDK, five matched forms, closed boundaries, dimensioned and composite areas, time-system conversions, construction without answer snapping/feedback, and lesson assets pass.');
