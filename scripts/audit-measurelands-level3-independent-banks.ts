...
function runtimeIssues(item: CandidateItem): string[] {
  ...
  const domains: Record<string, string> = { AC9M3M01: "unit_estimate", AC9M3M02: "instrument", AC9M3M03: "duration", AC9M3M04: "clock", AC9M3M05: "angle" };
  if (payload.domain !== domains[item.primaryDescriptorCode]) issues.push("renderer domain does not match descriptor");
  if (/money|dollar|coin|perimeter|surface area|temperature|cubic/i.test(`${item.prompt} ${JSON.stringify(item.renderer.payload)}`)) issues.push("item contains content outside the approved Level 3 scope");
  return issues;
}

const pretest = YEAR3_MEASURELANDS_INDEPENDENT_PRETEST_ITEMS as readonly CandidateItem[];
const posttest = YEAR3_MEASURELANDS_INDEPENDENT_POSTTEST_ITEMS as readonly CandidateItem[];
const allItems = [...pretest, ...posttest];
const pretestStandard = getMeasurelandsFormStandard(3, "pretest");
const posttestStandard = getMeasurelandsFormStandard(3, "posttest");
check(Boolean(pretestStandard), "Missing Level 3 pre-test standard.");
check(Boolean(posttestStandard), "Missing Level 3 post-test standard.");
if (pretestStandard) { const issues = validateIndependentMeasurelandsForm(pretestStandard, pretest); check(issues.length === 0, `Level 3 pre-test blueprint failure: ${issues.join(" | ")}`); }
if (posttestStandard) { const issues = validateIndependentMeasurelandsForm(posttestStandard, posttest); check(issues.length === 0, `Level 3 post-test blueprint failure: ${issues.join(" | ")}`); }
const parallelIssues = validateParallelMeasurelandsForms(pretest, posttest);
check(parallelIssues.length === 0, `Level 3 parallel-form failure: ${parallelIssues.join(" | ")}`);
check(pretest.length === 20 && posttest.length === 20, "Level 3 forms must contain 20 items each.");
check(new Set(allItems.map((item) => item.id)).size === 40, "Level 3 candidate IDs must be globally unique.");
check(new Set(allItems.map((item) => item.prompt)).size === 40, "Level 3 candidate prompts must be unique.");
check(new Set(allItems.map((item) => JSON.stringify(item.renderer.payload))).size === 40, "Level 3 candidates reuse a payload.");
...
for (const [label, form] of [["pre-test", pretest], ["post-test", posttest]] as const) {
  check(form.filter((item) => item.type === "numeric").length === 16, `Level 3 ${label} must have 16 generated responses.`);
  check(form.filter((item) => item.type === "mcq").length === 4, `Level 3 ${label} must have 4 selected responses.`);
}
const source = fs.readFileSync(path.join(process.cwd(), "data/assessments/year3MeasurelandsIndependentBanks.ts"), "utf8");
check(!source.includes("year3Measurelands/registry") && !source.includes("getY3MeasurelandsLessonQuizContribution") && !source.includes("PracticeTask"), "Level 3 bank imports lesson or weekly-quiz content.");
check(YEAR3_MEASURELANDS_PRETEST.every((item) => !("origin" in item)), "Level 3 legacy pre-test archive must remain distinguishable.");
check(YEAR3_MEASURELANDS_POSTTEST.every((item) => !("origin" in item)), "Level 3 legacy post-test archive must remain distinguishable.");
check(MEASURELANDS_FORM_MIGRATIONS.filter((migration) => migration.key.startsWith("3:")).every((migration) => migration.replacementStatus === "production_uncalibrated" && migration.productionReleaseGate === "approved" && migration.legacyStatus === "retired"), "Level 3 migration records must identify production banks and retired legacy forms.");
console.log(`Level 3 independent-bank audit: ${checksPassed} passed, ${failures.length} failed.`);
